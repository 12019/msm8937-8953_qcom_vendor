...
public class IrisPreview2 {
    ...
    private final CameraDevice.StateCallback mStateCallback = new CameraDevice.StateCallback() {
        ...
        public void onDisconnected(@NonNull CameraDevice cameraDevice) {
            ...
            cameraDevice.close();
            mCameraDevice = null;
            Log.e(TAG, "device onDisconnected");
        }
    ...
    private void setUpCameraOutputs() {
        try {
                CameraCharacteristics characteristics
                        = mManager.getCameraCharacteristics(mCameraId);

                // We don't use a front facing camera in this sample.
                Integer facing = characteristics.get(CameraCharacteristics.LENS_FACING);
                if (facing != null && facing == CameraCharacteristics.LENS_FACING_FRONT) {
                   Log.e(TAG, "not front facing camera");
                    //return;
                }
                mSensorSize  = characteristics.get(CameraCharacteristics.SENSOR_INFO_ACTIVE_ARRAY_SIZE);
                Log.e(TAG, "sensor size width= " + mSensorSize.width() +" height=" + mSensorSize.height());

                StreamConfigurationMap map = characteristics.get(
                                        CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP);
                if (map == null) {
                    Log.e(TAG, "cannot get configuration map");
                    return;
                }
                Size[] choices = map.getOutputSizes(SurfaceTexture.class);
                for (Size option : choices) {
                    Log.e(TAG, "preview width="+option.getWidth()+" height="+option.getHeight());
                }
                mPreviewSize = new Size(choices[0].getWidth(), choices[0].getHeight());
                return;
        } catch (CameraAccessException e) {
            e.printStackTrace();
    ...
    private void closeCamera() {
        ...
        try {
            ...
            Log.e(TAG, "acquired lock");
            if (null != mCaptureSession) {
                mCaptureSession.close();
                mCaptureSession = null;
            }
            Log.e(TAG, "closing capture session");
            if (null != mCameraDevice) {
                mCameraDevice.close();
                mCameraDevice = null;
            }
            Log.e(TAG, "closing device");
        } catch (InterruptedException e) {
