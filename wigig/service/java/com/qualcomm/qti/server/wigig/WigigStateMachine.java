...
public class WigigStateMachine extends StateMachine {
    ...
    // choose a default score higher than WIFI score but lower than
    // ethernet score
    private static int DEFAULT_SCORE = NetworkAgent.WIFI_BASE_SCORE + 9;
    ...
    private void handleSupplicantConnectionLoss() {
        ...
        mWigigNative.closeSupplicantConnection();
    ...
    private void setFstPropEnabled(boolean enabled) {
        ...
        SystemProperties.set("persist.fst.rate.upgrade.en", status);
        /* in case Wifi STA+SAP concurrency enabled, FST AP musn't be enabled as Wigig doesn't
         * support concurrency
         */
        if (!mWifiManager.getWifiStaSapConcurrency()) {
            SystemProperties.set("persist.fst.softap.en", status);
        }
    }
    ...
    private void startSoftApWithConfig(final WifiConfiguration configuration) {
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                try {
                    mNwService.startWigigAccessPoint();
                } catch(RemoteException e) {
                    loge("Failed to start Wigig soft AP " + e);
    ...
    class DisconnectedState extends State {
        public void enter() {
            ...
            /**
             * when saved networks exists, kickoff supplicant's periodic scan with an
             * initial scan request. In case there are no saved networks, application
             * is expected to do periodic scans
             */
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                case CMD_STOP_AP:
                    ...
                    try {
                        mNwService.stopWigigAccessPoint();
                    } catch(RemoteException e) {
                        logd("Failed to stop Wigig soft AP " + e);
                    }
