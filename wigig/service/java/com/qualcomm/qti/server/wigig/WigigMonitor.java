   ...
   /* Network connection completed */
    ...
    private static class WigigMonitorSingleton {
        ...
        public synchronized boolean startMonitoring(String iface) {
            ...
            if (mConnected) {
                ...
                int connectTries = 0;
                while (true) {
                    if (mWigigNative.connectToSupplicant()) {
                        m.mMonitoring = true;
                        m.mStateMachine.sendMessage(SUP_CONNECTION_EVENT);
                        mConnected = true;
                        new MonitorThread(mWigigNative, this).start();
                        return true;
                    }
                    if (connectTries++ < 5) {
                        try {
    ...
    private static class MonitorThread extends Thread {
        ...
        public MonitorThread(WigigNative wigigNative, WigigMonitorSingleton wigigMonitorSingleton) {
            super("WigigMonitor");
            mWigigNative = wigigNative;
            mWigigMonitorSingleton = wigigMonitorSingleton;
        }

        public void run() {
            if (DBG) {
                Log.d(TAG, "MonitorThread start with mConnected=" +
                     mWigigMonitorSingleton.mConnected);
            }
            for (;;) {
                if (!mWigigMonitorSingleton.mConnected) {
                    if (DBG) Log.d(TAG, "MonitorThread exit because mConnected is false");
                    break;
                }
                String eventStr = mWigigNative.waitForEvent();

                if (mWigigMonitorSingleton.dispatchEvent(eventStr)) {
