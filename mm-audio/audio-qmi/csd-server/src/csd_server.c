                                              ...
                                              void **connection_handle)
{
    ...
    LOGD("%s qmi_audio_connect_cb() IN\n",csd_log_time());

    if (get_new_client_handle_from_pool(client_handle,
                                        connection_handle) != 0) {
        LOGE("%s Failed to get free client handle, Max no of clients reached\n",csd_log_time());
        rc = QMI_CSI_CB_CONN_REFUSED;
    }

    LOGD("%s qmi_audio_connect_cb() OUT rc=%d\n",csd_log_time(), rc);
    return rc;
}
                                     ...
                                     void *service_cookie)
{
    ...
    LOGD("%s qmi_audio_disconnect_cb called on client 0x%08X",csd_log_time(),clnt_data);
                                                ...
                                                void *req_c_struct)
{
    ...
    LOGD("%s csd_server: csd_server_close_handle IN  \n",csd_log_time());
    handle = ((qmi_csd_close_req_msg_v01*)(req_c_struct))->handle;
    LOGD("%s csd_server:csd_close_req_handler CSD_CLOSE handle =0x%08X\n",csd_log_time(), handle);

    rc = csd_alsa_close_handle(clnt_data, handle);
    if (rc != 0) {
        LOGE("%s Failed to close handle=0x%08X rc=%d\n",csd_log_time(),handle, rc);
    }

EXIT:
    LOGD("%s csd_server:csd_close_req_handler CSD_CLOSE rc =0x%x\n",csd_log_time(), rc);
    ...
    resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp, sizeof(resp));
    if (resp_err != QMI_CSI_NO_ERR) {
        LOGD("%s csd_close_req_handler qmi_csi_send_resp error=0x%x\n",csd_log_time(), resp_err);
        rc = QMI_CSI_CB_INTERNAL_ERR;
    }

    LOGD("%s csd_server: csd_server_close_handle EXIT  \n",csd_log_time());
    return rc;
}
                                                 ...
                                                 void *req_c_struct)
{
    ...
    LOGD("%s csd_server_ioctl_dev_cmd_enable() IN\n",csd_log_time());
    ...
    if (no_of_devices != MAX_NO_ACTIVE_DEVICES) {
        rc = 1;
        LOGE("%s Cannot set devices, parameter list of devices=%d doesnt match"
             "required number=%d\n",csd_log_time(), no_of_devices, MAX_NO_ACTIVE_DEVICES);
        goto EXIT;
    }
    ...
    for (j = 0; j < no_of_devices; j++) {
        if (device_data->devices[j].dev_id != 0) {
            rc = 1;
            LOGE("%s Disable old devices before enabling new ones\n",csd_log_time());
            goto EXIT;
        }
        ...
        LOGD("%s dev_id[%08X] sr[%08X] bps[%08X]\n",csd_log_time(),
    ...
    if (rc != 0)
        LOGE("%s csd_alsa_enable_device() failed rc=%d\n",csd_log_time(), rc);

EXIT:
    LOGD("%s CSD_DEV_CMD_ENABLE rc =0x%x\n",csd_log_time(), rc);
    ...
    if (rc != 0) {
        ...
        LOGD("%s CSD_DEV_CMD_ENABLE SUCCESS => rc =0x%x\n",csd_log_time(), rc);
    ...
    if (resp_err != QMI_CSI_NO_ERR) {
        LOGE("%s Error qmi_csi_send_resp resp_err=%d\n",csd_log_time(), resp_err);
        rc = QMI_CSI_CB_INTERNAL_ERR;
    } else {
        rc = QMI_CSI_CB_NO_ERR;
    }
    LOGD("%s csd_server_ioctl_dev_cmd_enable() EXIT rc=%d\n",csd_log_time(),rc);
    return rc;
}
                                                 ...
                                                 void *req_c_struct)
{
    ...
    LOGD("%s csd_server_ioctl_dev_afelb() IN\n",csd_log_time());
    ...
    {
      rc = 1;
      LOGD("%s csd_server_ioctl_dev_afelb() failed due to bad param \n",csd_log_time());
    }

    if (rc != 0)
       LOGE("%s csd_alsa_afelb() failed rc=%d\n",csd_log_time(), rc);

EXIT:
    LOGD("%s CSD_DEV_CMD_AFELB rc =0x%x\n",csd_log_time(), rc);
    memset(&resp_msg, 0, sizeof(qmi_csd_dev_cmd_resp_msg));
    if (rc != 0) {
        ...
        LOGD("%s CSD_DEV_CMD_AFELB SUCCESS => rc =0x%x\n",csd_log_time(),rc);
    ...
    resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp_msg, respmsg_size);
    if (resp_err != QMI_CSI_NO_ERR) {
        LOGE("%s Error qmi_csd_ioctl_dev_cmd_afelb_req_handler resp_err=%d\n",csd_log_time(), resp_err);
        rc = QMI_CSI_CB_INTERNAL_ERR;
    } else {
        rc = QMI_CSI_CB_NO_ERR;
    }
    LOGD("%s csd_server_ioctl_dev_cmd_afelb() OUT rc=%d\n",csd_log_time(), rc);
    return rc;
}
                                                ...
                                                void *req_c_struct)
{
    ...
    LOGD("%s csd_server:csd_open_device_control_req_handler IN \n",csd_log_time());
    rc = csd_create_device_handle(clnt_data, &dev_handle);
    if (rc != 0)
        LOGE("%s Failed csd_create_device_handle handle rc=%d\n",csd_log_time(),rc);
                                                              ...
                                                              void *req_c_struct)
{
    ...
    LOGD("%s csd_server:csd_server_open_passive_control_voice_stream IN\n",csd_log_time());
    ...
    if (rc != 0)
        LOGE("%s FAILED csd_create_voice_stream_handle() rc=%d\n",csd_log_time(),rc);

EXIT:
    memset(&resp, 0,
           sizeof(qmi_csd_open_passive_control_voice_stream_resp_msg_v01));
    LOGD("%s CSD_OPEN_VOICE_STREAM rc=%d, vs_handle=0x%x\n",csd_log_time(), rc, vs_handle);
    if (rc != 0) {
        resp.resp.result = QMI_RESULT_FAILURE_V01;
        resp.resp.error = QMI_ERR_GENERAL_V01;
        resp.open_status_valid = true;
    ...
    resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp, sizeof(resp));
    if (resp_err != QMI_CSI_NO_ERR) {
        LOGD("%s Failed csd_server:qmi_csi_send_resp error=%d\n",csd_log_time(), resp_err);
        rc = QMI_CSI_CB_INTERNAL_ERR;
    } else {
        rc = QMI_CSI_CB_NO_ERR;
    }

    LOGD("%s csd_server_open_passive_control_voice_stream() EXIT rc=%d\n",csd_log_time(), rc);
    return rc;
}
                                               ...
                                               void *req_c_struct)
{
    ...
    LOGD("%s csd_server_open_voice_context IN\n",csd_log_time(), msg_id);
    ...
    rc = csd_create_voice_context_handle(clnt_data, req_ptr->session_name,
                                         &vc_handle);
    if (rc != 0) {
        LOGE("%s Failed csd_create_voice_context_handle() rc=%d\n",csd_log_time(), rc);
        goto EXIT;
    }
...
EXIT:
    ...
    if (rc != 0) {
        ...
        LOGD("%s CSD_OPEN_VOICE_CONTEXT SUCCESS => rc =0x%x\n",csd_log_time(), rc);
        resp.open_status_valid = false;
    ...
    resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp, sizeof(resp));
    if (resp_err != QMI_CSI_NO_ERR) {
        LOGD("%s csd_open_voice_context send_resp error=%d\n",csd_log_time(), resp_err);
        rc = QMI_CSI_CB_INTERNAL_ERR;
    } else {
        rc = QMI_CSI_CB_NO_ERR;
    }
    LOGD("%s csd_server_open_voice_context() EXIT rc=%d\n",csd_log_time(),rc);
    return rc;
}
                                              ...
                                              void *req_c_struct)
{
    ...
    LOGD("%s csd_server_open_voice_manage  IN msg_id=0x%x\n",csd_log_time(),msg_id);
    ...
    rc = csd_create_voice_manager_handle(clnt_data, req_ptr->session_name,
                                         &vm_handle);
    if (rc != 0) {
        LOGE("%s FAILED csd_create_voice_manager_handle() rc=%d\n",csd_log_time());
        goto EXIT;
    }

EXIT:
    LOGD("%s csd_create_voice_manager_handle() rc=%d, vm_handle=%d\n",csd_log_time(),rc, vm_handle);
    memset(&resp, 0, sizeof(qmi_csd_open_voice_manager_resp_msg_v01));
    if (rc != 0) {
        LOGD("%s CSD_OPEN_VOICE_MANAGER FAILED => rc =0x%x\n",csd_log_time(), rc);
        resp.resp.result = QMI_RESULT_FAILURE_V01;
        resp.resp.error = QMI_ERR_GENERAL_V01;
        resp.open_status_valid = true;
        resp.open_status = QMI_CSD_EFAILED_V01;
        ...
        // open succeed
        LOGD("%s CSD_OPEN_VOICE_MANAGER SUCCESS => rc =0x%x\n",csd_log_time(),rc);
        resp.open_status_valid = false;
    ...
    resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp, sizeof(resp));
    if (resp_err != QMI_CSI_NO_ERR) {
        LOGD("%s csd_server_open_voice_manage send_resp error=%d\n",csd_log_time(), resp_err);
        rc = QMI_CSI_CB_INTERNAL_ERR;
    } else {
        rc = QMI_CSI_CB_NO_ERR;
    }
    LOGD("%s csd_server_open_voice_manage() EXIT rc=%d\n",csd_log_time(), rc);
    return rc;
}
                                                       ...
                                                       void *req_c_struct)
{
    ...
    LOGD("%scsd_server_ioctl_vc_set_device_config() IN\n",csd_log_time());
    vc_handle = qmi_csd_vc_req->u.qmi_csd_set_device_config_req.handle;
    if (!is_valid_voice_context_handle(clnt_data, vc_handle)) {
        LOGE("%s Failed is_valid_voice_context_handle() vc_handle=%08x",csd_log_time(),
             vc_handle);
        rc = 1;
        goto EXIT;
    }
    ...
    if (qmi_csd_vc_req->u.qmi_csd_set_device_config_req.ec_ref_dev_num_valid) {
        LOGE("%s CSD doesnt support setting EC\n",csd_log_time());
        rc = 1;
    ...
    LOGD("%s csd_server:qmi_csd_ioctl_vc_cmd_req_handler cmd_token[%08X]\n"
...
EXIT:
    LOGD("%s csd_server_ioctl_vc_set_device_config() rc=%d\n",csd_log_time(),rc);
    if (rc != 0) {
        LOGE("%s CSD_VC_IOCTL_SET_DEVICE_CONFIG FAILED rc [%08X]\n",csd_log_time(), rc);
        ...
        resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
                                      resp_msg_size);
        if (resp_err != QMI_CSI_NO_ERR) {
            LOGD("%s csd_server:csd_server_ioctl_vc_set_device_config()\n"
                 "send_resp error=%d\n",csd_log_time(), resp_err);
            rc = QMI_CSI_CB_INTERNAL_ERR;
        }
    }

    LOGD("%s csd_server_ioctl_vc_set_device_config() EXIT rc=%d\n",csd_log_time(), rc);
    return rc;
}
                                                ...
                                                void *req_c_struct)
{
    ...
    LOGD("%s csd_server_ioctl_vc_cmd_enable() IN\n",csd_log_time());

    vc_handle = qmi_csd_vc_req->u.qmi_csd_enable_req.handle;
    if (!is_valid_voice_context_handle(clnt_data, vc_handle)) {
        LOGE("%s Failed is_valid_voice_context_handle vc_handle=0x%08x\n",
             csd_log_time(),vc_handle);
        rc = 1;
        goto EXIT;
    }
...
EXIT:
    LOGD("%s csd_server_ioctl_vc_cmd_enable() rc=%d\n",csd_log_time(),rc);
    if (rc != 0) {
        LOGE("%s CSD_VC_IOCTL_ENABLE FAILED\n",csd_log_time());
        ...
        resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
                                     resp_msg_size);
        if (resp_err != QMI_CSI_NO_ERR) {
            LOGE("%s csd_server_ioctl_vc_cmd_enable() send_resp error=0x%x\n",
                 csd_log_time(),resp_err);
            rc = QMI_CSI_CB_INTERNAL_ERR;
        } else {
            rc = QMI_CSI_CB_NO_ERR;
        }
        LOGD("%s csd_server_ioctl_vc_cmd_enable() EXIT rc=%d\n",csd_log_time(),rc);
        return rc;
    }
    LOGD("%s csd_server_ioctl_vc_cmd_enable() EXIT \n ",csd_log_time());
...
EXIT:
    LOGD("%s csd_server_ioctl_vm_cmd_attach_context() rc=%d\n",csd_log_time(),rc);
    if (rc != 0) {
        LOGE("%s IOCTL_VM_CMD_ATTACH_CONTEXT_REQ_V01 FAILED rc=%d\n",csd_log_time(),rc);
        ...
        resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
                                     resp_msg_size);
        if (resp_err != QMI_CSI_NO_ERR) {
            LOGE("%s csd_server_ioctl_vm_cmd_attach_context()=%d\n",csd_log_time(),resp_err);
            rc = QMI_CSI_CB_INTERNAL_ERR;
        } else {
            rc = QMI_CSI_CB_NO_ERR;
        }
        LOGD("%s csd_server_ioctl_vm_cmd_attach_context() EXIT rc=%d\n",csd_log_time(),rc);
        return rc;
    }
    LOGD("%s csd_server_ioctl_vm_cmd_attach_context() EXIT \n ",csd_log_time());
    return QMI_CSI_CB_NO_ERR;
}
...
EXIT:
    ...
    if (rc != 0) {
        LOGE("%s CSD_VC_IOCTL_SET_HDVOICE FAILED rc=%d\n",csd_log_time(),rc);
        ...
        resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
                                                                resp_msg_size);
        if (resp_err != QMI_CSI_NO_ERR) {
            LOGD("%s csd_server_ioctl_vm_cmd_set_hdvoice_mode qmi_csi_send_resp\n"
                 "resp_err=%d\n",csd_log_time(), resp_err);
        ...
        LOGD("%s csd_server_ioctl_vm_cmd_set_hdvoice_mode() EXIT rc=%d\n",csd_log_time(),rc);
        return rc;
    }
    LOGD("%s csd_server_ioctl_vm_cmd_set_hdvoice_mode() EXIT ",csd_log_time());
    return QMI_CSI_CB_NO_ERR;
}
                                                       ...
                                                       void *req_c_struct)
{
    ...
    LOGD("%s csd_server_ioctl_vm_cmd_set_widevoice() IN\n",csd_log_time());
...
EXIT:
    LOGD("%s csd_server_ioctl_vm_cmd_set_widevoice() rc=%d\n",csd_log_time(),rc);

    if (rc != 0) {
        LOGE("%s CSD_VC_IOCTL_SET_WIDE_VOICE FAILED rc=%d\n",csd_log_time(),rc);
        ...
        resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
                                                                resp_msg_size);
        if (resp_err != QMI_CSI_NO_ERR) {
            LOGD("%s csd_server_ioctl_vm_cmd_set_widevoice qmi_csi_send_resp\n"
                 "resp_err=%d\n",csd_log_time(),resp_err);
            rc = QMI_CSI_CB_INTERNAL_ERR;
        } else {
            rc = QMI_CSI_CB_NO_ERR;
        }
        LOGD("%s csd_server_ioctl_vm_cmd_set_widevoice() EXIT rc=%d\n",csd_log_time(),rc);
        return rc;
    }

    LOGD("%s csd_server_ioctl_vm_cmd_set_widevoice() EXIT ",csd_log_time());
    return QMI_CSI_CB_NO_ERR;
}
                                                      ...
                                                      void *req_c_struct)
{
    ...
    LOGD("%s csd_server_ioctl_vm_cmd_set_tty_mode()\n",csd_log_time());
...
EXIT:
    LOGD("%s csd_server_ioctl_vm_cmd_set_tty_mode() rc=%d\n",csd_log_time(),rc);
    if (rc != 0) {
        LOGE("%s CSD_VM_IOCTL_SET_TTY FAILED rc [%08X]\n",csd_log_time(),rc);
        ...
        resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
                                     resp_msg_size);
        if (resp_err != QMI_CSI_NO_ERR){
            LOGE("%s csd_server_ioctl_vm_cmd_set_tty_mode() send_resp error=%d\n",
                  csd_log_time(),resp_err);
            rc = QMI_CSI_CB_INTERNAL_ERR;
        } else {
            rc = QMI_CSI_CB_NO_ERR;
        }
        LOGD("%s csd_server_ioctl_vm_cmd_set_tty_mode() EXIT rc=%d\n",csd_log_time(),rc);
        return rc;
    }
    LOGD("%s csd_server_ioctl_vm_cmd_set_tty_mode() EXIT ",csd_log_time());
    return QMI_CSI_CB_NO_ERR;
}
                                                         ...
                                                         void *req_c_struct)
{
    ...
    LOGD("%s csd_server_ioctl_vs_cmd_set_ui_property() IN\n",csd_log_time());

    vs_handle = qmi_csd_vs_req->u.qmi_csd_get_ui_prop_req.handle;
    if (!is_valid_voice_stream_handle(clnt_data, vs_handle)) {
        LOGE("%s Failed is_valid_voice_stream_handle() vs_handle=0x%08x\n",
...
EXIT:
    LOGD("%s csd_server_ioctl_vs_cmd_set_ui_property() rc=%d\n",csd_log_time(),rc);
    if (rc != 0) {
        LOGE("%s CSD_VS_IOCTL_SET_UI_PROPERTY FAILED rc=%d\n",csd_log_time(),rc);
        ...
        resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
                                     resp_msg_size);
        if (resp_err != QMI_CSI_NO_ERR) {
            LOGE("%s csd_server_ioctl_vs_cmd_set_ui_property send_resp error=%d\n",
                 csd_log_time(),resp_err);
            rc = QMI_CSI_CB_INTERNAL_ERR;
        } else {
            rc = QMI_CSI_CB_NO_ERR;
        }
        LOGD("%s csd_server_ioctl_vs_cmd_set_ui_property() EXIT rc=%d\n",csd_log_time(), rc);
        return rc;
    }
    LOGD("%s csd_server_ioctl_vs_cmd_set_ui_property() EXIT ",csd_log_time());
    return QMI_CSI_CB_NO_ERR;
}
                                                     ...
                                                     void *req_c_struct)
{
    ...
    LOGD("%s csd_server_ioctl_vm_cmd_start_voice IN\n",csd_log_time());
...
EXIT:
    LOGD("%s csd_server_ioctl_vm_cmd_start_voice() rc=%d\n",csd_log_time(),rc);
    if (rc != 0) {
        LOGE("%s CSD_VM_IOCTL_START_VOICE FAILED rc=%d\n",csd_log_time(),rc);
        ...
        resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
                                     resp_msg_size);
        if (resp_err != QMI_CSI_NO_ERR) {
            LOGD("%s csd_server_ioctl_vm_cmd_start_voice() send_resp error=%d\n",
                 csd_log_time(),resp_err);
            rc = QMI_CSI_CB_INTERNAL_ERR;
        } else {
            rc = QMI_CSI_CB_NO_ERR;
        }
        LOGD("%s csd_server_ioctl_vm_cmd_start_voice() EXIT rc=%d\n",csd_log_time(),rc);
        return rc;
    }
    LOGD("%s csd_server_ioctl_vm_cmd_start_voice() EXIT \n",csd_log_time());
    return QMI_CSI_CB_NO_ERR;
}
                                                    ...
                                                    void *req_c_struct)
{
    ...
    qmi_csi_error resp_err;
    int rc = 0;
    LOGD("%s csd_server_ioctl_vm_cmd_stop_voice() IN\n",csd_log_time());
    vm_handle = qmi_csd_vm_req->u.qmi_csd_start_voice_req.handle;
    if (!is_valid_voice_manager_handle(clnt_data, vm_handle)) {
        LOGE("%s is_valid_voice_manager_handle() vm_handle=0x%08x\n",csd_log_time(),vm_handle);
        rc = 1;
        goto EXIT;
    }
...
EXIT:
    LOGD("%s csd_server_ioctl_vm_cmd_stop_voice() rc=%d\n",csd_log_time(),rc);
    if (rc != 0) {
        LOGE("%s CSD_VM_IOCTL_STOP_VOICE rc=%d\n",csd_log_time(),rc);
        ...
        if (resp_err != QMI_CSI_NO_ERR) {
            LOGE("%s csd_server_ioctl_vm_cmd_stop_voice() send_resp error=0x%x\n",
                 csd_log_time(),resp_err);
            rc = QMI_CSI_CB_INTERNAL_ERR;
        } else {
            rc = QMI_CSI_CB_NO_ERR;
        }
        LOGD("%s csd_server_ioctl_vm_cmd_stop_voice() EXIT rc=%d\n",csd_log_time(),rc);
        return rc;
    }
    LOGD("%s csd_server_ioctl_vm_cmd_stop_voice() EXIT ",csd_log_time());
    return QMI_CSI_CB_NO_ERR;
}
                                                       ...
                                                       void *req_c_struct)
{
    ...
    qmi_csi_error resp_err;
    int rc = 0;
    LOGD("%s csd_server_ioctl_vm_cmd_standby_voice() IN\n",csd_log_time());
    vm_handle = qmi_csd_vm_req->u.qmi_csd_standby_voice_req.handle;
    if (!is_valid_voice_manager_handle(clnt_data, vm_handle)) {
        LOGE("%s is_valid_voice_manager_handle() FAILED vm_handle=0x%08x\n",
             csd_log_time(),vm_handle);
        rc = 1;
        goto EXIT;
    }
...
EXIT:
    LOGD("%s csd_server_ioctl_vm_cmd_standby_voice() rc=%d\n",csd_log_time(),rc);
    if ((rc != 0)) {
        LOGE("%s CSD_VM_IOCTL_STANDBY_VOICE rc=%d\n",csd_log_time(),rc);
        ...
        resp_err = qmi_csi_send_resp(req_handle, msg_id,
                                      &resp_msg, resp_msg_size);
        if (resp_err != QMI_CSI_NO_ERR) {
            LOGE("%s csd_server_ioctl_vm_cmd_standby_voice() send_resp err=0x%x\n",
                 csd_log_time(),resp_err);
            rc = QMI_CSI_CB_INTERNAL_ERR;
        } else {
            rc = QMI_CSI_CB_NO_ERR;
        }
        LOGD("%s csd_server_ioctl_vm_cmd_standby_voice() EXIT rc=%d\n",csd_log_time(),rc);
        return rc;
    }
    LOGD("%s csd_server_ioctl_vm_cmd_standby_voice() EXIT ",csd_log_time());
    return QMI_CSI_CB_NO_ERR;
}
                                                             ...
                                                             void *req_c_struct)
{
    ...
    LOGD("%s csd_server_ioctl_vc_cmd_set_rx_volume_index() IN\n",csd_log_time());
    vc_handle = qmi_csd_vc_req->u.qmi_csd_set_device_config_req.handle;
    if (!is_valid_voice_context_handle(clnt_data, vc_handle)) {
        LOGE("%s is_valid_voice_context_handle FAILED vc_handle=0x%08x\n",
              csd_log_time(),vc_handle);
        rc = 1;
        goto EXIT;
    }
...
EXIT:
    LOGD("%s csd_server_ioctl_vc_cmd_set_rx_volume_index() rc=%d\n",csd_log_time(),rc);
    if (rc != 0) {
        LOGE("%s CSD_VC_IOCTL_SET_RX_VOLUME FAILED rc=%d\n",csd_log_time(),rc);
        ...
        resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
                                     resp_msg_size);
        if (resp_err != QMI_CSI_NO_ERR) {
            LOGE("%s csd_server_ioctl_vc_cmd_set_rx_volume_index()\n"
                 "send_resp error=%d\n",csd_log_time(),resp_err);
            rc = QMI_CSI_CB_INTERNAL_ERR;
        } else {
            rc = QMI_CSI_CB_NO_ERR;
        }
        LOGD("%s csd_server_ioctl_vc_cmd_set_rx_volume_index() EXIT rc=%d\n",csd_log_time(),rc);
        return rc;
    }
    LOGD("%s csd_server_ioctl_vc_cmd_set_rx_volume_index() EXIT ",csd_log_time());
    return QMI_CSI_CB_NO_ERR;
}
                                                  ...
                                                  void *req_c_struct)
{
    ...
    LOGD("%s csd_server_ioctl_vs_cmd_set_mute() IN\n",csd_log_time());
    vs_handle = qmi_csd_vs_req->u.qmi_csd_get_ui_prop_req.handle;
    if (!is_valid_voice_stream_handle(clnt_data, vs_handle)) {
        rc = 1;
        goto EXIT;
...
EXIT:
    LOGD("%s csd_server_ioctl_vs_cmd_set_mute() rc=%d\n",csd_log_time(),rc);
    if (rc != 0) {
        LOGE("%s CSD_VS_IOCTL_SET_MUTE FAILED rc=%d\n",csd_log_time(),rc);
        resp_msg.u.qmi_csd_set_ui_prop_resp.resp.result =
                                                        QMI_RESULT_FAILURE_V01;
        resp_msg.u.qmi_csd_set_ui_prop_resp.resp.error = QMI_ERR_GENERAL_V01;
        resp_msg.u.qmi_csd_set_ui_prop_resp.qmi_csd_status_code_valid = true;
        ...
        resp_err = qmi_csi_send_resp(req_handle, msg_id,
                                     &resp_msg, resp_msg_size);
        if (resp_err != QMI_CSI_NO_ERR) {
            LOGE("%s csd_server_ioctl_vs_cmd_set_mute() send_resp error=%d\n",
                  csd_log_time(),resp_err);
            rc = QMI_CSI_CB_INTERNAL_ERR;
        } else {
            rc = QMI_CSI_CB_NO_ERR;
        }
        LOGD("%s csd_server_ioctl_vs_cmd_set_mute() EXIT rc=%d\n",csd_log_time(),rc);
        return rc;

    }
    LOGD("%s csd_server_ioctl_vs_cmd_set_mute() EXIT ",csd_log_time());
    return QMI_CSI_CB_NO_ERR;
}
                                                       ...
                                                       void *req_c_struct)
{
    ...
    LOGD("%s csd_server_ioctl_vm_cmd_detach_context() IN\n",csd_log_time());
...
EXIT:
    LOGD("%s csd_server_ioctl_vm_cmd_detach_context() rc=%d\n",csd_log_time(),rc);
    if (rc != 0) {
        LOGE("%s IOCTL_VM_CMD_DETACH_CONTEXT_REQ_V01 FAILED rc %d\n",csd_log_time(),rc);
        ...
        resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
                                     resp_msg_size);
        if (resp_err != QMI_CSI_NO_ERR) {
            LOGE("%s csd_server_ioctl_vm_cmd_detach_context send_resp err=0x%x\n",
                 csd_log_time(),resp_err);
            rc = QMI_CSI_CB_INTERNAL_ERR;
        } else {
            rc = QMI_CSI_CB_NO_ERR;
        ...
        return rc;
        LOGD("%s csd_server_ioctl_vm_cmd_detach_context() EXIT rc=%d\n",csd_log_time(),rc);
    }
    LOGD("%s csd_server_ioctl_vm_cmd_detach_context() EXIT ",csd_log_time());
    return QMI_CSI_CB_NO_ERR;
}
                                                 ...
                                                 void *req_c_struct)
{
    ...
    LOGD("%s csd_server_ioctl_vc_cmd_disable() IN\n",csd_log_time());
...
EXIT:
    LOGD("%s csd_server_ioctl_vc_cmd_disable() rc=%d\n",csd_log_time(),rc);
    if ((rc != 0)) {
        LOGD("%s CSD_VC_IOCTL_DISABLE FAILED rc=%d\n",csd_log_time(),rc);
        ...
        resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
                                     resp_msg_size);
        if (resp_err != QMI_CSI_NO_ERR) {
            LOGD("%s csd_server_ioctl_vc_cmd_disable send_resp error=%d\n",
                 csd_log_time(),resp_err);
            rc = QMI_CSI_CB_INTERNAL_ERR;
        } else {
            rc = QMI_CSI_CB_NO_ERR;
        }
        LOGD("%s csd_server_ioctl_vc_cmd_disable() EXIT rc=%d\n",csd_log_time(), rc);
        return rc;
    }
    LOGD("%s csd_server_ioctl_vc_cmd_disable() EXIT ",csd_log_time());
    return QMI_CSI_CB_NO_ERR;
}
                                                  ...
                                                  void *req_c_struct)
{
    ...
    LOGD("%s csd_server_ioctl_dev_cmd_disable() IN\n",csd_log_time());
    ...
    if (rc != 0)
        LOGE("%s csd_server_ioctl_dev_cmd_disable() failed rc=%d\n",csd_log_time(),rc);

EXIT:
    LOGD("%s csd_server_ioctl_dev_cmd_disable() rc=%d\n",csd_log_time(),rc);
    memset(&resp_msg, 0, sizeof(qmi_csd_dev_cmd_resp_msg));
    if (rc != 0) {
        LOGE("%s CSD_DEV_CMD_DISABLE FAILED rc=%d\n",csd_log_time(),rc);
    ...
    if (resp_err != QMI_CSI_NO_ERR) {
        LOGE("%s csd_server_ioctl_dev_cmd_disable() send_resp err=%d\n",csd_log_time(),resp_err);
        rc = QMI_CSI_CB_INTERNAL_ERR;
    } else {
        rc = QMI_CSI_CB_NO_ERR;
    }
    LOGD("%s csd_server_ioctl_dev_cmd_disable() EXIT rc=%d\n",csd_log_time(),rc);
...
int main(int argc, char **argv)
{
    ...
    LOGD("%s Calling csd_alsa_preinit \n",csd_log_time());
    rc = csd_alsa_preinit();
    if (rc != 0) {
        LOGE("%s csd_alsa_preinit failed error rc=%d\n",csd_log_time(),rc);
        exit(1);
    }
