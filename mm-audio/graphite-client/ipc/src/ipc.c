...
void ipc_resp_thread(void *param __unused)
{
    ...
    while (1) {
        ...
        if ((resp_size = osal_dev_read(gpriv.dev, resp, IPC_RESP_SIZE_MAX)) <= 0) {
            OSAL_LOGE("%s: read failed %d", __func__, resp_size);
        } else {
            hdr = &resp->hdr;
    ...
    OSAL_LOGD("%s: Exiting response thread", __func__);
}
...
static int32_t ipc_write(void *data, uint32_t size, uint8_t pkt_type)
{
    ...
    OSAL_LOGV("%s: write size: %d", __func__, wpkt_size);
                     ...
                     uint16_t opcode, bool use_island)
{
    ...
    if (ret < 0)
        OSAL_LOGE("%s: Failed to send cmd pkt, ret = %d", __func__, ret);
...
int32_t ipc_deregister(uint8_t id)
{
    ...
    if (id == IPC_INVALID_CLIENT_ID) {
        OSAL_LOGE("%s: Invalid client ID", __func__);
        ret = OSAL_EBADPARAM;
        goto done;
    }
...
int32_t ipc_init(void)
{
    ...
    if (++gpriv.init_ref_cnt != 1) {
        OSAL_LOGE("%s: already initialized, ref_cnt = %d", __func__,
                  gpriv.init_ref_cnt);
        goto done;
    }
    ...
    if (ret < 0 ) {
        OSAL_LOGE("%s: dev open failed, ret = %d", __func__, ret);
        goto err_dev_open;
    }
    ...
    if (ret < 0) {
        OSAL_LOGE("%s: Failed to send reg pkt, ret = %d", __func__, ret);
        goto err_free;
    }
    ...
    if (ret < 0) {
        OSAL_LOGE("%s: Failed to create thread %d", __func__, ret);
        goto err_free;
    }
...
int32_t ipc_deinit(void)
{
    ...
    if (--gpriv.init_ref_cnt == 0) {
        ...
        if (ret < 0)
            OSAL_LOGE("%s: Failed to close device ret = %d", __func__, ret);
