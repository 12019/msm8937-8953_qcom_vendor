                      ...
                      const XML_Char **attr)
{
    ...
    OSAL_LOGV("%s: tag %s", __func__, tag_name);
...
static int32_t platform_get_info_from_xml(struct platform_info *pinfo)
{
    ...
    if (!file) {
        OSAL_LOGD("%s: Failed to open %s", __func__, PLATFORM_XML_PATH);
        ret = OSAL_EBADPARAM;
        goto done;
    }
    ...
    if (!parser) {
        OSAL_LOGE("%s: Failed to create XML parser!", __func__);
        ret = OSAL_EBADPARAM;
        goto err_close_file;
    }
    ...
    while (1) {
        ...
        bytes_read = fread(buf, 1, XML_READ_BUF_SIZE, file);
            OSAL_LOGE("%s: bytes_read %d", __func__, bytes_read);
        if (bytes_read < 0) {
...
static int32_t platform_set_default_info(struct platform_info *pinfo)
{
    ...
    if (!pinfo->ch) {
        OSAL_LOGE("%s: Memory allocation failed\n", __func__);
        ret = OSAL_ENOMEMORY;
        goto done;
    }
