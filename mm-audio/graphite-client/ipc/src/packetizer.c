...
int32_t pktzr_resp_cb(void *payload)
{
    ...
    if (!payload) {
        OSAL_LOGE("%s: payload is NULL", __func__);
        ret = OSAL_EBADPARAM;
        goto done;
    }
    ...
    hdr = &pkt->hdr;
    OSAL_LOGD("%s: opcode = 0x%x size = %d token = %d", __func__,
    ...
    if (hdr->opcode == GRAPHITE_EVENT) {
        if (ppriv->evt_cb != NULL)
            ...
            OSAL_LOGE("%s: event callback function is NULL", __func__);
    } else {
        token = hdr->token;
        ...
        osal_list_for_each_entry_safe(struct pktzr_node, wnode, n, &ppriv->wait_list, list) {
            OSAL_LOGV("%s: wnode %p, wnode->token = %d", __func__, wnode, wnode->token);

            if (wnode->token == token) {
                OSAL_LOGD("%s: token %d matched", __func__, token);
                ...
                if (wnode->cmd == GRAPHITE_CMD_DATA) {
                    if (ppriv->data_cmd_cb != NULL)
                        ...
                        OSAL_LOGE("%s: read callback function is NULL", __func__);
...
static int32_t pktzr_send_cmd(void *payload, uint32_t size, struct pktzr_cmd_rsp *rsp,
                              uint16_t cmd, bool use_island, bool sync_cmd)
{
    int32_t ret = OSAL_SUCCESS;
    struct ipc_pkt *pkt;
    struct pktzr_node *wnode;
    uint32_t pkt_size;

    if (!payload || !size || !rsp || !rsp->buf) {
        OSAL_LOGE("%s: Input parameters are NULL", __func__);
        ret = OSAL_EBADPARAM;
        goto done;
    }
    ...
    if (pkt_size > UINT_MAX) {
        OSAL_LOGE("%s: pkt_size is greater than UINT_MAX, size = %d", __func__, size);
    }

    pkt = (struct ipc_pkt *)osal_mem_calloc(1, pkt_size);
    if (!pkt) {
        OSAL_LOGE("%s: Memory allocation failed", __func__);
        ret = OSAL_ENOMEMORY;
        goto err_pkt_alloc;
    }
    ...
    if (!wnode) {
        OSAL_LOGE("%s: Failed to allocate memory", __func__);
        ret = OSAL_ENOMEMORY;
        goto err_node_alloc;
    }
    ...
    osal_list_add_tail(&ppriv->wait_list, &wnode->list);
    ...
    if (ret < 0) {
        OSAL_LOGE("%s: Failed to send pkt", __func__);
        goto err_send_msg;
    }
    osal_mem_free(pkt);
    osal_lock_release(ppriv->lock);
...
err_pkt_alloc:
    ...
    OSAL_LOGV("%s: Done, ret = %d", __func__, ret);
    return ret;
}
...
int32_t pktzr_init(event_cb event_cb_handler, data_cmd_cb data_cmd_cb_handler)
{
    ...
    ret = ipc_init();
    if (ret < 0) {
        OSAL_LOGE("%s: Initialization failed, ret = %d", __func__, ret);
        goto err_init;
    }

    ret = ipc_register(CLIENT_ID_AUDIO, pktzr_resp_cb);
    if (ret < 0) {
        OSAL_LOGE("%s: Failed to register, ret = %d", __func__, ret);
        goto err_reg;
    }
