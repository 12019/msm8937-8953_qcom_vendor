...
public:
    ...
    virtual int setVideoSurfaceProp(int width,int height,int orientation)
    {
        Parcel data, reply;
        data.writeInterfaceToken(IWiFiDisplayService::getInterfaceDescriptor());
        int res = -1;
        ...
        ALOGE("Calling transact in setVideoSurfaceProp");
        res = remote()->transact(SET_VIDEO_SURFACE_PROP, data, &reply);
        CHECK_TRANSACTION(res);
        return reply.readInt32();
    }
    ...
    virtual int getUIBCCategory() {
        Parcel data, reply;
        data.writeInterfaceToken(IWiFiDisplayService::getInterfaceDescriptor());
        int res = -1;
        res = remote()->transact(GET_UIBC_CAT, data, &reply);
        CHECK_TRANSACTION(res);
        return reply.readInt32();
    }
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch (code) {
        ...
        case SET_VIDEO_SURFACE_PROP: {
            CHECK_INTERFACE(IWiFiDisplayService, data, reply);
            int width = data.readInt32();
            int height = data.readInt32();
            int orien = data.readInt32();
            ALOGE("IWiFiDisplayService : on transact");
            reply->writeInt32(setVideoSurfaceProp(width,height,orien));
            return NO_ERROR;
        }
        break;
        ...
        case GET_UIBC_CAT: {
             CHECK_INTERFACE(IWiFiDisplayService, data, reply);
             reply->writeInt32(getUIBCCategory());
             return NO_ERROR;
        }
        break;
