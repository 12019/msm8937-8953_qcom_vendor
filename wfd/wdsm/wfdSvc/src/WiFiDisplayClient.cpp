...
int WiFiDisplayClient::setSurfaceProp(int width , int height , int orientation)
{
    ALOGE("In WiFiDisplayClient::setSurfaceProp -> checking service");
    const sp<IWiFiDisplayService>& pWfdSvc = getWiFiDisplayService();
    if (pWfdSvc != NULL)
    {
        ALOGE("In WiFiDisplayClient::setSurfaceProp");
        return pWfdSvc->setVideoSurfaceProp(width,height,orientation);
    }
    return -1;
}
...
int WiFiDisplayClient::getUIBCSupportCategory()
{
    const sp<IWiFiDisplayService>& pWfdSvc = getWiFiDisplayService();
    if (pWfdSvc != NULL)
    {
        return pWfdSvc->getUIBCCategory();
    }
    return -1;
}
