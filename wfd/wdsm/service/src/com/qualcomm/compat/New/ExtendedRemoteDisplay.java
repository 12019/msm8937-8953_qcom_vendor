...
public final class ExtendedRemoteDisplay {
    ...
    private static final int TEARDOWN_TIMEOUT = 10000; // 10 seconds
    ...
    * underlying WFD service is a single state machine
    ...
    private void disposeInternal(boolean finalized) {
        ...
        */
        ...
        Log.d(TAG, "current sState:" + sState);
        if (sState != WFDState.DEINIT){
            /*
             * Wait for timeout if not in Deinit state already
             */
            blockOnLock(TEARDOWN_TIMEOUT);
        }
