...
import com.qualcomm.wfd.client.net.WifiP2pNetManager;
import com.qualcomm.wfd.client.net.WigigP2pNetManager;
...
import android.net.wifi.p2p.WifiP2pConfig; // keep for p2p configuration
import android.net.wifi.p2p.WifiP2pInfo; // keep for display connection info
...
public class DeviceListFragment extends ListFragment {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (device != null) {
				...
				if (device.isStatusConnected()) {
                    ...
                    WifiP2pInfo info = device.getNetTypeManager().getLocalConnectionInfo();
                    if (info != null) {
                        onConnectionInfoAvailable(device.getNetType(), device.getNetTypeManager().getLocalConnectionInfo());
                    } else {
	...
	private void handleConnectClickHelper(NetDeviceIF device) {
		...
		activity.connect(config, device);
	}
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		activity.connect(config, deviceInfo);
		return true;
	}


	public void onConnectionInfoAvailable(WfdEnums.NetType netType, WifiP2pInfo info) {
	...
	public void onPeersChanged(Collection<NetDeviceIF> peers) {
		...
		if (activity.isDiscoveryRequested || !manualFindOnly) {
	...
	class EventHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case connectionInfoAvailable: {
				WifiP2pInfo info = (WifiP2pInfo)msg.obj;
				...
				if (deviceDetailsFrameLayout != null) {
					...
					int localDeviceType = activity.getLocalDeviceType();
					...
					if (connectedDevice.hasDeviceInfo() && connectedDevice.supportWfd()) {
						...
						if (connectedDevice.getWfdType().getCode() == localDeviceType) {
							Log.d(TAG, "connectionInfoAvailable: localDeviceType- " + localDeviceType +
	...
	public interface DeviceActionListener {
	    ...
	    void connect(WifiP2pConfig config, NetDeviceIF device);
