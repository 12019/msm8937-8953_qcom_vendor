...
public class WifiP2pNetDevice implements NetDeviceIF {
    ...
    public void connect(final ResultListener initListener) {
        ...
        if (mConfig == null) {
            ...
            if (sharedPrefs.getBoolean("perform_custom_group_owner_intent_value",
                    false) == true) {
                mConfig.groupOwnerIntent = Integer.parseInt(sharedPrefs.getString(
                        "group_owner_intent_value", "3"));
            }
            Toast.makeText(mActivity, "group owner intent value: "
    ...
    public String getIp() {
        ...
        //WifiP2pInfo localInfo = getNetTypeManager().getLocalDevice().getConnectionInfo();
        WifiP2pInfo localInfo = getNetTypeManager().getLocalConnectionInfo();
        if (localInfo != null && !localInfo.isGroupOwner) {
            Log.v(TAG, "get ip for group owner");
            return localInfo.groupOwnerAddress.getHostAddress();
        }
    ...
    public boolean hasConnectionInfo() {
        return mWifiP2pInfo != null;
    }

    // not in interface, for local device only
    ...
    */
    ...
    public boolean isGroupOwner() {
        return mDevice != null && mDevice.isGroupOwner();
    }
