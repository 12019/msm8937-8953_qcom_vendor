...
{
    RTSPSession::setSurfaceProp(width,height,orientation);
}
...
/*
 * Method:    getUIBCSupportCategory
 * Finds out what UIBC modes are supported
 * Generic / HID / both / None
 */
unsigned int getUIBCSupportCategory()
{
    return RTSPSession::getNegotiatedUIBCCategory();
}
...
{
  return RTSPSession::executeRuntimeCommand(cmd);
}
...
void eventMMUpdate (MMEventType mmEvent,MMEventStatusType status, long evtData1, long evtData2, long evtData3, long evtData4)
{
  ...
  switch (mmEvent)
  {
    ...
    break;
    case MM_EVENT_STREAMING_REQ:
    {
        if(status == MM_STATUS_SUCCESS)
        {
            if(evtData1 == 0)
            {
                ALOGE("Wifidisplay : StopStreaming Called");
                RTSPSession::StreamingSupportReq(false, NULL);

            }
            else
            {
                MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "Wifidisplay : StartStreaming Called CodecType =%p",(int)evtData1);
                RTSPSession::StreamingSupportReq(true, (void*)evtData1);
            }
        }
    }
    break;
  ...
  case MM_RTP_EVENT_RTCP_RR_MESSAGE:
      {
          ...
          snprintf(strarray[0], WIFIDISPLAY_STRING_SIZE, "RTCPRRMessage");
          snprintf(strarray[1], WIFIDISPLAY_STRING_SIZE, "%ld", evtData1); //Len of message

          if(evtData1 < 0 || !evtData2)
          {
              MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"RTCP Data Size 0 or Invalid ptr");
              return;
          }

          if (evtData1 > WIFIDISPLAY_STRING_SIZE)
          {
              // truncate string as sanity check (should not happen)
              evtData1 = WIFIDISPLAY_STRING_SIZE;
          }

          uint8 *pSrc = reinterpret_cast<uint8*>(evtData2);
          uint32 nSize = evtData1;

          MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "RTCPRRMessage msgsize=%u", nSize);

          memset(strarray[2], 0, sizeof(strarray[2]));
          memcpy(strarray[2], pSrc, nSize);
          strarray[2][WIFIDISPLAY_STRING_SIZE - 1] = '\0';
...
{
  int32 nSleepTime = 0;
  while (gSurface == NULL && (nSleepTime < 5000))//Framework yet to Set Surface
  {
     MM_Timer_Sleep(10);
     nSleepTime += 10;
  }
...
void sendIDRRequest()
{
  MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"Wifidisplay : sendIdrRequest : Called");
