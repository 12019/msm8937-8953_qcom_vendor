...
bool MMAdaptor::mbDirectStreamingReq = false;
...
bool MMAdaptor::updateLocalMMCapability(MMCapability* pLocalMMCapability, DeviceType devType) {
    ...
    WFD_MM_Extended_capability_t *extCap = NULL;
    if(devType == SOURCE)
        extCap = (pLocalMMCapability->plocalExCapability);
    else
        extCap = (pLocalMMCapability->pExtendedCapability);
    wfd_mm_get_local_capability(mmHandle,
                                pLocalMMCapability->pCapability,
                                &(pLocalMMCapability->m_nAVFormatCount),
                                &extCap,
                                &(pLocalMMCapability->pAuxCapability),
                                        ...
                                        MMCapability* pCommonCapability)
{
    ...
    if(pLocalMMCapability->plocalExCapability)
    {
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"Dumping Source Ext Capability - getNegotiatedCapability");
        int count = pLocalMMCapability->plocalExCapability->extended_video_config.extended_video_config.num_codec;
        MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_ERROR,"getNegotiatedCapability : count = %d ", count);
        for(int i = 0; i < count ; i++)
        {
            if((pLocalMMCapability->plocalExCapability->extended_video_config.extended_video_config.codec + i) != NULL)
            {
                MM_MSG_PRIO2(MM_GENERAL,MM_PRIO_ERROR,"getNegotiatedCapability : Name = %s , Profile = %d",
                    pLocalMMCapability->plocalExCapability->extended_video_config.extended_video_config.codec[i].name,
                    pLocalMMCapability->plocalExCapability->extended_video_config.extended_video_config.codec[i].profile);
                MM_MSG_PRIO3(MM_GENERAL,MM_PRIO_ERROR,"getNegotiatedCapability : CEA = %x , Vesa = %x , hh = %x",
                    pLocalMMCapability->plocalExCapability->extended_video_config.extended_video_config.codec[i].supported_cea_mode,
                    pLocalMMCapability->plocalExCapability->extended_video_config.extended_video_config.codec[i].supported_vesa_mode,
                    pLocalMMCapability->plocalExCapability->extended_video_config.extended_video_config.codec[i].supported_hh_mode);
            }
        }
    }
    wfd_mm_get_negotiated_capability(mmHandle,
                                     pLocalMMCapability->pCapability,
                                     pLocalMMCapability->plocalExCapability,
                                     pPeerMMCapability->pCapability,
                                     pPeerMMCapability->pExtendedCapability,
                                     pNegotiatedMMCapability->pCapability,
                                     pNegotiatedMMCapability->pExtendedCapability,
                                     pCommonCapability->pCapability,
                                     pCommonCapability->pExtendedCapability);
    return true;
}
...
bool MMAdaptor::createSession(MMCapability* pMMCapability, DeviceType devType) {
    ...
    if (!mmCreated) {
        ...
        WFD_status_t ret = wfd_mm_create_session(&mmHandle,
                                                 ...
                                                 pMMCapability->pAuxCapability,
                                                 &mmCallbacks,
                                                 pMMCapability->pExtendedCapability);
...
bool MMAdaptor::updateSession(MMCapability* pMMCapability, bool bResetCodec) {
    if (!isMMStreamEnabled()) {
        return true;
    }
    ...
    if (mmCreated) {
        ...
        WFD_status_t ret = wfd_mm_update_session(mmHandle,
                                                 pMMCapability->pCapability,
                                                 pMMCapability->pExtendedCapability,
                                                 pMMCapability->pAuxCapability,
                                                 NULL,
...
bool MMAdaptor::streamPlay(bool bsync) {
    if (!isMMStreamEnabled()) {
        return true;
    }
    ...
    if (mmCreated) {
        ...
        WFD_status_t ret = wfd_mm_stream_play(mmHandle, WFD_STREAM_AV,bsync ? NULL : &stream_play_cb);
...
bool MMAdaptor::streamPause(bool bsync) {
    if (!isMMStreamEnabled()) {
        return true;
    }
    ...
    if (mmCreated) {
        ...
        WFD_status_t ret = wfd_mm_stream_pause(mmHandle, WFD_STREAM_AV, bsync ? NULL : &stream_pause_cb);
...
bool MMAdaptor::setSurfaceProp(int width,int height,int orientation) {
    WFD_status_t retStatus = WFD_STATUS_FAIL;

    if (mmCreated)
    {
      retStatus = wfd_mm_set_surface_prop(mmHandle, width,height,orientation);
      if (retStatus == WFD_STATUS_SUCCESS)
      {
        return true;
      }
...
void MMAdaptor::stream_play_cb(void* mm_handle, WFD_status_t status) {
    ...
    RTSPSession::updateCurrentState(PLAY_DONE);
    SessionManager *pSM = SessionManager::Instance();
    if (pSM)
    {
...
void MMAdaptor::stream_pause_cb(void* mm_handle, WFD_status_t status) {
    ...
    RTSPSession::updateCurrentState(PAUSE_DONE);
    SessionManager *pSM = SessionManager::Instance();
    if (pSM)
    {
...
void MMAdaptor::event_update_cb(WFD_MM_Event_t event,
    ...
    case WFD_EVENT_MM_VIDEO:
        ...
        break;
        case WFD_EVENT_MM_STREAMING:
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                                    "event_update_cb() WFD_EVENT_MM_STREAMING");
        if(status == WFD_STATUS_STREAMING_REQ && !pEvtData)
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"MMAdaptor: Stop cmd received");
            eventMMUpdate(MM_EVENT_STREAMING_REQ,
                          convertWfdToMMStatus(status),0,0,0,0);

        }
        else if(status == WFD_STATUS_STREAMING_REQ && pEvtData)
        {
            codecInfoType *pnData = (codecInfoType *)pEvtData;
            codecInfoType sCodecInfo;
            strlcpy(&sCodecInfo.codecType[0], &pnData->codecType[0], sizeof(sCodecInfo.codecType));
            sCodecInfo.profile = pnData->profile;
            sCodecInfo.level = pnData->level;
            sCodecInfo.width = pnData->width;
            sCodecInfo.height = pnData->height;
            MM_MSG_PRIO5(MM_GENERAL, MM_PRIO_HIGH,
                         "MMadaptor Streaming Req,%s,%d,%d,%d,%d",
                         sCodecInfo.codecType, sCodecInfo.profile,
                         sCodecInfo.level, sCodecInfo.width,
                         sCodecInfo.height);
            eventMMUpdate(MM_EVENT_STREAMING_REQ,
                          convertWfdToMMStatus(status),reinterpret_cast<long>(&sCodecInfo),0,0,0);
        }
        break;
...
void MMAdaptor::StreamingQueryResponse(bool bStreamingStatus)
{
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "Calling StreamingQueryStatus() %d",
                bStreamingStatus);
#ifndef NO_MM_LIB
    if(mmCreated)
    {
        wfd_mm_streaming_query_response(mmHandle,bStreamingStatus);
    }
#endif
}
