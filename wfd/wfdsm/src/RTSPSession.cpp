...
void printMesg(rtspApiMesg &mesg)
{
   switch(mesg.error) {
    ...
    MM_MSG_PRIO3(MM_GENERAL,MM_PRIO_HIGH,"Session: %d Port0: %d Port1: %d", mesg.session, mesg.rtpPort0, mesg.rtpPort1);

    if (!mesg.ipAddr.empty()) {
        MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_HIGH,"IP: %s", mesg.ipAddr.c_str());
    }
}
...
void cback::openCallback(rtspApiMesg &mesg)
{
    ...
    if (!mesg.ipAddr.empty())
    {
        ...
        MM_MSG_SPRINTF_PRIO_1(MM_GENERAL, MM_PRIO_HIGH,
            "opencallback: peer ip address '%s'", pRtspSession->pPeerDevice->ipAddr.c_str());
    ...
    {
        char ip[20] = {0};
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"Querying  Peer Local IP");
    ...
    if(SOURCE == pSM->pMyDevice->getDeviceType())
    {
      ...
      static const uint32 MAC_ADDR_SZ = 24;
      char dev_addr[MAC_ADDR_SZ] = {0};
      MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"Querying Peer mac address");
      const int val = MAC_ADDR_SZ;
      int ret = getPeerMacAddress (dev_addr, (char *)pRtspSession->pPeerDevice->ipAddr.c_str(), val);
      if (ret == 0)
      {
          pRtspSession->pPeerDevice->macAddr = dev_addr;

          /* If it is SOURCE, query the network properties to decide whether to enable 4K or not */
          getNetworkProperties((const char *)pRtspSession->pPeerDevice->macAddr.c_str(),
                    &pSM->pMyDevice->pMMCapability->pCapability->link_properties.nss,
                    &pSM->pMyDevice->pMMCapability->pCapability->link_properties.rateflags,
                    &pSM->pMyDevice->pMMCapability->pCapability->link_properties.freq,
                    &pSM->pMyDevice->pMMCapability->pCapability->link_properties.enable_4k);

          MM_MSG_PRIO4(MM_GENERAL, MM_PRIO_HIGH,
                       "link properties: nss=%u,rateflags=%u,freq=%u,enable_4k=%u",
                       pSM->pMyDevice->pMMCapability->pCapability->link_properties.nss,
                       pSM->pMyDevice->pMMCapability->pCapability->link_properties.rateflags,
                       pSM->pMyDevice->pMMCapability->pCapability->link_properties.freq,
                       pSM->pMyDevice->pMMCapability->pCapability->link_properties.enable_4k);
      ...
      else
      {
          MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                      "Failed to query for peer address in open callback");
      }

      MMAdaptor::updateLocalMMCapability(pSM->pMyDevice->pMMCapability, pSM->pMyDevice->getDeviceType());

      if(pSM->pMyDevice->pMMCapability->m_nAVFormatCount <= 0)
      {
          if(pSM->pMyDevice->pMMCapability->plocalExCapability)
          {
              if(pSM->pMyDevice->pMMCapability->plocalExCapability->extended_video_config.extended_video_config.codec != NULL)
              {
                  MM_Free(pSM->pMyDevice->pMMCapability->plocalExCapability->extended_video_config.extended_video_config.codec);
                  pSM->pMyDevice->pMMCapability->plocalExCapability->extended_video_config.extended_video_config.codec = NULL;
                  pSM->pMyDevice->pMMCapability->plocalExCapability->extended_video_config.extended_video_config.num_codec = 0;
              }
              MM_Free(pSM->pMyDevice->pMMCapability->plocalExCapability);
              pSM->pMyDevice->pMMCapability->plocalExCapability = NULL;
          }
      }
      if(pSM->pMyDevice->pMMCapability->plocalExCapability)
      {
          MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"openCallback : dumping Source Ext Capability - Local");
          int count = pSM->pMyDevice->pMMCapability->plocalExCapability->extended_video_config.extended_video_config.num_codec;
          MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_ERROR,"openCallback : count = %d ", count);
          for(int i = 0; i < count ; i++)
          {
              if((pSM->pMyDevice->pMMCapability->plocalExCapability->extended_video_config.extended_video_config.codec + i) != NULL)
              {
                  MM_MSG_PRIO2(MM_GENERAL,MM_PRIO_ERROR,"openCallback : Name = %s , Profile = %d",
                  pSM->pMyDevice->pMMCapability->plocalExCapability->extended_video_config.extended_video_config.codec[i].name,
                  pSM->pMyDevice->pMMCapability->plocalExCapability->extended_video_config.extended_video_config.codec[i].profile);
                  MM_MSG_PRIO3(MM_GENERAL,MM_PRIO_ERROR,"openCallback : CEA = %x , Vesa = %x , hh = %x",
                  pSM->pMyDevice->pMMCapability->plocalExCapability->extended_video_config.extended_video_config.codec[i].supported_cea_mode,
                  pSM->pMyDevice->pMMCapability->plocalExCapability->extended_video_config.extended_video_config.codec[i].supported_vesa_mode,
                  pSM->pMyDevice->pMMCapability->plocalExCapability->extended_video_config.extended_video_config.codec[i].supported_hh_mode);
              }
            }
          }

          UIBCAdaptor::updateLocalUIBCCapability(pSM->pMyDevice->pMMCapability->pUibcCapability);
          MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"Local MM capability dump:");
...
#ifdef HDCP_DISPLAY_ENABLED
       if(HDCPManager::isHDCPAvailable() && HDCP1X_COMM_hdmi_status())
       {
    ...
    if(SOURCE != pSM->pMyDevice->getDeviceType())
    {
        WFD_MM_capability_t *pMyDeviceCapability =
            pSM->pMyDevice->pMMCapability->pCapability;

        mesg.wfd.client.setRtpPort0(
           pMyDeviceCapability->transport_capability_config.port1_id);

        mesg.wfd.client.setRtcpPort0(
           pMyDeviceCapability->transport_capability_config.port1_rtcp_id);

       MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
                    "rtcpctrl: opencallback set rtcpport=%u",
                    pMyDeviceCapability->transport_capability_config.port1_id);
    }
    ...
    /* Initialize bStandbyInProgress for each session*/
    pRtspSession->bStandbyInProgress = false;
...
void cback::intersectCallback(rtspApiMesg &mesg)
{
    ...
    MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_HIGH, "In intersect callback : level = %d , profile = %d , name = %s",
      pRtspSession->pPeerDevice->pNegotiatedMMCapability->pExtendedCapability->extended_video_config.extended_video_config.codec[0].level,
      pRtspSession->pPeerDevice->pNegotiatedMMCapability->pExtendedCapability->extended_video_config.extended_video_config.codec[0].profile,
      pRtspSession->pPeerDevice->pNegotiatedMMCapability->pExtendedCapability->extended_video_config.extended_video_config.codec[0].name);
    MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_HIGH, "In intersect callback : cea = %x , vesa = %x , hh = %x",
      pRtspSession->pPeerDevice->pNegotiatedMMCapability->pExtendedCapability->extended_video_config.extended_video_config.codec[0].supported_cea_mode,
      pRtspSession->pPeerDevice->pNegotiatedMMCapability->pExtendedCapability->extended_video_config.extended_video_config.codec[0].supported_vesa_mode,
      pRtspSession->pPeerDevice->pNegotiatedMMCapability->pExtendedCapability->extended_video_config.extended_video_config.codec[0].supported_hh_mode);
    ...
    /* Find an Intersection of codec for Aux Stream and
       populate negotiated capability with the same.*/
    pRtspSession->getAuxStreamIntersection();
    ...
    if (RTSPSession::m_eplayMode == AUDIO_ONLY)  {
      ...
      mesg.wfd.auxCodecInfo.setValid(false);
    }
               ...
               if (type == wfd_uibc_capability) {
                   ...
                   if(m14Support) {
                     ...
                     isParamSet = false; //Do not add wfd_uibc in M4 message
                       ...
                       int m4Enable = 0;
                       getCfgItem(UIBC_M4_ENABLE_KEY,&m4Enable);
                       if (m4Enable) {
                           mesg.wfd.uibcSet.setValid(true);
                           mesg.wfd.wfdParse(wfd_uibc_setting,
                                    string("wfd_uibc_setting: enable"));
                       }
                   }
               }
            }
    ...
    //Source is sanguine of its capabilities here after intersection rigmarole
    if(SOURCE == pSM->pMyDevice->getDeviceType()) {
    ...
    { //Start server on source irrespective of M4/M14 to be able to accept connection at any point
       MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"Start UIBC Server on source");

       //Use the session's resolution to configure UIBC height and width
       pRtspSession->pPeerDevice->pNegotiatedMMCapability->pUibcCapability->\
       negotiated_height = pRtspSession->pPeerDevice->pNegotiatedMMCapability\
       ->pCapability->video_config.video_config.h264_codec[0].max_vres;
       pRtspSession->pPeerDevice->pNegotiatedMMCapability->pUibcCapability->\
       negotiated_width = pRtspSession->pPeerDevice->pNegotiatedMMCapability\
...
void cback::setCallback(rtspApiMesg &mesg)
{
    ...
    if ((mesg.error == pendingCmdError) && pRtspSession->bStandbyInProgress)
    {
      /* If 1. set parameter was not sent due to an already pending command,
       * and 2. user had initiated standby --> try resending standby in
       * order to make sure standby message is sent to peer and
       * proper SM state transition takes place.
       */
       pRtspSession->reSendStandbyMsg();
       return;
    }
    ...
    if (mesg.error != noError)
    {
        ...
        {
            /*In case DS msg is failed from sink, we will fail the TCP-UDP
              switch msg. But this is initiated from native. Hence we dont wan't
              to update this event to upper layers. Distinguish between switch
              iniated from upper layer and for DS req.
            */
            if(!mesg.wfd.dirStreamMode.getValid())
            {
            ...
            {
                if(!strncmp(mesg.wfd.dirStreamMode.getMode().c_str(),"active",sizeof("active")))
                {
                /* Place Holder for error while setting DS active */
                //Sink failed to support DS.. Fall back to mirroring usecase
                //pRtspSession->setTransport(pRtspSession->m_CurrState.mode);
                if(pRtspSession->m_CurrState.inPlay != true)
                {
                  MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"setCb: Calling Play");
                  MMAdaptor::streamPlay();
                }
                MMAdaptor::StreamingQueryResponse(false);
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                            "Failed DS due to fail msg from Sink");
              }
              else
              {
                //Sink failed to move to Mirroring after DS.. We should teardown
                //here! Below code is incorrect. TODO: DRUNWAL
                MMAdaptor::StreamingQueryResponse(false);
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                            "Failed dirStreamMode to Enabled ErrorCase");
              }
            }
        }
    ...
    if (pRtspSession->rtspState == SESS_ESTABLISHED || pRtspSession->rtspState == STANDBY)
    {
        WFD_MM_capability_t *pMMCfg = pRtspSession->pPeerDevice->
                                                     pNegotiatedMMCapability->pCapability;
        //Transport negotiation success
        if(pSM->pMyDevice->getDeviceType() == SOURCE)
        {
            if(mesg.wfd.dirStreamMode.getValid())
            {
                if (mesg.wfd.client.getValid())
                {
                    ...
                    {
                        if (pRtspSession->pPeerDevice->pNegotiatedMMCapability->
                                pCapability->transport_capability_config.eRtpPortType != RTP_PORT_TCP) {
                            pRtspSession->pPeerDevice->pNegotiatedMMCapability->
                                pCapability->transport_capability_config.eRtpPortType = RTP_PORT_TCP;
                            pRtspSession->pPeerDevice->pNegotiatedMMCapability->
                                pCapability->transport_capability_config.port1_id =
                                                         mesg.wfd.client.getRtpPort0();
                            pRtspSession->pPeerDevice->pNegotiatedMMCapability->
                                pCapability->transport_capability_config.port1_rtcp_id =
                                                         mesg.wfd.client.getRtcpPort0();
                    ...
                    {
                        if (pRtspSession->pPeerDevice->pNegotiatedMMCapability->
                                pCapability->transport_capability_config.eRtpPortType != RTP_PORT_UDP) {
                            pRtspSession->pPeerDevice->pNegotiatedMMCapability->
                                pCapability->transport_capability_config.eRtpPortType = RTP_PORT_UDP;
                            pRtspSession->pPeerDevice->pNegotiatedMMCapability->
                                pCapability->transport_capability_config.port1_id =
                ...
                if(!strncmp(mesg.wfd.dirStreamMode.getMode().c_str(), "active", sizeof("active")))
                {
                    /* Place Holder for Success while setting DS active */
                    MMAdaptor::StreamingQueryResponse(true);
                    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                                "Success dirStreamMode Enable NoError case");
                }
                else if(!strncmp(mesg.wfd.dirStreamMode.getMode().c_str(), "inactive", sizeof("inactive")))
                {
                    /* Place Holder for Success while setting DS inactive */
                    MMAdaptor::StreamingQueryResponse(false);
                    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                                "Failed dirStreamMode to Enabled NoError case");
                }
            }
            else if (mesg.wfd.client.getValid())
            {
                MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,
                                           "Received new port from Sink. Update it");
                pMMCfg->transport_capability_config.port1_id =
                                                       mesg.wfd.client.getRtpPort0();
                pMMCfg->transport_capability_config.port1_rtcp_id =
                                                      mesg.wfd.client.getRtcpPort0();
                pMMCfg->transport_capability_config.eRtpPortType =
                                  mesg.wfd.client.getTCP()?RTP_PORT_TCP:RTP_PORT_UDP;
        ...
        else
        {
            ...
            /*In case sink does not support DS (updated via UI) make sure we do not
              process the setCB received and simply ignore it
             */
            if (!strncmp(mesg.wfd.dirStreamMode.getMode().c_str(),"active",sizeof("active")))
            {
                if(pRtspSession->m_bDirStrSupported == false)
                {
                    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,
                                 "DS is not supported for sink. Exit setCB");
                    mesg.wfd.dirStrSupport.setSupport(false);
                    return;
                }
                MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"Enable DS mode");
                pRtspSession->pPeerDevice->pNegotiatedMMCapability->pCapability->direct_streaming_mode = true;
            }

            if(!strncmp(mesg.wfd.dirStreamMode.getMode().c_str(),"inactive", sizeof("inactive")))
            {
                pRtspSession->pPeerDevice->pNegotiatedMMCapability->pCapability->direct_streaming_mode = false;
                MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"Disable DS mode");
                MMCapability tempCap;
                tempCap.configure(mesg.wfd);
                if(tempCap.pCapability != NULL)
                {
                    tempCap.pCapability->pSurface = pRtspSession->pPeerDevice->
                               pNegotiatedMMCapability->pCapability->pSurface;
                    /*Set codecReset to true from here since we are moving to Mirroring*/
                    MMAdaptor::updateSession(&tempCap,true);
                }
            }
            ...
            if (mesg.wfd.client.getValid())
            {
                    ...
                    pCapability->transport_capability_config.eRtpPortType == RTP_PORT_UDP)
                {
                    if (mesg.wfd.client.getTCP())
                    {
                        ...
                        pRtspSession->pPeerDevice->pNegotiatedMMCapability->
                            pCapability->transport_capability_config.eRtpPortType = RTP_PORT_TCP;
                        MMAdaptor::updateSession(pRtspSession->pPeerDevice->pNegotiatedMMCapability);
                        if(pRtspSession->m_bSessionState != PAUSE)
                        {
                            MMAdaptor::streamPlay(true);
                        }
                        if (mesg.wfd.tcpWindowSize.getValid())
                        {
                ...
                else
                {
                    if (!mesg.wfd.client.getTCP())
                    {
                        ...
                        pMMCfg->transport_capability_config.rtcpSock = -1;
                        pRtspSession->pPeerDevice->pNegotiatedMMCapability->pCapability->
                             transport_capability_config.eRtpPortType = RTP_PORT_UDP;
                        MMAdaptor::updateSession(pRtspSession->pPeerDevice->pNegotiatedMMCapability);
                        if(pRtspSession->m_bSessionState != PAUSE)
                        {
                            MMAdaptor::streamPlay(true);
                        }
                        /* If source has sent a latency for UDP session set that.*/
                        MMAdaptor::streamControl(WFD_DEVICE_PRIMARY_SINK, AV_CONTROL_SET_DECODER_LATENCY,
                                                    (uint64)mesg.wfd.tcpWindowSize.getWindowSize());
            ...
            /*Check for video_formats_extension*/
            if(!strncmp(mesg.wfd.dirStreamMode.getMode().c_str(),"active", sizeof("active")))
            {
                MMCapability tempCap;
                tempCap.configure(mesg.wfd);
                tempCap.pCapability->pSurface = pRtspSession->pPeerDevice->
                                           pNegotiatedMMCapability->pCapability->pSurface;
                if((mesg.wfd.codecInfo != NULL &&
                     mesg.wfd.videoHeaderCodecInfo.getValid() && tempCap.pExtendedCapability)
                      ||
                   (mesg.wfd.auxCodecInfo.getValid() && tempCap.pAuxCapability))
                {
                    MMAdaptor::updateSession(&tempCap);
                }
            }

        }
    }
    else if (pRtspSession->rtspState == CAP_NEGOTIATING) {
        if(SOURCE != pSM->pMyDevice->getDeviceType()) {//Only sink needs to be configured here in M4
            pRtspSession->pPeerDevice->pNegotiatedMMCapability->configure(mesg.wfd);
            MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"Dump final negotiated MMCapability:");
            pRtspSession->pPeerDevice->pNegotiatedMMCapability->dump();
        }
...
void cback::setupCallback(rtspApiMesg &mesg)
{
    ...
    if (RTSPSession::m_eplayMode == AUDIO_ONLY) {
      ...
      pRtspSession->pPeerDevice->pNegotiatedMMCapability->pExtendedCapability->
                                              extended_video_config.extended_video_config.num_codec = 0;
      mesg.wfd.videoHeader.setValid(false);
      mesg.wfd.h264Cbp.setValid(false);
      mesg.wfd.h264Chi444p.setValid(false);
      mesg.wfd.h264Chp.setValid(false);
    ...
    if(SOURCE != pSM->pMyDevice->getDeviceType())
    {
      ...
      if (mesg.wfd.server.getRtcpPort0())
      {
          MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_HIGH,
                       "Server has RTCP port %d", mesg.wfd.server.getRtcpPort0());
          pNegotiatedMMCapability->pCapability->transport_capability_config.port1_rtcp_id =
              mesg.wfd.server.getRtcpPort0();
      }
      else
      {
            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
                         "Server does not support RTCP. Close rtcpsockfd %d",
                          pSM->pMyDevice->pMMCapability->pCapability->transport_capability_config.rtcpSock);

            if (pSM->pMyDevice->pMMCapability->pCapability->transport_capability_config.rtcpSock > 0)
            {
                close(pSM->pMyDevice->pMMCapability->pCapability->transport_capability_config.rtcpSock);
                pSM->pMyDevice->pMMCapability->pCapability->transport_capability_config.rtcpSock = -1;
            }
      }
      ...
      MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
                   "Setup callback rtpsocket=%d,rtcpsocket=%d",
                   pNegotiatedMMCapability->pCapability->transport_capability_config.rtpSock,
                   pNegotiatedMMCapability->pCapability->transport_capability_config.rtcpSock);
    ...
    if(pSM->pMyDevice->getDeviceType() != SOURCE && getStreamingSurface() !=NULL)
    {
      ...
      MMAdaptor::setSurfaceProp(pRtspSession->m_PreSurfProp.width,
                                pRtspSession->m_PreSurfProp.height,
                                pRtspSession->m_PreSurfProp.orientation);
      MMAdaptor::streamPlay();
    }
...
void cback::playCallback(rtspApiMesg &mesg)
{
    ...
    pRtspSession->m_bSessionState = PLAY;
...
void cback::pauseCallback(rtspApiMesg &mesg)
{
    ...
    pRtspSession->m_bSessionState = PAUSE;
    if (pSM && pSM->pMyDevice && pSM->pMyDevice->getDeviceType() != SOURCE &&
        pRtspSession->pPeerDevice->pNegotiatedMMCapability->
         pCapability->transport_capability_config.eRtpPortType == RTP_PORT_TCP)
    {
...
RTSPSession::RTSPSession(WFDSession* pWfdSession, Device* pDev) {
    ...
    m_bDirStrSupported = false;
    bThreadCreated = false;
}


RTSPSession::~RTSPSession() {
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "RTSPSession d'tor");
    if (bThreadCreated)
    {
      MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "Waiting for rtsp thread to exit");
      pthread_join(session_thread, NULL);
      MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "Rtsp thread exited");
    }
    delete events;
   MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "RTSPSession d'tor done");
}
...
void* RTSPSession::rtspServerLoopFunc(void *s)
{
    ...
    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"rtspServer thread exit");
    pthread_exit(NULL);
    return NULL;
}
...
bool RTSPSession::startServer(string ipAddr, int rtspPort, int uibcPort) {
    ...
    int retThreadCreate = pthread_create(&session_thread, NULL, &RTSPSession::rtspServerLoopFunc, (void *)server);

    if (0 == retThreadCreate)
    {
      bThreadCreated = true;
      MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH, "rtsp_server thread created");
      rtspStateTransition(STARTING); //RTSP server has started at this point
    }
    else
    {
      MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_ERROR, "rtsp_server thread create failed with error %d", retThreadCreate);
    }

    return bThreadCreated;
}
...
bool RTSPSession::startClient(string ipAddr, int rtpPort0, int rtpPort1, int rtspPort, int hdcpPort) {
    ...
    int retThreadCreate = pthread_create(&session_thread, NULL, &RTSPSession::rtspClientLoopFunc, (void *)client);

    if (0 == retThreadCreate)
    {
      bThreadCreated = true;
      MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH, "rtsp_client thread created");
    }
    else
    {
      MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_ERROR, "rtsp_client thread create failed with error %d", retThreadCreate);
    }
    ...
    for (it=pSM->vecWFDSessions.begin(); it!=pSM->vecWFDSessions.end(); it++) {
        ...
        if (pRtspSession != NULL) {
            ...
            if (pRtspSession->server != NULL
                ...
                /* mark standby in progress for cases only when standby was
                 * requested by user but not handeled by RTSP because it was
                 * in middle of processing another request, hence marked standy as
                 * pending comamnd.
                 */
                pRtspSession->bStandbyInProgress = true;
...
bool RTSPSession::executeRuntimeCommand(int cmd)
{
    LOGD("RTSPSession::executeRuntimeCommand - Command [%d]", cmd);
    ...
    {
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,
            "RTSPSession::Error. No SessionManager instance");
        return false;
    }
    ...
    if(pRtspSession == NULL || pSM->pMyDevice == NULL)
    {
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"RTSPSession:: Something amiss.");
        return false;
    }

    //Sink Hack - Streaming support status is required here
    if (pSM->pMyDevice->getDeviceType() != SOURCE &&
        (cmd == WFD_MM_CMD_ENABLE_STREAMING_FEATURE ||
         cmd == WFD_MM_CMD_DISABLE_STREAMING_FEATURE))
    {
        pRtspSession->m_bDirStrSupported = !(pRtspSession->m_bDirStrSupported);
        MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_HIGH,"RTSPSession:: DirStrSupport = [%d]",
            pRtspSession->m_bDirStrSupported);
        return true;
    }
    else
    {
        //For all the other commands, let Source MM component handle it
...
{
  MM_MSG_PRIO2(MM_GENERAL,MM_PRIO_HIGH,
              "RTSPSession::setResolution %ld %ld", type, value);
  ...
  for (it=pSM->vecWFDSessions.begin(); it!=pSM->vecWFDSessions.end(); it++)
  {
    ...
    if (rtspSession != NULL)
    {
      ...
      switch (resolutionType)
      {
        case WFD_CEA_RESOLUTIONS_BITMAP:
          if (commonCapability->pCapability->video_config.video_config.h264_codec->supported_cea_mode & value)
          {
            ...
            negotiatedCapability->pExtendedCapability->active = false;
            negotiatedCapability->pCapability->active = true;
            bUpdate = true;
          }
          //Need to loop through all profiles.
          else if (commonCapability->pExtendedCapability->extended_video_config.extended_video_config.codec->supported_cea_mode & value)
          {
            negotiatedCapability->pExtendedCapability->extended_video_config.extended_video_config.codec->supported_cea_mode = value;
            negotiatedCapability->pExtendedCapability->extended_video_config.extended_video_config.codec->supported_vesa_mode = 0;
            negotiatedCapability->pExtendedCapability->extended_video_config.extended_video_config.codec->supported_hh_mode = 0;
            negotiatedCapability->pExtendedCapability->active = true;
            negotiatedCapability->pCapability->active = false;
            bUpdate = true;
          }
      ...
      case WFD_VESA_RESOLUTIONS_BITMAP:
          ...
          //Need to loop through all profiles.
          else if (commonCapability->pExtendedCapability->extended_video_config.extended_video_config.codec->supported_vesa_mode & value)
          {
            negotiatedCapability->pExtendedCapability->extended_video_config.extended_video_config.codec->supported_cea_mode = 0;
            negotiatedCapability->pExtendedCapability->extended_video_config.extended_video_config.codec->supported_vesa_mode = value;
            negotiatedCapability->pExtendedCapability->extended_video_config.extended_video_config.codec->supported_hh_mode = 0;
            negotiatedCapability->pExtendedCapability->active = true;
            negotiatedCapability->pCapability->active = false;
            bUpdate = true;
      ...
      case WFD_HH_RESOLUTIONS_BITMAP:
          ...
          //Need to loop through all profiles.
          else if (commonCapability->pExtendedCapability->extended_video_config.extended_video_config.codec->supported_hh_mode & value)
          {
            negotiatedCapability->pExtendedCapability->extended_video_config.extended_video_config.codec->supported_cea_mode = 0;
            negotiatedCapability->pExtendedCapability->extended_video_config.extended_video_config.codec->supported_vesa_mode = 0;
            negotiatedCapability->pExtendedCapability->extended_video_config.extended_video_config.codec->supported_hh_mode = value;
            negotiatedCapability->pExtendedCapability->active = true;
            negotiatedCapability->pCapability->active = false;
            bUpdate = true;
          }
      ...
      if (bUpdate && MMAdaptor::updateSession(negotiatedCapability))
      {
        if(resParams)
        {
          //resParams has width at [0], height at [1] and FPS at [2]
            if (negotiatedCapability->pExtendedCapability->active)
            {
                negotiatedCapability->pExtendedCapability->extended_video_config.extended_video_config.codec->max_hres = resParams[0];
                negotiatedCapability->pExtendedCapability->extended_video_config.extended_video_config.codec->max_vres = resParams[1];
            }
            else
            {
...
void RTSPSession::sendBufferingControlRequest(int ControlType, int cmdVal)
{
    ...
    for (it=pSM->vecWFDSessions.begin(); it!=pSM->vecWFDSessions.end(); it++) {
        ...
        if (rtspSession != NULL) {
            rtspWfd myWfd;
            if (rtspSession->server != NULL) {
                MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"Calling Set: sendBufferingControl");
...
void RTSPSession::reSendStandbyMsg()
{
    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"RTSPSession: reSendStandbyMsg");
    standby(0);
}
...
bool RTSPSession::updateCurrentState(int mode)
{
    MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_ERROR,"updateCurrentState: Called with %d",mode);
    vector<WFDSession*>::iterator it;
    ...
    for (it=pSM->vecWFDSessions.begin(); it!=pSM->vecWFDSessions.end(); it++) {
        ...
        if(mode == PAUSE_DONE)
        {
            pRtspSession->m_CurrState.inPause = true;
            pRtspSession->m_CurrState.inPlay  = false;
            MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"updateCurrentState: Pause set");
        }
        else if(mode == PLAY_DONE)
        {
            pRtspSession->m_CurrState.inPause = false;
            pRtspSession->m_CurrState.inPlay  = true;
            MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"updateCurrentState: Play set");
        }
    }
    return true;
}

bool RTSPSession::prepareForTransportChange(int mode)
{
    if(m_CurrState.inPause != true)
    {
/* TODO, ask for correct param name
        int ret = android::AudioSystem::setDeviceConnectionState(
              AUDIO_DEVICE_OUT_PROXY,
              (audio_policy_dev_state_t)0,//android_audio_legacy::AudioSystem::DEVICE_STATE_UNAVAILABLE
              "");
        MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_ERROR,"prepareForTC:AudioProxy:= %d",ret);
*/
    ...
    do
    {
        ...
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"prepareForTC: pause wait");
        MM_Timer_Sleep(5);
        count++;
    }while(!m_CurrState.inPause && count < 100); //Wait for max 500ms.

    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"prepareForTC: out of pause wait");
    m_CurrState.mode = mode;

    return true;
}

bool RTSPSession::sendAVFormatChangeInformation(bool mode,int codec)
{
    bool bOk = true;
    vector<WFDSession*>::iterator it;
    ...
    for (it=pSM->vecWFDSessions.begin(); it!=pSM->vecWFDSessions.end(); it++) {
        ...
        if (pRtspSession != NULL) {
            if(pRtspSession->server != NULL)
            {
                WFD_transport_capability_config_t *transportCfg =
                    &(pRtspSession->pPeerDevice->pNegotiatedMMCapability->
                        pCapability->transport_capability_config);
                rtspWfd myWfd;
                /* 0 is DS mode inactive and 1 is DS mode active */
                myWfd.dirStreamMode.setMode(mode);
                /* Setting client port and port information */
                if(pRtspSession->m_sLocalTransportInfo.rtpPort) {
                    myWfd.client.setRtpPort0(pRtspSession->m_sLocalTransportInfo.rtpPort);
                }
                if(pRtspSession->m_sLocalTransportInfo.rtcpPort) {
                    myWfd.client.setRtcpPort0(pRtspSession->m_sLocalTransportInfo.rtcpPort);
                }
                myWfd.client.setValid(true);

                if(mode == DS)
                {
                    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"DRTCP: Mode == DS");
                    WFD_MM_Extended_capability_t *pExCapability =
                        pRtspSession->pPeerDevice->pMMCapability->pExtendedCapability;
                    /* Populate Video Format Extensions based on index received */
                    myWfd.videoHeaderCodecInfo.setNative(pExCapability->extended_video_config.
                                                         extended_video_config.native_bitmap);
                    myWfd.videoHeaderCodecInfo.setPrefDispSupp(pExCapability->extended_video_config.
                                                               extended_video_config.
                                                               preferred_display_mode_supported);
                    myWfd.videoHeaderCodecInfo.setValid(true);

                    /*TODO: Check for codecCount against total codecs*/
                    WFD_extended_codec_config_t tempCodec =
                          pExCapability->extended_video_config.extended_video_config.codec[codec];
                    string name(tempCodec.name);
                    myWfd.codecInfo = new CodecInfo(name,
                                                    tempCodec.profile,
                                                    tempCodec.level,tempCodec.max_hres,
                                                    tempCodec.max_vres,
                                                    tempCodec.supported_cea_mode,
                                                    tempCodec.supported_vesa_mode,
                                                    tempCodec.supported_hh_mode,
                                                    tempCodec.decoder_latency,tempCodec.min_slice_size,
                                                    tempCodec.slice_enc_params,
                                                    tempCodec.frame_rate_control_support);

                    if (NULL == myWfd.codecInfo)
                    {
                        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "SendAVFormatChange failed to create codecInfo");
                        bOk = false;
                        break;
                    }

                    myWfd.codecInfoCount = 1;

                    if(transportCfg->eRtpPortType != RTP_PORT_TCP)
                    {
                        /* For DS move to TCP if in UDP*/
                       // pRtspSession->prepareForTransportChange(mode);
                        transportCfg->eBeforeDSPortType = RTP_PORT_UDP;
                        myWfd.client.setTCP(true);
                    }
                    /* Doing it outside to override previously populated
                       value for TCP buffering at sink and provide DS
                       related values */
                    myWfd.tcpWindowSize.setValid(true);
                    myWfd.tcpWindowSize.setWindowSize(1000);

                    if(pRtspSession->m_bAuxStreamSupported)
                    {
                        WFD_MM_Aux_capability_t *pAuxCapability =
                            pRtspSession->pPeerDevice->pNegotiatedMMCapability->pAuxCapability;
                        myWfd.auxCodecInfo.setName(pAuxCapability->name[0]);
                        myWfd.auxCodecInfo.setMaxOverlay(pAuxCapability->maxOverlay);
                        myWfd.auxCodecInfo.setBlendMode(pAuxCapability->blendMode);
                        myWfd.auxCodecInfo.setAuxRes(pAuxCapability->supportedBitMap);
                        myWfd.auxCodecInfo.setValid(true);
                    }
                }
                else if(mode == MIRROR)
                {
                    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"DRTCP: Mode == mirror correction");
                    /* Populate Video Formats of session */
                    WFD_MM_capability_t *pCapability =
                        pRtspSession->pPeerDevice->pNegotiatedMMCapability->pCapability;
                    if(pCapability != NULL)
                    {
                        WFD_h264_codec_config_t* pH264Config =
                            &(pCapability->video_config.video_config.h264_codec[0]);
                        uint8 nativeBM = pCapability->video_config.video_config.native_bitmap;
                        uint8 prefDisp = pCapability->video_config.video_config.preferred_display_mode_supported;
                        /* Preparing Video Header*/
                        myWfd.videoHeader.setNative(nativeBM);
                        myWfd.videoHeader.setPrefDispSupp(prefDisp);
                        myWfd.videoHeader.setValid(true);
                        /* Preparing Codec */
                        bitset<BITSET_2> tempProfile, tempLevel;
                        int profBit = pRtspSession->getBitSetPosition((int)pH264Config->h264_profile);
                        int levBit = pRtspSession->getBitSetPosition((int)pH264Config->h264_level);

                        tempProfile.set(profBit);//tempProfile.set(pH264Config->h264_profile);
                        tempLevel.set(levBit);//tempLevel.set(pH264Config->h264_level);

                        h264Codec tempCodec(tempProfile,tempLevel,0,0,0,0,0,0,0,0,0);

                        myWfd.h264Cbp.setProfile(tempCodec.getProfile());
                        myWfd.h264Cbp.setLevel(tempCodec.getLevel());

                        MM_MSG_PRIO4(MM_GENERAL,MM_PRIO_HIGH,"CODECINFO -- :\
                            pH264Config[%d][%d] tempProfLevel[%d][%d]",
                            pH264Config->h264_profile,pH264Config->h264_level,
                            tempProfile, tempLevel);
                        MM_MSG_PRIO4(MM_GENERAL,MM_PRIO_HIGH,"CODECINFO cont-- :\
                            tempCodecProfLevel[%d][%d] myWfd[%d][%d]",
                            tempCodec.getProfile(),tempCodec.getLevel(),
                            myWfd.h264Cbp.getProfile(), myWfd.h264Cbp.getLevel());

                        myWfd.h264Cbp.setCeaSupp(pH264Config->supported_cea_mode);
                        myWfd.h264Cbp.setVesaSupp(pH264Config->supported_vesa_mode);
                        myWfd.h264Cbp.setHhSupp(pH264Config->supported_hh_mode);
                        myWfd.h264Cbp.setLatency(pH264Config->decoder_latency);
                        myWfd.h264Cbp.setMinSliceSize(pH264Config->min_slice_size);
                        myWfd.h264Cbp.setFrameCtlSupp(pH264Config->frame_rate_control_support);
                        myWfd.h264Cbp.setValid(true);
                    }

                    MM_MSG_PRIO2(MM_GENERAL,MM_PRIO_HIGH,"prepareForTC : before = %d , current = %d",
                          transportCfg->eBeforeDSPortType,transportCfg->eRtpPortType);
                    if(transportCfg->eBeforeDSPortType == RTP_PORT_UDP &&
                       transportCfg->eRtpPortType == RTP_PORT_TCP)
                    {
                        /* If previously we were on UDP then after DS we need to
                           move back to UDP if still in TCP.*/
                        //pRtspSession->prepareForTransportChange(mode);
                        myWfd.client.setTCP(false);
                        transportCfg->eBeforeDSPortType = RTP_PORT_TCP;
                    }
                }
                pRtspSession->server->Set(pRtspSession->rtspSessionId, myWfd);
            }
...
int RTSPSession::getBitSetPosition(int num)
{
    int count = -1;
    int tmpNum = num;
...
void RTSPSession::setSurfaceProp(int width,int height,int orientation)
{
    SessionManager *pSM = SessionManager::Instance();
    if((pSM == NULL) || (pSM->pMyDevice == NULL))
            return;
    ...
    for (it=pSM->vecWFDSessions.begin(); it!=pSM->vecWFDSessions.end(); it++)
    {
        ...
        if (wfdSession == NULL)
        {
          MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"No Active WFD Session, can't set Surface property");
          return;
        }
        if(SOURCE != pSM->pMyDevice->getDeviceType())
        {
           MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"RTSPSession::setSurfaceProp updating Property");
           //MMAdaptor::setSurfaceProp(width,height,orientation);
           RTSPSession* rtspSession = wfdSession->getRtspSession();
           if(rtspSession)
           {
               rtspSession->m_PreSurfProp.width  = width;
               rtspSession->m_PreSurfProp.height = height;
               rtspSession->m_PreSurfProp.orientation = orientation;
           }
        }
    }
    return;
...
unsigned int RTSPSession::getNegotiatedUIBCCategory() {
    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"getNegotiatedUIBCCategory()");
    SessionManager *pSM = SessionManager::Instance();
    if(!pSM) {
       MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,
    ...
    if (wfdSession == NULL) {
          MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,
                      "No Active WFD Session, can't get Common Resloution");
          return 0;
    }
    RTSPSession *pRTSPSession = wfdSession->getRtspSession();
    if (pRTSPSession == NULL)
    ...
    return pRTSPSession->pPeerDevice->pNegotiatedMMCapability->\
...
void RTSPSession::StreamingSupportReq(bool bFlag, void *pData)
{

    /*--------------------------------------------------------------------------
       1. parse the codec infor received altogether from Player
       2. send information to sink
       3. if not matching the codec info send Failure status to Miracast
          component to start mirroring
    ----------------------------------------------------------------------------
    */
    ...
    if(pSM == NULL)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,"inStreamingSupportReq pSM NULL");
        MMAdaptor::StreamingQueryResponse(false);
        return;
    }
    ...
    if(pRtspSession == NULL)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,"inStreamingSupportReq pRtspSession NULL");
        MMAdaptor::StreamingQueryResponse(false);
        return;
    }

    // When we are moving back to Mirroring
    if(bFlag == 0)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,"StreamingSupportReq calling inactive");
        sendAVFormatChangeInformation(bFlag,-1);
        return;
    }

    // When we are moving to DS
    int codecMatchIndex = -1;
    int ResolutionMatchIndex = -1;
    int receivedCodeCnt = 0;
    codecInfoType* CodecInfoRec = (codecInfoType *)pData;
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                "in StreamingSupportReq for streaming");
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                 "streaming req in RTSP Codec = %s", CodecInfoRec->codecType);
    MM_MSG_PRIO4(MM_GENERAL, MM_PRIO_ERROR,
                 "streaming in RTSP Profile %d & Level %d,Height %d & Width %d ",
                 CodecInfoRec->profile, CodecInfoRec->level,CodecInfoRec->height,
                 CodecInfoRec->width);
    ...
    if(pRtspSession != NULL)
    {
        ...
        {
            int numOfCodecs =
            pExCapability->extended_video_config.extended_video_config.num_codec;
            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                         "streaming in RTSP numOfCodecs = %d", numOfCodecs);
            if(numOfCodecs != 0)
            {
                receivedCodeCnt = numOfCodecs;
                for(int ii = 0; ii < numOfCodecs; ii++)
                {
                    WFD_extended_codec_config_t tempCodec =
                                  pExCapability->extended_video_config.
                                                  extended_video_config.codec[ii];

                    if((strncmp(tempCodec.name, CodecInfoRec->codecType, sizeof(CodecInfoRec->codecType)) == 0)
                       && (CodecInfoRec->profile == tempCodec.profile)
                       && (CodecInfoRec->level <= tempCodec.level))
                    {
                        if(CodecInfoRec->height <=
                               tempCodec.max_mode_list[WFD_VIDEO_CEA_MODE].height &&
                           CodecInfoRec->width <=
                               tempCodec.max_mode_list[WFD_VIDEO_CEA_MODE].width)
                        {
                            MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_ERROR,
                                         "streaming in RTSP CEA Match H= %d W= %d",
                                         tempCodec.max_mode_list[WFD_VIDEO_CEA_MODE].height,
                                         tempCodec.max_mode_list[WFD_VIDEO_CEA_MODE].width);
                            codecMatchIndex = ii;
                            break;
                        ...
                        if(CodecInfoRec->height <=
                               tempCodec.max_mode_list[WFD_VIDEO_VESA_MODE].height &&
                           CodecInfoRec->width <=
                               tempCodec.max_mode_list[WFD_VIDEO_VESA_MODE].width)
                        {
                            codecMatchIndex = ii;
                            MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_ERROR,
                                         "streaming in RTSP VESA Match H= %d W= %d",
                                         tempCodec.max_mode_list[WFD_VIDEO_VESA_MODE].height,
                                         tempCodec.max_mode_list[WFD_VIDEO_VESA_MODE].width);
                            break;
                        }
                        if(CodecInfoRec->height <=
                               tempCodec.max_mode_list[WFD_VIDEO_HH_MODE].height &&
                           CodecInfoRec->width <=
                               tempCodec.max_mode_list[WFD_VIDEO_HH_MODE].width)
                        {
                            codecMatchIndex = ii;
                            MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_ERROR,
                                         "streaming in RTSP HH Match H= %d W= %d",
                                         tempCodec.max_mode_list[WFD_VIDEO_HH_MODE].height,
                                         tempCodec.max_mode_list[WFD_VIDEO_HH_MODE].width);
                            break;
                        }
        ...
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                        "pExCapability is NULL in StreamingSupportReq");
        }
    }

    if(codecMatchIndex != -1 && codecMatchIndex < receivedCodeCnt)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "sendAVFormatChangeInformation");
        sendAVFormatChangeInformation(bFlag, codecMatchIndex);
    }
    else
    {
        // Send information to MMAdaptor to continue with mirrorring
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "dirStream is not success");
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "in StreamingSupportReq");
        MMAdaptor::StreamingQueryResponse(false);
    }
}

void RTSPSession::getAuxStreamIntersection()
{
    vector<WFDSession*>::iterator it;
    ...
    for (it=pSM->vecWFDSessions.begin(); it!=pSM->vecWFDSessions.end(); it++) {
        WFDSession* wfdSession = *it;
        RTSPSession* pRtspSession = wfdSession->getRtspSession();
        if (pRtspSession != NULL && pRtspSession->pPeerDevice != NULL &&
            pRtspSession->pPeerDevice->pMMCapability != NULL &&
            pRtspSession->pPeerDevice->pMMCapability->pAuxCapability != NULL) {
            int count = pRtspSession->pPeerDevice->pMMCapability->pAuxCapability->count;

            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "getAuxStreamIntersection - count = %d",count);
            pRtspSession->m_bAuxStreamSupported = false;
            int localCount = pSM->pMyDevice->pMMCapability->pAuxCapability->count;
            int j = 0;

            /* Loop for total auxiliary info received from Sink */
            for(int i = 0 ; i < count; i++)
            {
                /* Loop to Check it against supported Aux Codec on Source */
                for(int k = 0; k < localCount; k++)
                {
                    if(!strncmp(
                        pRtspSession->pPeerDevice->pMMCapability->pAuxCapability->name[i],
                        pSM->pMyDevice->pMMCapability->pAuxCapability->name[k],
                        sizeof(pSM->pMyDevice->pMMCapability->pAuxCapability->name[k])))
                    {
                        if(pRtspSession->pPeerDevice->pNegotiatedMMCapability->pAuxCapability != NULL)
                        {
                            strlcpy
                            (pRtspSession->pPeerDevice->pNegotiatedMMCapability->pAuxCapability->name[j],
                             pRtspSession->pPeerDevice->pMMCapability->pAuxCapability->name[i],
                             sizeof(pRtspSession->pPeerDevice->pMMCapability->pAuxCapability->name[i])
                            );
                            pRtspSession->m_bAuxStreamSupported = true;
                            j++; // Added for future enhancement where we will support more codecs.
                            break;
                        }
                    }
                }
            ...
            if(pRtspSession->m_bAuxStreamSupported)
            {
                pRtspSession->pPeerDevice->pNegotiatedMMCapability->pAuxCapability->count = count;
                pRtspSession->pPeerDevice->pNegotiatedMMCapability->pAuxCapability->maxOverlay =
                    pRtspSession->pPeerDevice->pMMCapability->pAuxCapability->maxOverlay;
                pRtspSession->pPeerDevice->pNegotiatedMMCapability->pAuxCapability->blendMode=
                    pRtspSession->pPeerDevice->pMMCapability->pAuxCapability->blendMode;

                int peerBitmap  = pRtspSession->pPeerDevice->pMMCapability->pAuxCapability->supportedBitMap;
                int localBitmap = pSM->pMyDevice->pMMCapability->pAuxCapability->supportedBitMap;
                int negBM = prepareNegotiatedAuxBitmap(localBitmap,peerBitmap);
                MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_HIGH,
                    "getAuxStreamIntersection: bitmap Src[%d] Sink[%d] Neg[%d]",
                    localBitmap, peerBitmap,negBM);
                pRtspSession->pPeerDevice->pNegotiatedMMCapability->pAuxCapability->supportedBitMap=negBM;
            }
            else
            {
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                    "getAuxStreamIntersection: No Support for overlay!");
                pRtspSession->pPeerDevice->pNegotiatedMMCapability->pAuxCapability->count = 0;
            }
        }
    }
}

int RTSPSession::prepareNegotiatedAuxBitmap(int localBM, int peerBM)
{
    /* Get Mode and Value */
    int localMode  = localBM & 0x7;
    int localValue = (localBM >> 3) & 0x1F;

    int peerMode  = peerBM & 0x7;
    int peerValue = (peerBM >> 3) & 0x1F;

    int negMode = 0, negValue = 0;
    if(localMode == peerMode)
    {
        negMode = localMode;
        negValue =((localValue < peerValue)?localValue:peerValue);
    }
    else
        negMode = 3; // None of CEA/VESA/HH , as there is no other value

    MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
                    "prepareNegotiatedAuxBitmap: negMode = %d , negValue = %d",
                    negMode,negValue);

    return ((negValue<<3)|negMode);
}
