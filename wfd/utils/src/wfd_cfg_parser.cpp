...
#pragma clang diagnostic pop
      ...
      void parseWFDSessionCodec(string);
      void parseVideoHeader(string);
      void parseVideoCodec(string, int index);
      uint8 convertLevelToBit(string, string);
...
static ceaStruct ceaVideoModeData = {
   {
      ...
      194400,  // 1920x1080p24
      972000,  // 3840x2160p30
     1944000,  // 3840x2160p60
     1036800,  // 4096x2160p30
     2073600,  // 4096x2160p60
      810000,  // 3840x2160p25
     1620000,  // 3840x2160p50
      864000,  // 4096x2160p25
     1728000,  // 4096x2160p50
      777600,  // 3840x2160p24
      829440   // 4096x2160p24
   },
   {  // CEA VALID MODES
...
void  parseUIBCSupport(string uibcBuffer)
{
   ...
   if(bValid)
   {
      ...
      if(bGeneric)
      {
         ...
         MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_HIGH,"parseUIBCSupport: generic input = %d",pReadCfgCapabilities->uibcCapability.config.generic_input_type);
      }
      if(bHID)
      {
...
bool initVideoFormatExtResources()
{
    ...
    /*Sanity check*/
    if(pReadCfgCapabilities->pExtCfgCapabilities != NULL)
    {
        MM_Free(pReadCfgCapabilities->pExtCfgCapabilities);
        pReadCfgCapabilities->pExtCfgCapabilities = NULL;
    }

    /*Initialize*/
    pReadCfgCapabilities->pExtCfgCapabilities = static_cast<WFD_MM_Extended_capability_t *>
        (MM_Calloc(1, sizeof(WFD_MM_Extended_capability_t)));

    if(pReadCfgCapabilities->pExtCfgCapabilities == NULL)
    {
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,
            "wfd_cfg_parser:initVideoFormatExtResouces() - Failed to allocate resources");
        return false;
    }

    /*Allocate resourcse for all the extension codecs*/
    if(pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec != NULL)
    {
        MM_Free(pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec);
        pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec = NULL;
    }

    if(nCount > 0)
    {
       pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec =
           static_cast<WFD_extended_codec_config_t *>(MM_Calloc(1, sizeof(WFD_extended_codec_config_t) * nCount));

       if(pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec == NULL)
       {
           MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,
               "wfd_cfg_parser:initVideoFormatExtResouces() - Failed to allocate additional resources");
           return false;
       }
    }
...
bool initAuxStreamInfo()
{
    ...
    {
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,
            "wfd_cfg_parser:initAuxStreamInfo() - Failed to allocate resources");
        return false;
    }
...
/*==============================================================================
         ...
         FUNCTION:    convertLevelToBit

         DESCRIPTION:
         @brief        Convert profile level to corresponding bit value
*//**
...
*//*==========================================================================*/
uint8 convertLevelToBit(string codec,string level)
{
    uint8 bLevel = BIT0;

    if(strncmp(codec.c_str(),"H.264",codec.length()) == 0)
    {
        bLevel = level.compare("3.1") ?
                  (level.compare("3.2")?
                     (level.compare("4")?
                        (level.compare("4.1")?
                           (level.compare("4.2")? BIT0   // Level 3.1, BIT0 by default
    ...
    /*else if() -- fill for all other codecs at later point of time*/
...
void parseExtendedVideoCodec(string videoBuffer, int index)
{
   ...
   strlcpy((pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).name,
           codecName.c_str(),sizeof((pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).name));

   /*Store Profile Value*/
   (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).profile
      = (uint8)atoi((getValueForTag(videoBuffer, "Profile")).c_str());

   /*Parse Level based on Codec Name*/
   (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).level
      = convertLevelToBit(codecName, getValueForTag(videoBuffer,"Level"));

   /*Store Horizontal Resolution*/
   (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).max_hres
      = uMaxHRes = (uint16)atoi((getValueForTag(videoBuffer, "HorizontalResolution")).c_str());

   /*Store Vertical Resolution*/
   (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).max_vres
      = uMaxVRes = (uint16)atoi((getValueForTag(videoBuffer, "VerticalResolution")).c_str());


    if((uMaxVRes > 1080) || (uMaxHRes > 1920)) /* Check if the wlan is capable of supporting 4k */
    {
        if(pReadCfgCapabilities->pCfgCapabilities)
        {
   ...
   (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).decoder_latency
       = (uint8)atoi((getValueForTag(videoBuffer, "Latency")).c_str());

   /*Store Minimum Slice size*/
   (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).min_slice_size
       = (uint16)atoi((getValueForTag(videoBuffer, "MinimumSliceSize")).c_str());

   /*Store Slice Encoding Params*/
   (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).slice_enc_params
       = (uint8)atoi((getValueForTag(videoBuffer, "SliceEncodingParams")).c_str());

   /*Store Frame Rate Control Support*/
   (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).frame_rate_control_support
      = (uint8)atoi((getValueForTag(videoBuffer, "FrameRateControlSupport")).c_str());

   fpsSupported = (unsigned int )atoi((getValueForTag(videoBuffer, "VideoFps")).c_str());

   MM_MSG_PRIO4(MM_GENERAL,MM_PRIO_HIGH,"wfd_cfg_parser : user enterted resolution for codecNum = %d [%d x %d %d fps]",index,uMaxHRes,uMaxVRes,fpsSupported);
   ...
   switch(mnHW_ID)
   {
      ...
      case 243:
          ...
          {
             uMaxHRes = 1920;
             (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).max_hres = 1920;
             uMaxVRes = 1080;
             (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).max_vres = 1080;
   ...
   videoModeWidth = ceaVideoModeData.ceaWidth;
   videoModeValidRes = ceaVideoModeData.ceaValidRes;
   (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).supported_cea_mode
      = ceaBitMap = getVideoModes(uMacroBlocks, ceaModeCount, uMaxHRes, uMaxVRes );
   ...
   videoModeWidth = vesaModeData.vesaWidth;
   videoModeValidRes = vesaModeData.vesaValidRes;
   (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).supported_vesa_mode
      = vesaBitMap = getVideoModes(uMacroBlocks, vesaModeCount, uMaxHRes, uMaxVRes );
   ...
   videoModeWidth = hhModeData.hhWidth;
   videoModeValidRes = hhModeData.hhValidRes;
   (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).supported_hh_mode
      = hhBitMap = getVideoModes(uMacroBlocks, hhModeCount, uMaxHRes, uMaxVRes );
   ...
   {
      MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR," Defaulting to VGA");
      (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).supported_cea_mode = H264_CEA_640x480p60;
      (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).max_hres = 640;
      (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).max_vres = 480;
      nativeModes = 0;
   }

   /*Update native_bitmap*/
   pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.native_bitmap = nativeModes;
}
...
void parseExtVideoCodecHeader(string buffer)
{
    ...
    MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_HIGH,
        "wfd_cfg_parser: parseAVFormatsExtn: Total profiles supported = %d", nTotalProfiles);
     ...
     */
    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"wfd_cfg_parser: parseAVForamtsExtn: Allocate Resources");
    initVideoFormatExtResources();

    /* Update the total number of codecs in cap. data structure*/
    pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.num_codec =
        nTotalProfiles;
    ...
    if(tagValue.length())
    {
        pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.preferred_display_mode_supported =
            atoi(tagValue.c_str());
    }
}
...
/*==============================================================================
         ...
         DESCRIPTION:
         @brief        Parse XML for VideoFormatsExtension
*//**
...
{
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"wfd_cfg_parser: parseAVFormatsExtension");

    string tagValue;
    if(((mnHW_ID == 294 || mnHW_ID == 295) || (mnHW_ID == 304 || mnHW_ID == 293)
        || (mnHW_ID == 303 || mnHW_ID == 307) || (mnHW_ID == 206) || (mnHW_ID == 313)
        ||(mnHW_ID == 239 || mnHW_ID == 240 || mnHW_ID == 241 || mnHW_ID == 242 || mnHW_ID == 243)
        || (mnHW_ID == 320))
        && (wfdDevice == SOURCE))
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
        "wfd_cfg_parser: AVFormatsExtension - Not Supported for Source");
        ...
        if(pReadCfgCapabilities->numAVFormatsExtention > 0)
        {
            ...
            for(int index = 1; index <= pReadCfgCapabilities->numAVFormatsExtention; index++)
            {
                ...
                MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_LOW,
                    "wfd_cfg_parser: parseAVFormatsExtension: Parsing codec Tag = %s",aCodecTag);
...
void parseAuxCodec(string buffer)
{
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"wfd_cfg_parser: parseAuxCodec");
    ...
    if(tagValue.length())
    {
        ...
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,"wfd_cfg_parser: parseAuxCodec codec= %d",nTotalCodec);
        ...
        for(int i=0;i < nTotalCodec;i++)
        {
            ...
            strlcpy(pReadCfgCapabilities->pAuxCfgCapabilities->name[i],name.c_str(),
                sizeof(pReadCfgCapabilities->pAuxCfgCapabilities->name[i]));
        }
    }
    ...
    value = atoi((getValueForTag(buffer, "BlendMode")).c_str());
    pReadCfgCapabilities->pAuxCfgCapabilities->blendMode = value;
    ...
    switch(mnHW_ID)
    {
      ...
      case 320:
          if(hRes > 640 || vRes > 480)
          {
              ...
              vRes = 480;
              MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
                "wfd_cfg_parser: Overlay resolution %d x %d",hRes,vRes);
          }
          break;
      default:
          break;
    ...
    MM_MSG_PRIO4(MM_GENERAL, MM_PRIO_HIGH,"parseAuxCodec name[0] = %s , maxoverlay = %d , blend mode = %d, mode = %x",
                                pReadCfgCapabilities->pAuxCfgCapabilities->name[0],
                                pReadCfgCapabilities->pAuxCfgCapabilities->maxOverlay,
                                pReadCfgCapabilities->pAuxCfgCapabilities->blendMode,
...
void parseWhiteListApp(string buffer)
{
    ...
    if(nTotalApp > 0)
    {
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,"wfd_cfg_parser: parseWhiteListApp Count = %d",nTotalApp);
        if(!initWhiteList(nTotalApp))
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,"wfd_cfg_parser: parseWhiteListApp - Initialization failed.");
            err = true;
        }
        else
        {
    ...
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"wfd_cfg_parser: parseWhiteListApp - No Apps");
        err = true;
    }
...
bool initWhiteList(int count)
{
    ...
    if(pReadCfgCapabilities->whiteListApp.name != NULL)
    {
        for(int i = 0; i < count; i++)
        {
            ...
            if(pReadCfgCapabilities->whiteListApp.name[i] == NULL)
            {
                ...
                "wfd_cfg_parser: initWhiteList - \
                                        Exit failure - Count = %d",i);
                return false;
            }
            else
    ...
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
        "wfd_cfg_parser: initWhiteList - Exit - failure");
    return false;
}
...
void  parseXml(string cfgBuffer)
{
   ...
   tagValue = getValueForTag(cfgBuffer, "VideoFormatsExtension");
   if(tagValue.length())
   {
   ...
   else
   {
      ...
      MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"No support for VideoFormatsExtension");
      pReadCfgCapabilities->pExtCfgCapabilities = NULL;
      pReadCfgCapabilities->numAVFormatsExtention = 0;
   }
   ...
   {
      /*wfd_aux_stream_info is not supported or this is source*/
...
//Parse for configurable items here itself in order to cache for later use
   ...
   tagValue = getValueForTag(cfgBuffer,DYNAMIC_RESOLUTION_CHANGE_KEY);
   if(tagValue.length())
   {
     MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
            "DYNAMIC_RESOLUTION_CHANGE in cfg = %d",atoi(tagValue.c_str()));
     switch(mnHW_ID)
     {
         /*--1--*/
         case 294:
         ...
         case 320:
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                    "Setting DYNAMIC_RESOLUTION_CHANGE to 0");
            tagValue = string("0");
            break;
     }
     updateVar(DYNAMIC_RESOLUTION_CHANGE_KEY,DYNAMIC_RESOLUTION_CHANGE,tagValue);
   }
...
void  parseCfgforExtCodec(const char *filename, readConfigFile *readCfgCaps)
{
   ...
   if(length == (unsigned long) bytesRead)
   {
      ...
      /*Parse WFD AV Formats Extension*/
      tagValue = getValueForTag(buffer, "VideoFormatsExtension");
      if(tagValue.length())
      {
         parseAVFormatsExtension(tagValue);
      }
...
void  dumpcapabilitiesRead()
{
   ...
   if(pReadCfgCapabilities->numAVFormatsExtention > 0)
   {
      MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"Dumping Extended Video Codec information:");
      for(int index = 0; index < pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.num_codec; index++)
      {
         MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "Codec name: %s",
            (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).name);
         MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "Codec profile: %d",
            (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).profile);
         MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "Codec level: %d",
            (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).level);
         MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "Codec max hres: %d",
            (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).max_hres);
         MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "Codec max vres: %d",
            (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).max_vres);
         MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "Codec CEA mode: %d",
            (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).supported_cea_mode);
         MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "Codec VESA mode: %d",
            (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).supported_vesa_mode);
         MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "Codec HH mode: %d",
            (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).supported_hh_mode);
         MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "Codec Frame Rate Control Support: %d",
            (pReadCfgCapabilities->pExtCfgCapabilities->extended_video_config.extended_video_config.codec[index]).frame_rate_control_support);
      }
   }
   ...
   if(pReadCfgCapabilities->pAuxCfgCapabilities)
   {
      for(int index = 0; index < pReadCfgCapabilities->pAuxCfgCapabilities->count; index++)
      {
        MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH, "Aux Stream Codec [%d] = %s",index,
                                pReadCfgCapabilities->pAuxCfgCapabilities->name[index]);
      }
      MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "Aux Stream MaxOverlay = %d",
                                pReadCfgCapabilities->pAuxCfgCapabilities->maxOverlay);
      MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "Aux Stream BlendMode = %d",
                                pReadCfgCapabilities->pAuxCfgCapabilities->blendMode);
