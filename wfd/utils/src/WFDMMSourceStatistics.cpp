...
@par     SIDE EFFECTS:
                            None
*//*=========================================================================*/
void WFDMMSourceStatistics::resetStats()
{
    mWfdProfStats.nCurrentDisplayFrames =
    mWfdProfStats.nCurrentFramesDelivered =
    mWfdProfStats.nCurrentFramesEncoded =
    mWfdProfStats.nLastPrintStatTime = 0;
}
...
#define FPS_PRINT_INTERVAL 1000000 // 1 second in us
    ...
    OMX_TICKS currTime, diffTime;
    GetCurTime(currTime);

    if(WFD_UNLIKELY(mWfdProfStats.nLastPrintStatTime == 0))
    {
        mWfdProfStats.nLastPrintStatTime = currTime;
    }

    diffTime = currTime - mWfdProfStats.nLastPrintStatTime;

    if(WFD_UNLIKELY(diffTime > FPS_PRINT_INTERVAL))
    {
        WFDMMLOGE("******************SOURCE STATISTICS**********************");

        WFDMMLOGE1("I/P FPS from display  = %.2lf",
            mWfdProfStats.nCurrentDisplayFrames*1000000.0/diffTime);

        WFDMMLOGE1("I/P FPS for encoder = %.2lf",
            mWfdProfStats.nCurrentFramesDelivered*1000000.0/diffTime);

        WFDMMLOGE1("O/P FPS for encoder = %.2lf",
            mWfdProfStats.nCurrentFramesEncoded*1000000.0/diffTime);
...
{
    using namespace WFDMMSourceStats;
    OMX_S64* arr = new OMX_S64[WFD_MM_SRC_MAX_STAT_KEYS];
    if(arr)
    {
        arr[TOT_DISP_FRAMES]  = mWfdProfStats.nTotalDisplayFrames;
...
int WFDMMSourceStatistics::setup()
{
...
/*=============================================================================
         ...
         FUNCTION:          recordDisplayFrame

         DESCRIPTION:
*//**       @brief          records the event of receiving frame from display
*//**
@par     DEPENDENCIES:
                            None
*//*
...
void WFDMMSourceStatistics::recordDisplayFrame()
{
    mWfdProfStats.nTotalDisplayFrames++;
    mWfdProfStats.nCurrentDisplayFrames++;
}
...
const OMX_BUFFERHEADERTYPE* pEncOPBuffHdr)
{
    ...
    */
    ...
    if(pEncOPBuffHdr && pEncOPBuffHdr->nFilledLen && mStatSlots)
    {
        buff_hdr_extra_info* tempExtra = NULL;
        OMX_TICKS encTime = 0;

        tempExtra = static_cast<buff_hdr_extra_info*>
            (pEncOPBuffHdr->pAppPrivate);
                ...
                */
                ...
                else
                {
                    ...
                    */
                    ...
                    ALOGE_IF(mProfilingEnabled,
                        "Received frame number %lld with TS %lld at time %lld, at slot %d",
        ...
        if(tempExtra)
        {
            ...
            */
            ...
            if(tempExtra->nEncryptTime)//Ignore spurious frames
            {
                ...
                if(WFD_UNLIKELY(mWfdProfStats.nMinEncryptTime == 0))
                {
...
const OMX_BUFFERHEADERTYPE* pBuffHdr)
{
    if(pBuffHdr)
    {
        ...
        if(tempExtra)
        {
            ...
            if(WFD_UNLIKELY(mWfdProfStats.nMinMuxTime == 0))
            {
            ...
            ALOGE_IF(mProfilingEnabled,
                "Frame num %lld encoding: %lld us encryption: "\
                "%lld us muxing: %lld us rndTrip: %lld us",
                tempExtra->nFrameNo, encTime, tempExtra->nEncryptTime,
                muxTime, rndTripTime);
            mWfdProfStats.nTotMuxTime += muxTime;
