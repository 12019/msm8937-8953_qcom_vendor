...
static bool is11ad() {
    ...
    if (property_get("sys.wfdservice.11ad", value, NULL)
            && (!strncmp("1", value, 1) || !strncmp("true", value, 4))) {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "current net type: WIGIG_P2P");
        return true;
    }
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "current net type: WIFI_P2P");
    return false;
}
...
int getP2pInterfaceName(char* pInterfaceName, int size)
{
    ...
    for (ifr = ifc.ifc_req; ifr < ifend; ifr++)
    {
      if (ifr->ifr_addr.sa_family == AF_INET)
      {
        ...
        if ((is11ad() && strncmp(ifr->ifr_name, "wigig", 5) == 0)
                || (!is11ad() && strncmp(ifr->ifr_name, "p2p", 3) == 0))
        {
...
int getPeerMacAddress( char* pMacAddr, char* pPeerIpAddr, int size )
{
    ...
    else
    {
        p2pifname[IFNAMSIZ-1]='\0';
        strlcpy(ifr.ifr_name, p2pifname, IFNAMSIZ);
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "getP2pInterfaceName ,"\
                                            "interface name = %s",ifr.ifr_name);
    }
    ...
    if( !arpFP )
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "getPeerMacAddress :/proc/net/arp file open failed");
        return -1;
    }
    ...
    while( fgets( line, sizeof(line), arpFP ) )
    {
         ...
         */
        if( strncmp( hwa, "00:00:00:00:00:00", 18  ) == 0 || strncmp( hwa, "FF:FF:FF:FF:FF:FF", 18 ) == 0 || strncmp( hwa, "ff:ff:ff:ff:ff:ff", 18 ) == 0 )
            continue;
            /*
             * Found!
             */
...
unsigned int getLinkSpeed(char *macaddr, unsigned int len, bool& queryIfName) {
    ...
    if(queryIfName)
    {
        if (getP2pInterfaceName(p2pifname, IFNAMSIZ) == -1)
        {
            if (is11ad()) {
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "getP2pInterfaceName failed "\
                        " Falling back to wigig0");
                strlcpy(p2pifname, "wigig0", IFNAMSIZ);
            } else {
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "getP2pInterfaceName failed "\
                        " Falling back to p2p0");
                strlcpy(p2pifname, "p2p0", IFNAMSIZ);
            }
        }
        else
        {
...
#endif
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM, "getLinkSpeed interface name = %s",iwrrequest.ifr_name);
    ...
    MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_MEDIUM,
                 "NetUtils:Macaddress = %s Linkspeed result = %d, linkspeed = %u",
                 inputstring,result,linkspeed * 500000);
...
bool getNetworkProperties(const char *mac_char,
                        unsigned int *nss,
                        unsigned int *rateflags,
                        unsigned int *freq,
...
#else
    if (getP2pInterfaceName(ifname, IFNAMSIZ) == -1)
    {
        if (is11ad()) {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "getP2pInterfaceName failed "\
                    " Falling back to wigig0");
