...
void rtspSink::applySettings(rtspParams *parsed)
{
    ...
    if (parsed->mesg.cmd == setParameterCmd &&
        ...
        // Cache client's rtcp port as it will get overwritten in setWFd and
        // then re-update client's wfd client port. This is due to miracast
        // spec weirdness where rtcp ports need to be determined during M3
        // capability negociation whereas RTSP spec sepcifies that such is
        // needed only on RTSP Setup and Response.
        const unsigned rtcpPort = session.getWfd().client.getRtcpPort0();
        session.setWfd(theirWfd);
        session.getWfd().client.setRtcpPort0(rtcpPort);
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "rtcpctrl: re-update device client's rtcp port to %u", rtcpPort);
    }
}
