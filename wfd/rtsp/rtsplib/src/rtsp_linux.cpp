...
void SETSOCKOPT(SOCKET cmdSock, int reuseaddr)
{
    if (setsockopt(cmdSock, SOL_SOCKET, SO_REUSEADDR,
        ...
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: Error: setsockopt %s" , std::string(strerror(errno)).c_str());
        SET_ERROR;
        return;
    }
}

void INETPTON(std::string ipaddr, sockaddr_in &saddr)
{
    if (inet_pton(AF_INET, ipaddr.c_str(), &saddr.sin_addr) < 0) {

        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: Error: inetpton  %s" , std::string(strerror(errno)).c_str());
        SET_ERROR;
        return;
...
void SEND(SOCKET sock, std::string message)
{
    if (send(sock, (const void *)message.c_str(), message.length(), MSG_NOSIGNAL) < 0) {
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: Error: send %s ", std::string(strerror(errno)).c_str());
        SET_ERROR;
        return;
    }
}
...
int RECV(SOCKET sock, char *recvline)
{
    ...
    if ((retval = recv(sock, (void *)recvline, MAXLEN, 0)) < 0) {
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: Error: recv %s" , std::string(strerror(errno)).c_str());
        SET_ERROR;
    }
    if (retval == 0) {
...
void LISTEN(SOCKET sock, int backlog)
{
    if (listen(sock, backlog) < 0) {
        ...
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: Error: listen %s " , std::string(strerror(errno)).c_str());
        SET_ERROR;
        return;
    }
}
...
SOCKET SOCK()
{
	...
	if ((retval = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        ...
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: Error: socket %s", std::string(strerror(errno)).c_str());
        SET_ERROR;
    }
    else
    {
...
void CONNECT(SOCKET sock, sockaddr_in &saddr)
{
    ...
    if (connect(sock, (sockaddr *)&saddr, addrlen) < 0) {
        ...
        {
          MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: connect %s", std::string(strerror(errno)).c_str());
          fd_set fdReadSet;
          FD_ZERO(&fdReadSet);
          ...
          if (selectStatus > 0) {
            if(FD_ISSET(sock, &fdReadSet))
            {
              ...
              {
                MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: Error: connect %s",std::string(strerror(errno)).c_str());
                SET_ERROR;
                return;
              }
            }
        ...
        if(errno == EISCONN)
        {
          MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: Client already Connected : %s", std::string(strerror(errno)).c_str());
          CLEAR_ERROR;
          return;
        }
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: Error: connect %s", std::string(strerror(errno)).c_str());
...
void BIND(SOCKET sock, sockaddr_in &saddr)
{
    ...
    if (bind(sock, (sockaddr *)&saddr, addrlen) < 0) {
        ...
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: Error: bind %s " , std::string(strerror(errno)).c_str());
        SET_ERROR;
        return;
    }
}
...
SOCKET ACCEPT(SOCKET sock, sockaddr_in &saddr)
{
    ...
    if ((retval = accept(sock, (sockaddr *)&saddr, (socklen_t *)&addrlen)) < 0) {
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: Error: accept %s" , std::string(strerror(errno)).c_str());
        SET_ERROR;
