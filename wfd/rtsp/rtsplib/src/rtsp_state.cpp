...
void rtspM6::response(rtspFSM *s, rtspSink *b)
{
    ...
    rtspWfd& sessionWfd = b->session.getWfd();

    // Server has RTCP port
    unsigned int serverRtcpPort = parsed.state.wfd.server.getRtcpPort0();
    if (serverRtcpPort > 0) {
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
                     "rtspM6::response found serverRtcpPort %u", serverRtcpPort);
        sessionWfd.server.setRtcpPort0(serverRtcpPort);
    }

    MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
        "setup response sessionWfd clientRtcpPort=%u, serverRtcpPort=%u",
        sessionWfd.client.getRtcpPort0(), sessionWfd.server.getRtcpPort0());
