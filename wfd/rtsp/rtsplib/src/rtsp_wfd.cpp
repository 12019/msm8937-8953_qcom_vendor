...
/*
 * Convert string to 64 bit bitset
 */
bitset<BITSET_16> stringToBitSize16(string value)
{
...
/*
 * Overloaded codec info output stream operator
 */
RTSPStringStream &operator<<(RTSPStringStream &stream, CodecInfo &video)
{
    int length = strlen(video.name);
    stream << setfill('0') << setw(length) << video.name << " ";
    stream << setfill('0') << setw(2) << hex << video.profile.to_ulong() << " ";
    stream << setfill('0') << setw(2) << hex << video.level.to_ulong() << " ";
    stream << setfill('0') << setw(10) << hex << video.ceaSupp.to_ulong() << " ";
    stream << setfill('0') << setw(10) << hex << video.vesaSupp.to_ullong() << " ";
    stream << setfill('0') << setw(10) << hex << video.hhSupp.to_ulong() << " ";
    stream << setfill('0') << setw(2) << hex << video.latency << " ";
    stream << setfill('0') << setw(4) << hex << video.minSliceSize << " ";
    stream << setfill('0') << setw(4) << hex << video.sliceEnc.to_ulong() << " ";
    stream << setfill('0') << setw(2) << hex << video.frameRateCtlSupp.to_ulong() << " ";
...
RTSPStringStream &operator>>(RTSPStringStream &stream, CodecInfo &video)
{
    string nameS,profileS, levelS, hResS, vResS, ceaS, vesaS, hhS,
           latencyS, minS, sliceEncS, frameS;
    ...
    video.ceaSupp = stringToBitSize8(ceaS);
    video.vesaSupp = stringToBitSize16(vesaS);
    video.hhSupp = stringToBitSize8(hhS);
    video.latency = static_cast<unsigned short>(stringToNumHex(latencyS));
    video.minSliceSize = static_cast<unsigned short>(stringToNumHex(minS));
    video.sliceEnc = stringToBitSize2(sliceEncS);
 ...
 * Overloaded video codec & operator
 */
CodecInfo CodecInfo::operator&(const CodecInfo &theirs)
{
    bitset<BITSET_2> intersectProfile;
    bitset<BITSET_2> intersectLevel;
    bitset<BITSET_8> intersectCeaSupp;
    bitset<BITSET_16> intersectVesaSupp;
    bitset<BITSET_8> intersectHhSupp;
    unsigned short intersectLatency;
    unsigned short intersectMinSliceSize;
    unsigned short intersectHres, intersectVres;
    ...
    CodecInfo codec(name,intersectProfile, intersectLevel, intersectHres,
                    intersectVres, intersectCeaSupp, intersectVesaSupp,
                    intersectHhSupp, intersectLatency,
                    intersectMinSliceSize, intersectSliceEnc,
                    intersectFrameRate);
...
{
    if(this != &theirs) {
        strlcpy(name,theirs.name,sizeof(name));
        profile = theirs.profile;
        level   = theirs.level;
        maxHres = theirs.maxHres;
        maxVres = theirs.maxVres;
        ceaSupp = theirs.ceaSupp;
        vesaSupp = theirs.vesaSupp;
        hhSupp = theirs.hhSupp;
        latency = theirs.latency;
        minSliceSize = theirs.minSliceSize;
        sliceEnc = theirs.sliceEnc;
        frameRateCtlSupp = theirs.frameRateCtlSupp;
    }
    return *this;
}

/*
 * Overloaded aux codec output stream operator
 */
RTSPStringStream &operator<<(RTSPStringStream &stream, auxCodec &aux)
{
    if (aux.valid)
    {
        int count = aux.getCodecCount();
        for(int i = 0; i < count; i++)
        {
             stream << aux.getName(i);
             MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,"RTSP_LIB :: &operator<< codec = %s",(aux.getName(i)).c_str());
             if(i < (count - 1))
                 stream << "; ";
             else
                 stream << " ";
        ...
        stream << setfill('0') << setw(2) << hex << aux.getMaxOverlay() << " ";
        stream << setfill('0') << setw(2) << hex << aux.getBlendMode()  << " ";
        stream << setfill('0') << setw(2) << hex << aux.auxRes.to_ulong();
    }
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,"RTSP_LIB :: &operator<< string = %s", stream.str().c_str());
    return stream;
}
...
RTSPStringStream &operator>>(RTSPStringStream &stream, auxCodec &aux)
{
    string nameS,maxOvS,blendModeS,resS;
    bool cont = true;
    int strLen = 0;
    aux.resetCodecCount();
    do
    {
        stream >> nameS;
        strLen = nameS.length();
        /* It means we have more than one codec */
        if(nameS.c_str()[strLen - 1] == ';')
            nameS.erase(strLen,strLen);
        /* If only one codec or we have reached
           parsing last codec which will not have ";"
           then we need to end parsing for codec*/
        else
            cont = false;
        aux.setName(nameS);
    }while(cont);
    ...
    aux.setMaxOverlay(stringToNum(maxOvS));
    aux.setBlendMode(stringToNum(blendModeS));
    aux.auxRes = stringToBitSize2(resS);
    if(aux.getCodecCount() > 0)
        aux.setValid(true);

    return stream;
}

auxCodec auxCodec::operator=(const auxCodec &theirs)
{
    if(this != &theirs) {
        int count = theirs.nameCnt;
        for(int i = 0; i<count;i++)
        {
            name[i] = theirs.name[i];
        }
        maxOverlay = theirs.maxOverlay;
        nameCnt    = theirs.nameCnt;
        blendMode  = theirs.blendMode;
        auxRes     = theirs.auxRes;
        valid      = theirs.valid;
    }
    return *this;
}
...
auxCodec auxCodec::operator&(const auxCodec &theirs)
{
    string intersectName[WFD_MAX_AUX_CODEC_SUPPORT];
    if(this != &theirs) {
        int theirCount = theirs.nameCnt;
        int localCount = nameCnt;
        int finalCount = 0;
        if(theirCount > WFD_MAX_AUX_CODEC_SUPPORT)
            theirCount = WFD_MAX_AUX_CODEC_SUPPORT;
        for(int i = 0; i<localCount;i++)
        {
            for(int j = 0; j < theirCount; j++)
            {
                if(name[i] == theirs.name[j])
                {
                    intersectName[finalCount] = name[i];
                    finalCount++;
                    break;
                }
            }
        ...
        {
            name[k] = intersectName[k];
        }
        maxOverlay = (maxOverlay <= theirs.maxOverlay ? maxOverlay : theirs.maxOverlay);
        nameCnt    = finalCount;
        blendMode  = blendMode & theirs.blendMode;
        auxRes     = auxRes & theirs.auxRes;
        valid = false;
        if(finalCount > 0)
            valid = true;
    }
...
rtspWfd& rtspWfd::operator&(const rtspWfd &rhs)
{
    ...
    auxCodecInfo = auxCodecInfo & rhs.auxCodecInfo;
    return *this;
}
...
void rtspWfd::assign(const rtspWfd &theirs)
{
   ...
   if ( theirs.contentProtection.getValid()){
    ...
    if(theirs.codecInfoCount > 0)
    {
        videoHeaderCodecInfo = theirs.videoHeaderCodecInfo;
        codecInfoCount = theirs.codecInfoCount;
        codecInfo = theirs.codecInfo;
    }

    if(theirs.dirStreamMode.getValid())
    {
        dirStreamMode = theirs.dirStreamMode;
    }

    if(theirs.auxCodecInfo.getValid())
    {
        auxCodecInfo = theirs.auxCodecInfo;
    }
}
...
rtspWfd& rtspWfd::operator+=(const rtspWfd &rhs)
{
    ...
    /*Populate if wfd_video_formats_extension is present*/
    if(rhs.codecInfoCount > 0) {
        /*Get hold of the count*/
        codecInfoCount = rhs.codecInfoCount;

        /*Copy the codec header*/
        if(rhs.videoHeaderCodecInfo.getValid()) {
            videoHeaderCodecInfo = rhs.videoHeaderCodecInfo;
        }
        ...
        codecInfo = (CodecInfo*)MM_Malloc(codecInfoCount*sizeof(CodecInfo));
        if(codecInfo != NULL) {
            for(int i = 0 ; i < codecInfoCount ; i++) {
                codecInfo[i] = rhs.codecInfo[i];
            }
        }
    }

    if(rhs.auxCodecInfo.getValid())
    {
        auxCodecInfo = rhs.auxCodecInfo;
    }
...
void rtspWfd::wfdParse(rtspWfdParams param, string line)
{
    ...
    break;
    case wfd_video_formats_extension:
        pTempPtr = NULL;
        codecCount = 0;
        codecInfoCount = 0;
        {
            char *pTraversePtr = NULL;
            string traverse(listStr);
            char *traverseList = (char*)traverse.c_str();
            char *pch;
            /*TODO: Revisit this logic*/
            /*Find the first space*/
            pch = strchr(traverseList, ' ');
            if(pch != NULL) {
                /*Update pch to point after space*/
                pch++;

                /*Find the second space*/
                pch = strchr(pch, ' ');
                if(pch != NULL) {
                    /*Codec is present. Parse it!*/
                    if ((fieldStr = strtok_r(traverseList, ",", &pTraversePtr))) {
                        codecCount++;
                        while((fieldStr = strtok_r(NULL, ",", &pTraversePtr)))
                        {
                            codecCount++;
                        }
                    }
        ...
        if(codecCount > 0)
        {
            ...
            codecInfo = (CodecInfo*)MM_Malloc(codecCount*sizeof(CodecInfo));
            if(codecInfo == NULL)
            {
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"RTSP_LIB :: codecInfo is not created");
                break;
            }
            codecInfoCount = codecCount;
            if ((fieldStr = strtok_r(listStr, ",", &pTempPtr))) {
              bool bParseVideoHeader = true;
              do
              {
                /*Catch the space*/
                string begin = findStart(fieldStr, ' ');
                ...
                if (!data.length())
                {
                    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"RTSP_LIB :: codecInfo is not created, Data is NULL");
                    break;
                }

                /*This is needed only once*/
                if(bParseVideoHeader) {
                  RTSPStringStream in(data);
                  in >> videoHeaderCodecInfo;
                  data.erase(0, 6);
                  bParseVideoHeader = false;
                }

                fieldStr = (char *)data.c_str();
                string prepData(fieldStr);
                RTSPStringStream prepIn(prepData);
                prepIn >> codecInfo[counter++];
              }while((fieldStr = strtok_r(NULL, ",", &pTempPtr)) && (--codecCount));
            }
        }
    break;
    case wfd_aux_stream_info:
    {
        pTempPtr = NULL;
        /* Point to data after the param name*/
        if((fieldStr = strtok_r(listStr, ",", &pTempPtr)))
        {
            string begin = findStart(fieldStr, ' ');
            fieldStr = (char *)begin.c_str();
            ...
            if (!data.length())
            {
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"RTSP_LIB :: wfd_aux_stream_info is not Pres");
                break;
            }
    ...
    break;
    case wfd_direct_streaming_mode:
    {
        string data = findStart(listStr,' ');
        if(!data.length())
            break;
        ...
        dirStreamMode.setValid(true);
        if(modeVal == "active")
        {
            dirStreamMode.setMode(true);
        }
        else if(modeVal == "inactive")
        {
            dirStreamMode.setMode(false);
        }
    }
    break;
    case wfd_invalid:
...
void rtspWfd::dump()
{
    ...
    /*Dump only if wfd_video_formats_extension is present*/
    if(codecInfoCount > 0)
    {
        RTSPStringStream ss;
        if(videoHeaderCodecInfo.getValid())
        {
            ss.str("");
            ss << videoHeaderCodecInfo;
            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,"RTSP_LIB :: %s", ss.str().c_str());
        }

        for(int count = 0 ; count < codecInfoCount ; count++)
        {
            ss.str("");
            ss << codecInfo[count];
            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,"RTSP_LIB :: %s", ss.str().c_str());
    ...
    if(auxCodecInfo.getValid() && auxCodecInfo.getCodecCount() > 0)
    {
        RTSPStringStream ss;
        ss << auxCodecInfo;
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,"RTSP_LIB :: %s", ss.str().c_str());
    ...
    if(dirStreamMode.getValid()) {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"RTSP_LIB :: Direct Streaming Valid");
    }else
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"RTSP_LIB :: Direct Streaming InValid");
    }
