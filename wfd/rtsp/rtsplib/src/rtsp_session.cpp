        ...
        for (int i = 1; i < numSupportedWfdParams; i++) {
            if (GetP->mesg.wfdParams[i]) {
                ...
                case wfd_video_formats_extension:
                    codecCount = state->state.wfd.codecInfoCount;
                    if(state->state.wfd.codecInfo!=NULL &&
                        codecCount > 0){
                       ss << wfdTable[i].wfdName << ": ";
                       ss << state->state.wfd.videoHeaderCodecInfo<<" ";
                       int counter = 0;
                       while(counter < codecCount)
                       {
                           if((state->state.wfd.codecInfo + counter) != NULL)
                           {
                               ss << state->state.wfd.codecInfo[counter++];
                               if(counter < codecCount)
                                  ss << ", ";
                           }
                       }
                       ss << crlf;
                    ...
                    {
                        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"RTSP_LIB :: video formats extensions NONE");
                        ss << wfdTable[i].wfdName << ": " << none << crlf;
                    }
                break;
                case wfd_aux_stream_info:
                    codecCount = state->state.wfd.auxCodecInfo.getCodecCount();
                    if(state->state.wfd.auxCodecInfo.getValid() && codecCount > 0){
                        ss << wfdTable[i].wfdName << ": ";
                        ss << state->state.wfd.auxCodecInfo;
                    }
                    else
                    {
                        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"RTSP_LIB :: wfd_aux_stream_info NONE");
                        ss << wfdTable[i].wfdName << ": " << none << crlf;
                    }
                break;
                default:
    ...
    if (SetP->mesg.type == cmdRequest) {
        ...
        for (int i = 1; i < numSupportedWfdParams; i++) {
            if (SetP->mesg.wfdParams[i]) {
                ...
                case wfd_video_formats_extension:
                    if(state->state.wfd.codecInfo!=NULL &&
                       state->state.wfd.codecInfoCount > 0){
                       ss << wfdTable[i].wfdName << ": ";
                       if (state->state.wfd.videoHeaderCodecInfo.getValid())
                           ss << state->state.wfd.videoHeaderCodecInfo << " ";
                       ss << state->state.wfd.codecInfo[0];
                       ss << crlf;
                    }
                break;
                case wfd_aux_stream_info:
                    if(state->state.wfd.auxCodecInfo.getValid() &&
                       state->state.wfd.auxCodecInfo.getCodecCount() > 0)
                    {
                       ss << wfdTable[i].wfdName << ": ";
                       ss << state->state.wfd.auxCodecInfo;
                    }
                ...
                case wfd_direct_streaming_mode:
                    if(state->state.wfd.dirStreamMode.getValid()){
                       ss << wfdTable[i].wfdName << ": ";
                       ss << state->state.wfd.dirStreamMode.getMode();
                       ss << crlf;
                    }
                break;
                default:
        ...
        if (state->state.wfd.client.getValid() && state->state.wfd.client.getRenegotiated()) {
            ...
            stream << state->state.wfd.client << crlf;
        }
#endif
        if (bError) {
          myStr = ss.str();
...
RTSPStringStream &operator<<(RTSPStringStream &stream, setupCommand *Setup)
{
    ...
    if (Setup->mesg.type == cmdRequest) {
        ...
        const unsigned clientRtcpPort = wfd.client.getRtcpPort0();

        if(wfd.client.getValid() && clientRtcpPort > 0)
        {
            stream << ";"<< "client_port=" << dec << wfd.client.getRtpPort0();
            stream << "-" << dec << clientRtcpPort;
            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
                "rtcpctrl: rtsp setup request set client rtcp port = %u",
                         clientRtcpPort);
        }
