...
void rtspServer::eventLoop()
{
    ...
    for (;;) {
            ...
            newConn->request();
             if (IS_ERROR) {
                ...
                CLEAR_ERROR;
                  ...
                  if (status >= FD_SETSIZE) {
                    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: acceptConnect invalid file_descriptor");
                    SET_ERROR;
                  }
                  ERROR_CHECK_SERVER ("RTSP_LIB :: acceptConnect failed");
        ...
        if (FD_ISSET(cmdSock, &read_fd_set)) {
            ...
            if (status >= FD_SETSIZE) {
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: acceptCmdConnect invalid file_descriptor");
                SET_ERROR;
            }
