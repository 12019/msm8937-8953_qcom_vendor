...
#define WFD_MAX_AUX_CODEC_SUPPORT 3
...
namespace rtsp_wfd {
    ...
    enum connector_type {
        VGA_connector,
        S_video_connector,
        Composite_video_connector,
        Component_video_connector,
        DVI_connector,
        HDMI_connector,
        reserved_connector_1,
        WiFi_display,
        Japanese_D_connector,
        SDI_connector,
        DP_connector,
        reserved_connector_2,
...
/*
 * wfd_video_formats_extension: Codec
 */
class CodecInfo{
public:
    CodecInfo(string nName,
              bitset<BITSET_2> profile,
              bitset<BITSET_2> level,
              unsigned short maxHres,
              unsigned short maxVres,
              bitset<BITSET_8> ceaSupp,
              bitset<BITSET_16> vesaSupp,
              bitset<BITSET_8> hhSupp,
              unsigned short latency,
              unsigned short minSliceSize,
              bitset<BITSET_2> sliceEnc,
              ...
              ceaSupp(ceaSupp), vesaSupp(vesaSupp), hhSupp(hhSupp),
              latency(latency), minSliceSize(minSliceSize),
              sliceEnc(sliceEnc), frameRateCtlSupp(frameRateCtlSupp)
    {
      strlcpy(name,nName.c_str(),sizeof(name));
    }
    ...
    unsigned short getMaxVres() const { return maxVres; }
    bitset<BITSET_8> getCeaSupp() const { return ceaSupp; }
    bitset<BITSET_16> getVesaSupp() const { return vesaSupp; }
    bitset<BITSET_8> getHhSupp() const { return hhSupp; }
    unsigned short getLatency() const { return latency; }
    unsigned short getMinSliceSize() const { return minSliceSize; }
    bitset<BITSET_2> getSliceEnc() const { return sliceEnc; }
    ...
    void setVesaSupp(bitset<BITSET_16> v) { vesaSupp = v; }
    void resetVesa() { vesaSupp.reset(); }
    void setVesaBit(unsigned b)
    {
        assert(b < BITSET_16);
    ...
    friend RTSPStringStream &operator>>(RTSPStringStream &stream, CodecInfo&);
    friend RTSPStringStream &operator<<(RTSPStringStream &stream, CodecInfo&);
...
class auxCodec {
public:
    /* Rest other variables ned to be set using set api calls */
    auxCodec() :nameCnt(0),valid(false) {}

    /* Member functions */
    //void resetModes() { blendMode.reset(); }
    void setBlendMode(int mode)
    {
        for(int i = 0 ; i < BITSET_2 ; i++)
        {
            if((mode & 0x1) > 0)
            {
              blendMode.set(i);
    ...
    void setAuxRes(int res)
    {
        /* Bits 2:0 maps to mode
           0b000 , 0b001, 0b010 */
        int mode = res & 0x7;
        /*
            mode = 0 - CEA , 1 - VESA , 2 - HH
         */
        auxRes.reset();
        if(mode > 0) // For CEA it will be 0
            auxRes.set(mode - 1);

        /* 7:3 contains index to table based on
           mode selected */
        int value = (res >> 3) & 0x1F;
        for(int i = 0; i < 5; i++)
        {
            if(value & 0x1)
            {
    ...
    int getAuxRes() const
    {
        ...
        int res = 0;
        for(int i = 3; i < BITSET_2 ; i++)
        {
            res |= auxRes[i];
        }
        dispMode = ((res << 3) | dispMode);
        return dispMode;
    }
    ...
    string getName(int count) const
    {
        string res = "";
        if(count < nameCnt)
            return name[count];
    ...
    int getCodecCount(){ return nameCnt;}
    void resetCodecCount(){nameCnt = 0;}

    void setMaxOverlay(int max_overlay){maxOverlay = max_overlay;}
    int getMaxOverlay(){return maxOverlay;}

    friend RTSPStringStream &operator>>(RTSPStringStream &stream, auxCodec&);
    friend RTSPStringStream &operator<<(RTSPStringStream &stream, auxCodec&);
    auxCodec operator&(const auxCodec&);
    auxCodec operator=(const auxCodec&);

private:
    string name[WFD_MAX_AUX_CODEC_SUPPORT];
    int nameCnt;
    int maxOverlay;
    bitset<BITSET_2> blendMode;
    bitset<BITSET_2> auxRes;
    bool valid;
};
...
class dirStreamSupport {
public:
    dirStreamSupport() : isSupport(true) {}

    bool getSupport() const {return isSupport;}
    void setSupport(bool support) {isSupport = support; }
private:
    bool isSupport;
};
...
public:
    ...
    void setNative(bitset<BITSET_2> d) {native = d; }
...
public:
    ...
    void setRtcpPort0(unsigned port) {rtcpPort0 = port; }
...
public:
    rtspWfd()
    {
        ...
        codecInfo = NULL;
        codecInfoCount = 0;
        dirStrSupport.setSupport(true);
        connectorType = rtsp_wfd::WiFi_display;
    }
    ...
    rtspWfd(audioCodec lpcm,
            ...
            isSecondarySink(false),connectorType(rtsp_wfd::WiFi_display)
    {
        UNUSED(video);
        codecInfo = NULL;
        codecInfoCount = 0;
...
#endif
        if(codecInfo!= NULL){
            wfdValid.set(wfd_video_formats_extension);
        }

        if(dirStreamMode.getValid()){
            wfdValid.set(wfd_direct_streaming_mode);
        }

        if(auxCodecInfo.getValid()){
            wfdValid.set(wfd_aux_stream_info);
        }
        /*if (isKeepAlive == true) {
    ...
    void reset()
    {
        ...
        videoHeaderCodecInfo.setValid(false);
        auxCodecInfo.setValid(false);
        if(codecInfo)
        {
            MM_Free(codecInfo);
            codecInfo = NULL;
    ...
    CodecInfo *codecInfo;
    videoCodec videoHeaderCodecInfo;
    auxCodec auxCodecInfo;
    dirStreamSupport dirStrSupport;
