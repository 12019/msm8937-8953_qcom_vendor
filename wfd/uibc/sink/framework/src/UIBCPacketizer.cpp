...
int32 UIBCPacketizer::feedInputBody(WFD_uibc_event_t* event, uint8* buffer,uint16 size)
{
  ...
  switch( event->type )
  {
    ...
    case WFD_UIBC_HID_KEY:
    {
      inputBodyLength = feedHIDKeyEvent(event, buffer, size);
      break;
    }
    default:
    {
...
/*==========================================================================
   FUNCTION     : feedHIDKeyEvent

   DESCRIPTION  : Feeds the inputbody for key event (HID)
...
int32 UIBCPacketizer::feedHIDKeyEvent(WFD_uibc_event_t * event,
                                          uint8 *        buffer,
                                          uint16         size)
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_DEBUG, "UIBCPacketizer:feedHIDKeyEvent");

  int32 keyEventLength = UIBC_HID_EVENT_HEADER_LENGTH /*5*/+
                         UIBC_HID_KEY_EVENT_IE_LENGTH /*8 for IR*/;
  int32 numBytes = 0;

  if( event!=  NULL && buffer != NULL && size >= keyEventLength )
  {
    WFD_uibc_key_event_parms keyEvent = event->parms.key_event;
    buffer[0] = USB; //HID Input Path
    buffer[1] = KeyBoard; //HID Type
    buffer[2] = HID_REPORT;
    //second,third octets  represents length of  description field.
    buffer[3] = (uint8)( UIBC_HID_KEY_EVENT_IE_LENGTH >> 8 );
    buffer[4] = (uint8)( UIBC_HID_KEY_EVENT_IE_LENGTH );
    switch( keyEvent.type )
    {
      case WFD_UIBC_KEY_DOWN:
      {
        buffer[5] = keyEvent.key_code_2;
        buffer[6] = 0;
        buffer[7] = keyEvent.key_code_1;
        buffer[8] = 0;
        buffer[9] = 0;
        buffer[10] = 0;
        buffer[11] = 0;
    ...
    //Padding will be present if total length is not 16 bit aligned
    keyEventLength += (keyEventLength % 2);
  ...
  else
  {
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                "UIBCPacketizer:feedHIDKeyEvent:Bad parm");
  }
  return numBytes;
}
