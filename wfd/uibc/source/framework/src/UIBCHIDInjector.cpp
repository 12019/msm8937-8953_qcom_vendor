...
mHIDType(-1)
{
    m_pTEO = MM_New_Args(WFDMMTouchEventObserver,(WFDMMTouchEventObserver::UHID));
...
int UIBCHIDInjector::read_uhid_event()
{
    ...
    case UHID_START:
        ...
        if (mHIDType == HID_MOUSE)
            m_eStatus = HID_DEV_CREATED;
        if (mHIDType == HID_KEYBOARD ) {
            if((m_pTEO->isWFDHIDDeviceFound() == true)) {
               m_eStatus = HID_DEV_CREATED;
    ...
    case UHID_OUTPUT:
        ...
        break;
    case UHID_OUTPUT_EV:
        MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_HIGH, "UHID_OUTPUT_EV from uhid-dev "
                     "type %d code %d value %d",ev.u.output_ev.type, ev.u.output_ev.code,
