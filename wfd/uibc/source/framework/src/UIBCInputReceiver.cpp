      ...
      if (m_packetReadIndex >= 0) {
            ...
            if(len >= 0 && ((m_packetReadIndex + len)< UIBC_PACKET_MAXIMUM_SIZE) && m_pHIDInjector){
               ...
               if(usageType == HID_REPORT) {
                ...
                MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                   "!!!UIBCInputReceiver:Out of bound access! due to rogue length %d", len);
                return FALSE;
            }
        } else {
            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                "!!!UIBCInputReceiver:Out of bound access! due to index %d", m_packetReadIndex);
            return FALSE;
        }
#endif
    break;
