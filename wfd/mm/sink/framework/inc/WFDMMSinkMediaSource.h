...
public:
    ...
    OMX_ERRORTYPE flushVideoQ();

    void restartVideoThread();

    void restartAudioThread();

    void restartGenericThread();
...
private:
    ...
    static void GenericThreadEntry(void* pThis,
                          unsigned int nSignal);
    void GenericThread(unsigned int nSignal);
    ...
    void updateTrackInfo(bool bOnOpenComplete);
    ...
    int fetchGenericSample
    (
        uint32
    );

    int fetchGenericSampleSecure
    (
        uint32
    );
    ...
    void pauseGenericThread();

    bool allocateAudioBuffers();
    bool deallocateAudioBuffers();

    bool allocateGenericBuffers();
    bool deallocateGenericBuffers();
