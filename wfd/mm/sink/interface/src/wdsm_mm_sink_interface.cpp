...
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "wfd_mm_get_video_capability_sink");
...
/** @brief Get local MM capability structure
  ...
  * @param[in] WFD_MM_HANDLE - WFD MM Sink instance handle
               ...
               int* - Integer pointer to hold the count of extended formats
               WFD_MM_Extended_capability_t - pointer to local extended capability structure
               WFD_device_t  - WFD device type
  *
  * @return  WFD_status_t - status
  */
WFD_status_t wfd_mm_get_local_capability_sink(WFD_MM_HANDLE hHandle,
                                              ...
                                              int* nAVFormatCount,
                                              WFD_MM_Extended_capability_t** pMMExtCfg,
                                              WFD_MM_Aux_capability_t** pMMAuxCfg,
                                              WFD_device_t tDevice)
{
  ...
  if(pMMCfg)
  {
   ...
   MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "wfd_mm_get_local_capability_sink entry pMMCfg =%x",
                pMMCfg);
    ...
    if(fp)
    {
      ...
      readCfgCapabilities.pExtCfgCapabilities = *pMMExtCfg;
      readCfgCapabilities.pAuxCfgCapabilities = *pMMAuxCfg;
      ...
      *pMMExtCfg = readCfgCapabilities.pExtCfgCapabilities;
      *nAVFormatCount = readCfgCapabilities.numAVFormatsExtention;
      *pMMAuxCfg = readCfgCapabilities.pAuxCfgCapabilities;
    ...
    int nRtcpEnabled = 0;
    getCfgItem(RTCP_RR_ENABLED_KEY ,&nRtcpEnabled);
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
                 "rtcpctrl: feature rtcp enabled=%d", nRtcpEnabled);
    ...
    if(1 == nRtcpEnabled)
    {
        ...
        MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
                     "rtcpctrl: Initializing this device's(sink) rtcp port=%d, sockfd=%d",
                     pMMCfg->transport_capability_config.port1_rtcp_id, pMMCfg->transport_capability_config.rtcpSock);
    }
    else
    {
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
                     "rtcpctrl: close rtcp sockfd %d as RTCP RR is not enabled", pMMCfg->transport_capability_config.rtcpSock);
        close(pMMCfg->transport_capability_config.rtcpSock);
    pMMCfg->transport_capability_config.rtcpSock = -1;
    }

    MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
        "enableWfd: transport capability config.ports %d, %d from getIPPortPair",
        (int)pMMCfg->transport_capability_config.port1_id ,
        (int)pMMCfg->transport_capability_config.port1_rtcp_id);
    ...
    /*No Direct Streaming support in case CONFIG file is missing*/
    //pMMExtCfg = NULL;
    //*nAVFormatCount = 0;
...
WFD_status_t wfd_mm_create_session_sink(
                    ...
                    WFD_MM_Aux_capability_t *pWFD_negotiated_capability_Aux,
                    WFD_MM_Extended_capability_t *pWFD_negotiated_capability_Ex,
                    WFD_MM_callbacks_t *pCallBack)
{
    WFD_status_t nErr = WFD_STATUS_FAIL;
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "wfd_mm_create_session_sink");
                 ...
                 pWFD_negotiated_capability && pCallBack)
    {
        *pHandle = (WFD_MM_HANDLE)WFDMMSink::CreateInstance();
        ...
        nErr = ((WFDMMSink*)(*pHandle))->setupMMSession(
                                    ...
                                    pWFD_negotiated_capability_Ex,
                                    pWFD_negotiated_capability_Aux,
                                    pCallBack);
    }
...
void wfd_mm_idr_request_sink(void *hHandle)
{
   ...
   {
      MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "wfd_mm_idr_request_sink: requesting IDR");
...
static int32 wfd_mm_construct_session_url( char* url, int32 urlSize, WFD_MM_capability_t* pMMCfg_negotiated)
{
  ...
  if(url != NULL && pMMCfg_negotiated != NULL)
  {
    ...
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "wfd_mm_construct_session_url: WFD session url is %s", url);
                                               ...
                                               WFD_MM_capability_t* pMMCfg_negotiated)
{
    ...
    WFD_status_t nErr = WFD_STATUS_FAIL;
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "wfd_mm_create_HDCP_session_sink");
...
WFD_status_t wfd_mm_update_session_sink(WFD_MM_HANDLE hHandle,
                                        WFD_MM_capability_t *WFD_negotiated_capability,
                                        WFD_MM_Extended_capability_t *WFD_negotiated_ext_capability,
                                        WFD_MM_Aux_capability_t* WFD_negotiated_aux_capability,
                                        ...
                                        bool bResetCodec)
{
    ...
    if( hHandle)
    {
        ...
        WFDMMSinkUpdateSession(WFD_negotiated_capability,
                               WFD_negotiated_ext_capability,
                               WFD_negotiated_aux_capability,
...
/** @brief Set Surface Property to Sink
  *
  * @param[in] WFD_MM_HANDLE - WFD MM Sink instance handle
               width - Width of Surface
               height - Height of Surface
               orientation - Orientation of Surface
  *
  * @return  WFD_status_t - status
  */
WFD_status_t wfd_mm_set_surface_prop_sink(WFD_MM_HANDLE hHandle, int width, int height, int orientation)
{
    WFD_status_t nErr = WFD_STATUS_SUCCESS;
    if(hHandle)
    {
        (void)((WFDMMSink*)hHandle)->setSurfaceProp(width,height,orientation);
    }
    return nErr;
}
