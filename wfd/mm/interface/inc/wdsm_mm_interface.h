...
typedef struct
{
    ...
    uint16          max_vres;
    uint32          supported_cea_mode;
    WFD_mode_res_t  cea_mode_list[MAX_MODE_LIST];
    uint64          supported_vesa_mode;
    WFD_mode_res_t  vesa_mode_list[MAX_MODE_LIST];
    uint32          supported_hh_mode;
    WFD_mode_res_t  hh_mode_list[MAX_MODE_LIST];
    uint8           decoder_latency;
    uint16          min_slice_size;
    uint8           slice_enc_params;
    uint8           frame_rate_control_support;
    WFD_mode_res_t  max_mode_list[WFD_VIDEO_MAX_MODE];
}WFD_extended_codec_config_t;
...
#define        H264_CEA_3840x2160p30       BIT17
#define        H264_CEA_3840x2160p60       BIT18
#define        H264_CEA_4096x2160p30       BIT19
#define        H264_CEA_4096x2160p60       BIT20
#define        H264_CEA_3840x2160p25       BIT21
#define        H264_CEA_3840x2160p50       BIT22
#define        H264_CEA_4096x2160p25       BIT23
#define        H264_CEA_4096x2160p50       BIT24
#define        H264_CEA_3840x2160p24       BIT25
#define        H264_CEA_4096x2160p24       BIT26
...
#define        H264_VESA_2560x1600p30      BIT30
#define        H264_VESA_2560x1600p60      BIT31
#define        H264_VESA_4096x2160p30      BIT32
#define        H264_VESA_4096x2160p60      BIT33
...
typedef struct {
    ...
    uint8           preferred_display_mode_supported;
    /* Extended codec type  */
    WFD_extended_codec_config_t* codec;
    uint8           num_codec;
} WFD_extended_video_codec_config_t;
...
#define WFD_MAX_NUM_Extended_Codec 10

typedef union {
    WFD_extended_video_codec_config_t   extended_video_config;
} WFD_extended_video_config;
...
/* Extended A/V capability used for DirStr*/
typedef struct {
    WFD_extended_video_config       extended_video_config;
    bool                            active;
}WFD_MM_Extended_capability_t;

typedef struct {
    uint8                           count;
    char                            name[WFD_GENERIC_INVALID][10];
    uint8                           maxOverlay;
    uint8                           blendMode;
...
/** @brief Get local MM capability structure
  ...
  * @param[in] WFD_MM_HANDLE - WFD MM Source instance handle
               ...
               int* - Integer pointer to hold count of additional codec supported
...
WFD_status_t wfd_mm_get_local_capability
  (WFD_MM_HANDLE, WFD_MM_capability_t*,int*,WFD_MM_Extended_capability_t**,WFD_MM_Aux_capability_t**,WFD_device_t);

/** @brief get negotiated capability
  *
  * @param[in] WFD_MM_HANDLE - WFD MM Source instance handle
               WFD_MM_capability_t - pointer to user-preferred capability structure
               WFD_MM_Extended_capability_t - pointer to user-preferred extended capability structure
...
WFD_status_t wfd_mm_get_negotiated_capability
  (WFD_MM_HANDLE,
   WFD_MM_capability_t*, WFD_MM_Extended_capability_t*,
   WFD_MM_capability_t*, WFD_MM_Extended_capability_t*,
   WFD_MM_capability_t*, WFD_MM_Extended_capability_t*,
   WFD_MM_capability_t*, WFD_MM_Extended_capability_t*);
...
WFD_status_t wfd_mm_create_session
  (WFD_MM_HANDLE*, WFD_device_t, WFD_MM_capability_t*, WFD_MM_Aux_capability_t*,
   WFD_MM_callbacks_t*, WFD_MM_Extended_capability_t * = NULL);
...
/** @brief update the session with new capabilty parameters
  ...
  * @param[in] WFD_MM_HANDLE - WFD MM Source instance handle
               ...
               WFD_MM_Extended_capability_t - pointer to extended video capability structure
               wfd_mm_update_session_cb  - Callback to inform session manager if there is any change in the MM capabilities
  *
  * @return  WFD_status_t - status
  */
WFD_status_t wfd_mm_update_session(WFD_MM_HANDLE,
                                   WFD_MM_capability_t*,
                                   WFD_MM_Extended_capability_t*,
                                   WFD_MM_Aux_capability_t*,
                                   wfd_mm_update_session_cb,
                                   bool);//not supported in initial implementation
...
/** @brief set the surface property at sink
  *
  * @param[in] WFD_MM_HANDLE - WFD MM Sink instance handle
  *
  * @param[in] uint32 - width
  *
  * @param[in] uint32 - height
  *
  * @param[in] uint32 - orientation
  *
  * @return  WFD_status_t - status
  */
WFD_status_t wfd_mm_set_surface_prop(WFD_MM_HANDLE hHandle, int width , int height, int orientation);
...
void wfd_mm_streaming_query_response(WFD_MM_HANDLE hHandle, bool bStreamingStatus);
