...
/** @brief create MM session
  ...
  * @return  WFD_status_t - status
  */
WFD_status_t wfd_mm_create_session(WFD_MM_HANDLE *phHandle, WFD_device_t tDevice,
                                   WFD_MM_capability_t *pNegCap, WFD_MM_Aux_capability_t  *pAuxCap,
                                   WFD_MM_callbacks_t* pCallback, WFD_MM_Extended_capability_t *pNegCapEx)
{
  WFD_status_t nErr = WFD_STATUS_FAIL;
  ...
  if( phHandle && (g_wfd_mm_handle_count < WFD_MM_MAX_HANDLES) )
  {
    *phHandle = NULL;
    if(tDevice == WFD_DEVICE_SOURCE)
    {
      nErr = wfd_mm_create_session_source(phHandle, tDevice, pNegCap, pAuxCap, pNegCapEx,pCallback);
    }
    else if( (tDevice == WFD_DEVICE_PRIMARY_SINK)||(tDevice == WFD_DEVICE_SECONDARY_SINK) )
    {
      nErr = wfd_mm_create_session_sink(phHandle, tDevice, pNegCap,pAuxCap,pNegCapEx,pCallback);
    }
...
/** @brief update the session with new capabilty parameters
  ...
  * @return  WFD_status_t - status
  */
WFD_status_t wfd_mm_update_session(WFD_MM_HANDLE hHandle,
                                   WFD_MM_capability_t* pCapability,
                                   WFD_MM_Extended_capability_t* pExtCapability ,
                                   WFD_MM_Aux_capability_t* pAuxCapability,
                                   wfd_mm_update_session_cb pCallback,
                                   bool bResetCodec)
{
  WFD_status_t nErr = WFD_STATUS_FAIL;
  int32 nHandleIndex = -1;
  ...
  if( nHandleIndex >=0  )
  {
    if(g_wfd_mm_handle_list[nHandleIndex].tDevice == WFD_DEVICE_SOURCE)
    {
      nErr = wfd_mm_update_session_source(hHandle, pCapability, pCallback, pExtCapability);
    }
    else if( (g_wfd_mm_handle_list[nHandleIndex].tDevice == WFD_DEVICE_PRIMARY_SINK)||(g_wfd_mm_handle_list[nHandleIndex].tDevice == WFD_DEVICE_SECONDARY_SINK) )
    {
      nErr = wfd_mm_update_session_sink(hHandle, pCapability,pExtCapability,pAuxCapability,pCallback, bResetCodec);
    }
  }
...
{
  WFD_status_t nErr = WFD_STATUS_FAIL;
  nErr = wfd_mm_get_negotiated_capability_source(hHandle,
                                                 pMMCfg_local, pMMCfg_local_ex,
                                                 pMMCfg_remote, pMMCfg_remote_ex,
                                                 pMMCfg_negotiated, pMMCfg_negotiated_ex,
                                                 pMMCfg_common, pMMCfg_common_ex);
  return nErr;
}
...
/** @brief Get local MM capability structure
  ...
  * @param[in] WFD_MM_HANDLE - WFD MM Source instance handle
               ...
               WFD_MM_Extended_capability_t - pointer to extended local capability structure
               WFD_device_t  - WFD device type
  *
  * @return  WFD_status_t - status
  */
WFD_status_t wfd_mm_get_local_capability
  (WFD_MM_HANDLE hHandle, WFD_MM_capability_t* pMMCfg, int* nAVFormatCount,
   WFD_MM_Extended_capability_t** pMMExtCfg, WFD_MM_Aux_capability_t** pMMAuxCfg,
   WFD_device_t tDevice)
{
  WFD_status_t nErr = WFD_STATUS_FAIL;
  ...
  if(tDevice == WFD_DEVICE_SOURCE)
  {
    nErr = wfd_mm_get_local_capability_source(hHandle, pMMCfg,nAVFormatCount,pMMExtCfg,pMMAuxCfg,tDevice);
  }
  else if( (tDevice == WFD_DEVICE_PRIMARY_SINK)||(tDevice == WFD_DEVICE_SECONDARY_SINK) )
  {
    nErr = wfd_mm_get_local_capability_sink(hHandle, pMMCfg,
                                            nAVFormatCount,pMMExtCfg,pMMAuxCfg,tDevice);
  }
  return nErr;
}
...
/** @brief set the surface property at sink
  ...
  * @return  WFD_status_t - status
  */
WFD_status_t wfd_mm_set_surface_prop
  (WFD_MM_HANDLE hHandle, int width , int height, int orientation)
{
  WFD_status_t nErr = WFD_STATUS_FAIL;
  nErr = wfd_mm_set_surface_prop_sink(hHandle, width,height,orientation);
  return nErr;
}
...
WFD_status_t wfd_mm_send_runtime_command(WFD_MM_HANDLE hHandle, WFD_runtime_cmd_t eCommand)
{
  ...
  if( nHandleIndex >=0  )
  {
    ...
    {
      /* Currently not used for Sinks*/
      nErr = WFD_STATUS_NOTSUPPORTED;
    }
  }
  return nErr;
}
...
{
    wfd_mm_streaming_query_response_source(hHandle,bStreamingStatus);
}
