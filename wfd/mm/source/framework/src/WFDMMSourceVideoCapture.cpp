...
)
{
    ...
    if(!pConfig )
    {
        result = OMX_ErrorBadParameter;
        WFD_OMX_ERROR_CHECK(result,
        "WFDMMSourceVideoCapture::Invalid data to resume configure capture module");
    }
    memcpy(&m_sConfig, pConfig, sizeof(m_sConfig));
    WFDMMLOGH2("Video Resoution after resume %d %d",
                m_sConfig.nFrameWidth,m_sConfig.nFrameHeight);
    SETSTATEOPENED;
    qdutils::setSecondaryDisplayStatus(qdutils::DISPLAY_VIRTUAL,
                                       qdutils::EXTERNAL_RESUME);
...
OMX_ERRORTYPE WFDMMSourceVideoCapture::captureThread(int nSignal)
{
    ...
    OMX_BUFFERHEADERTYPE* pBufferHdr;
    int32 resolutionSize = m_sConfig.nFrameWidth * m_sConfig.nFrameHeight;
    uint32 bufferCnt = 0;

    WFDMMLOGH1("CaptureThread video resolution %d",
                resolutionSize);
    /*--------------------------------------------------------------------------
    ...
    */
    ...
    if(nSignal == VIDEO_CAPTURE_START_EVENT)
    {
        ...
        while(1)
        {
            ...
            {
#ifdef SYSTRACE_PROFILE
                ATRACE_NAME("SMS Read");
#endif

                ret = m_pSurfaceMediaSrc->read(&pMediaBuffer);
            }
            int32 resolutionSizeNew =  m_sConfig.nFrameWidth * m_sConfig.nFrameHeight;
            if (pMediaBuffer)
            {
                pData = (uint8*)pMediaBuffer->data();
            }
            if(!ret && pMediaBuffer && pData &&
                (resolutionSize != resolutionSizeNew))
            {
                WFDMMLOGH("Resolution updated. DRC sequence starts");
                buffer_handle_t *pBufferHandle;
                unsigned long nBufferHandle = 0;
                for (int i = 0; i < sizeof(pBufferHandle); i++)
                {
                     nBufferHandle |= (unsigned long)pData[4 + i] << (i * 8);
                }
                pBufferHandle = (buffer_handle_t *)nBufferHandle;
                private_handle_t *pPvtHandle = (private_handle_t*)pBufferHandle;
                if(pPvtHandle->base_metadata)
                {
                    uint32 nSMSRes = resolutionSize;
                    MetaData_t *metadata =
                       reinterpret_cast<MetaData_t*>(pPvtHandle->base_metadata);
                    if(metadata && metadata->operation & UPDATE_BUFFER_GEOMETRY)
                    {
                        WFDMMLOGM2("Display sliceWidth=%d,sliceHeight=%d",
                                     metadata->bufferDim.sliceWidth,
                                     metadata->bufferDim.sliceHeight);
                        nSMSRes = metadata->bufferDim.sliceWidth *
                                  metadata->bufferDim.sliceHeight;
                     }
                     WFDMMLOGH2("DRC New %lu  buffer Res %lu",
                                                  resolutionSizeNew, nSMSRes);
                     if(nSMSRes != resolutionSizeNew)
                     {
                         BufferDim_t bufferDim =
                            {m_sConfig.nFrameWidth,m_sConfig.nFrameHeight};
                         int nRets =
                                 setMetaData((private_handle_t *)pBufferHandle,
                                 UPDATE_BUFFER_GEOMETRY, &bufferDim);
                         WFD_OMX_ERROR_CHECK(nRets,"setMetaData failed!");
                         pMediaBuffer->release();
                         continue;
                     }
                     else
                     {
                         WFDMMLOGH1("Display resolution changed start %d",
                                    bufferCnt);
                         if(bufferCnt < 5)
                         {
                             ++bufferCnt;
                         ...
                         {
                             resolutionSize = resolutionSizeNew;
                             WFDMMLOGH1("Display resoluton changed done %d",
                                          bufferCnt);
                             bufferCnt = 0;
                         }
                     }
            ...
            WFDMMLOGM3("m_pSurfaceMediaSrc->read returned %d %p in %lld",
            ...
            if(pMediaBuffer && ret == 0)
            {
                ...
                {
                    WFDMMLOGM4("refcount = %d size = %d rangeOffset = %d range_length = %d",
                    pMediaBuffer->refcount(), pMediaBuffer->size(),
                    ...
                    */
                    ...
                    {
                        m_pWFDMMSrcStats->recordDisplayFrame();
                    }
    ...
    {
        WFDMMLOGH("VIDEO_CAPTURE_STOP_EVENT");
        if(m_pSurfaceMediaSrc.get() != NULL)
        {
            m_pSurfaceMediaSrc->stop();
        }
...
)
{
    ...
    if(ISSTATEINIT || ISSTATEOPENED || bForce)
    {
        ...
        */
        ...
        WFDMMLOGL("Push frame to Input Q");
...
void WFDMMSourceVideoCapture::inputDeliveryHandler()
{
    ...
    WFDMMLOGM1("Timer kicked in with diff of %lld!",
    ...
    /*--------------------------------------------------------------------------
     Piggyback on this timer to print FPS stats
    ----------------------------------------------------------------------------
    */
    if(WFD_LIKELY(m_pWFDMMSrcStats))
    {
        m_pWFDMMSrcStats->printStatistics();
    }
    ...
    for(OMX_U32 i = 0; i < nFrames; i++)
    {
        ...
        if(pBufferHdr[i])
        {
           ...
           if(nRet == VC_INVALID_OP)
            ...
            {
                WFDMMLOGM1("No frame push Buffer Back because %d",nRet);
                m_pVideoInputQ->Push(&pBufferHdr[i], sizeof(&pBufferHdr[i]));
                break;
            }
        }
    ...
    */
    ...
    if(nFramesRead)
    {
        if(pBufferHdr[0])
        {
            WFDMMLOGM2("FRC Buffer Header 0 %p BUffer0 = %p",
                pBufferHdr[0],pBufferHdr[0]->pOutputPortPrivate);
        }
        if(pBufferHdr[1])
        {
            WFDMMLOGM2("FRC Buffer Header 1 %p BUffer1 = %p",
                pBufferHdr[1],pBufferHdr[1]->pOutputPortPrivate);
        }
    }
    if(m_pLastXmitBuffer)
    {
        WFDMMLOGM1("FRC Prev = %p", m_pLastXmitBuffer);
    }


    if(!m_bFirstFrameFetched)
    {
        WFDMMLOGH("FRC: Before Start");
    }
    else if(m_sConfig.bEnableFrameSkip)
    {
        if(nFramesRead == 1 && (m_nTimerCtr % 2 == 0))
        {
            ...
            --------------------------------------------------------------------
            */
            WFDMMLOGM("FRC: Frame Skip enabled:send new frame");
        ...
        else
        {
            ...
            {
                WFDMMLOGH("FRC: Case 1 in Frame Skipping, time to retransmit");
                pBufferHdr[0] = 0;
                m_pVideoInputQ->Pop(&(pBufferHdr[0]), sizeof(pBufferHdr[0]), 0);

                if(!pBufferHdr[0])
                {
                    WFDMMLOGM("No free buffer return");
                    return;
                }
    ...
    else if(!m_bMissedFrame && nFramesRead == 1)
    {
        ...
        ------------------------------------------------------------------------
        */
        WFDMMLOGH("FRC: Normal Buffer Case");
        pBufferHdr[0]->nTimeStamp = nCurrTime;
        if(m_pLastXmitBuffer == NULL)
        {
...
int WFDMMSourceVideoCapture::state(int state, bool get_true_set_false)
{
    ...
    {
        m_nState = state;
        WFDMMLOGH1("WFDMMSourceVideoCapture Moved to state %d",state);
    }
