...
const uint32 WFDMMSource::WFD_MM_SOURCE_STREAMING_START = 5;
const uint32 WFDMMSource::WFD_MM_SOURCE_STREAMING_STOP = 6;
const uint32 WFDMMSource::WFD_MM_SOURCE_THREAD_EXIT_EVENT = 7;
...
(
  ...
  WFD_MM_Aux_capability_t      *pAuxCap,
...
)
{
    ...
    OMX_U32 nMuxNumOvrlyBuffers = 0;



    if( (eWFDDeviceType != WFD_DEVICE_SOURCE ) || (pAuxCap == NULL) ||
        (pNegotiatedCap == NULL) ||
    ...
    /**---------------------------------------------------------------------------
    Create a local copy of aux capability
    ------------------------------------------------------------------------------
    */
    m_pAuxCap = (WFD_MM_Aux_capability_t *)
    ...
    memset(m_pAuxCap, 0, sizeof(WFD_MM_Aux_capability_t));
    memcpy(m_pAuxCap, pAuxCap, sizeof(WFD_MM_Aux_capability_t));

    /**---------------------------------------------------------------------------
    Create a local copy of the extended negotiated capability
    ------------------------------------------------------------------------------
    */
    if(pNegotiatedCapEx &&
       pNegotiatedCapEx->extended_video_config.extended_video_config.num_codec > 0)
    {
        m_pNegotiatedCapEx = static_cast<WFD_MM_Extended_capability_t *>
            (MM_Calloc(1, sizeof(WFD_MM_Extended_capability_t)));

        if (m_pNegotiatedCapEx == NULL)
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
              "WFDMMSource::allocating memory for extended negotiated capability failed");
            m_result = OMX_ErrorInsufficientResources;
            return ;
        }

        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "extended_video_config num_codec=%d",
                     (int)pNegotiatedCapEx->extended_video_config.extended_video_config.num_codec);

        m_pNegotiatedCapEx->extended_video_config.extended_video_config.codec =
            (WFD_extended_codec_config_t *)(MM_Malloc(sizeof(WFD_extended_codec_config_t)));

        if(m_pNegotiatedCapEx->extended_video_config.extended_video_config.codec != NULL)
        {
            memcpy(m_pNegotiatedCapEx->extended_video_config.extended_video_config.codec,
                   pNegotiatedCapEx->extended_video_config.extended_video_config.codec,
                   sizeof(WFD_extended_codec_config_t));
            m_pNegotiatedCapEx->extended_video_config.extended_video_config.native_bitmap =
                pNegotiatedCapEx->extended_video_config.extended_video_config.native_bitmap;
            m_pNegotiatedCapEx->extended_video_config.extended_video_config.preferred_display_mode_supported =
                pNegotiatedCapEx->extended_video_config.extended_video_config.preferred_display_mode_supported;
            m_pNegotiatedCapEx->extended_video_config.extended_video_config.num_codec =
                pNegotiatedCapEx->extended_video_config.extended_video_config.num_codec;
        }
        else
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                        "WFDMMSource:: m_pNegotiatedCapEx->extended_video_config.extended_video_config.codec is null");
            MM_Free(m_pNegotiatedCapEx);
            m_pNegotiatedCapEx = NULL;
        }
    }
    else
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "Not allocating m_pNegotiatedCapEx");
    }
    ...
    */
    ...
    /* Just check if extended video codec is valid or not */
    if(isVideoCodecValidEx(m_pNegotiatedCapEx) > 0)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
          "WFDMMSource::isVideoCodecValidEx -> Valid");
    }
    else
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
          "WFDMMSource::isVideoCodecValidEx -> Not Valid");
        if(m_pNegotiatedCapEx &&
           m_pNegotiatedCapEx->extended_video_config.extended_video_config.codec != NULL)
        {
            MM_Free(m_pNegotiatedCapEx->extended_video_config.extended_video_config.codec);
            m_pNegotiatedCapEx->extended_video_config.extended_video_config.codec = NULL;
        }
    }
...
*//*
         PARAMETERS:
*//**       @param
                           WFD_MM_Extended_capability_t  - WFD Extended Negotiated Capability
...
int WFDMMSource::isVideoCodecValidEx(WFD_MM_Extended_capability_t* pNegotiatedCapEx)
{
    int isValid = 0;
    if(pNegotiatedCapEx != NULL &&
       pNegotiatedCapEx->extended_video_config.extended_video_config.codec != NULL)
    {
        if(strncmp(pNegotiatedCapEx->extended_video_config.extended_video_config.codec->name,
                          "H.264",6) == 0)
        {
            isValid = 1;
        }
...
OMX_ERRORTYPE WFDMMSource::InitializeThreadsAndQueues()
{
    ...
    */
    if((0 != MM_Signal_Create(m_signalQ,
                              (void*)&WFD_MM_SOURCE_STREAMING_START,
                              NULL,
                              &m_WFDMMSourceStartStreamingSignal)))
    {
        m_result = OMX_ErrorInsufficientResources;
        return m_result;
    }
    ...
    */
    if((0 != MM_Signal_Create(m_signalQ,
                              (void*)&WFD_MM_SOURCE_STREAMING_STOP,
                              NULL,
                              &m_WFDMMSourceStopStreamingSignal)))
    {
        m_result = OMX_ErrorInsufficientResources;
        return m_result;
    }
...
OMX_ERRORTYPE WFDMMSource::CreateComponents()
{
    ...
    */
    ...
    m_pTEO = new WFDMMTouchEventObserver();
    if(!m_pTEO)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
            "Create Components: Failed to create TouchEventObserver");
        return OMX_ErrorInsufficientResources;
    }
    ...
    if( NULL!= m_pNegotiatedCap)
    {
      if(m_pSink &&
        ((m_pNegotiatedCap->video_method != WFD_VIDEO_INVALID) ||
         (m_pNegotiatedCapEx && m_pNegotiatedCapEx->extended_video_config.extended_video_config.codec != NULL)))
      {
...
#ifdef OVERLAY_CAPTURE
        if(m_pAuxCap != NULL && m_pAuxCap->count > 0)
        {
            m_pOverlaySource = new WFDMMSourceOverlaySource();
            if(!m_pOverlaySource)
            {
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                    "Create Components: Failed to create overlay Source");
                return OMX_ErrorInsufficientResources;
            }
        }
        else
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                "Create Components: Overlay not negotiated for current session");
        }
#endif
      }
   }
...
OMX_ERRORTYPE WFDMMSource::ConfigureComponents()
{
    ...
    */
    ...
    /**-------------------------------------------------------------------------
    Calculate num of buffers between OverlaySource and Mux
    ----------------------------------------------------------------------------
    */
#ifdef OVERLAY_CAPTURE
    if(m_pSink && m_pOverlaySource)
    {
        m_result = m_pSink->GetInNumBuffers(PORT_INDEX_OVERLAY, &nTempNumBufA);
    }

    if(m_result != OMX_ErrorNone)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
            "Failed to Get Mux Overlay Num Buffers");
    }

    if (m_pOverlaySource)
    {
        m_result = m_pOverlaySource->GetOutNumBuffers(0, &nTempNumBufB);
    }

    if(m_result != OMX_ErrorNone)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
            "Failed to Get Overlay Num Buffers");
    }

    if(m_pSink && m_pOverlaySource)
    {
        m_nBuffersOvrlySrcToMux = MAX(nTempNumBufA, nTempNumBufB);
    }
#endif
    /**---------------------------------------------------------------------------
    ...
    if (m_result == OMX_ErrorNone)
    {
        ...
        if(m_pSink)
        {
            ...
            if(m_pMuxConfig && m_result == OMX_ErrorNone)
            {
                ...
                m_pMuxConfig->nOverlayBufferCount = m_nBuffersOvrlySrcToMux;
    ...
    */
        ...
        if( m_pVideoSource )
        {
                ...
                frame_rate_control_support)
            {
                ...
                if(frame_rate_control_support & 0x1)
                {
                    ...
                    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,"Skip frame enabled, limit %lld",
    ...
    if(m_pOverlaySource)
    {
        if(!m_pOverlayConfig)
        {
            (void)ExtractOverlayParameters();
        }

        m_result = m_pOverlaySource->configure(OverlayDeliveryFn,
                                               eventHandlerCb,
                                               m_pOverlayConfig,
                                               WFDMM_OCAP_MODULE_ID,
                                               this);
        ...
        if (m_result != OMX_ErrorNone)
        {
            sCallBacks.update_event_cb(WFD_EVENT_MM_VIDEO,
                                       WFD_STATUS_FAIL, NULL);
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "Configure OverlaySource Failed");
        }
    }
#endif
...
void WFDMMSource::initDefaults()
{
    ...
    m_WFDMMSourceStartStreamingSignal = NULL;
    m_WFDMMSourceStopStreamingSignal = NULL;
    m_WFDMMSourceexitSignal = NULL;
    m_WFDMMSourceThreadHandle = NULL;
    m_wfdmmhandle = NULL;
    m_pAudioConfig = NULL;
    m_pOverlayConfig = NULL;
    m_bFrameSkipEnabled = OMX_FALSE;
    ...
    m_nBuffersOvrlySrcToMux = 0;
    m_IsErrorHandling = OMX_FALSE;
    m_nMinBitrate = 0;
    m_nMaxBitrate = 0;
    m_nCurrentPTS = 0;
    m_LinkSpeedSampleCnt = 0;
    m_nAudioTimeAdjust = 0;
    m_bHDCPStatusNotified = OMX_FALSE;
    memset(&sCallBacks,0,sizeof(WFD_MM_callbacks_t));
    m_bReinitUtils = true;
    memset(&m_sDSCodecInfo, 0, sizeof(m_sDSCodecInfo));

    FILE *pDumpFile = fopen("/data/media/dump.ts", "rb");
    if(pDumpFile != NULL)
    {
        fclose(pDumpFile);
        pDumpFile = NULL;
        if(MM_File_Delete("/data/media/dump.ts") != 0)
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                "WFDMMSource::Caution!!Dump file not deleted.");
    }

    // Get rtcp feature settings
    int nRtcpNotificationEnabled = 0;
    getCfgItem(RTCP_RR_NOTIFICATION_ENABLED_KEY ,&nRtcpNotificationEnabled);
    m_bNotifyRtcpRR = (1 == nRtcpNotificationEnabled ? true : false);
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
                 "rtcpctrl: isRtcpEnabled=%d", m_bNotifyRtcpRR);
}
...
WFDMMSource::~WFDMMSource()
{
    ...
    if(m_WFDMMSourceStartStreamingSignal)
    {
        MM_Signal_Release(m_WFDMMSourceStartStreamingSignal);
    }

    if(m_WFDMMSourceStopStreamingSignal)
    {
        MM_Signal_Release(m_WFDMMSourceStopStreamingSignal);
    }
    ...
    if (m_pOverlaySource)
    {
        MM_Delete(m_pOverlaySource);
        m_pOverlaySource = NULL;

        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
            "WFDMMSource::~WFDMMSource m_pOverlaySource is freed");
    }
#endif
    ...
    if (m_pNegotiatedCapEx)
    {
        if (m_pNegotiatedCapEx->extended_video_config.extended_video_config.codec)
        {
          MM_Free(m_pNegotiatedCapEx->extended_video_config.extended_video_config.codec);
          m_pNegotiatedCapEx->extended_video_config.extended_video_config.codec = NULL;
        }
                                                     ...
                                                     wfd_mm_update_session_cb pUpdateSessionCb)
{
    ...
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,"WFDMMSrc Update Capability called");
    ...
    {
        if(WFD_negotiated_capability_Ex &&
           WFD_negotiated_capability_Ex->extended_video_config.extended_video_config.num_codec > 0)
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM, "WFDMMSrc Resolution  Ex");
            getResolutionRefreshRate(
            WFD_negotiated_capability_Ex->extended_video_config.extended_video_config.codec->supported_cea_mode,
            WFD_negotiated_capability_Ex->extended_video_config.extended_video_config.codec->supported_vesa_mode,
            WFD_negotiated_capability_Ex->extended_video_config.extended_video_config.codec->supported_hh_mode);
        }
        else
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM, "WFDMMSrc Resolution  Normal");
            getResolutionRefreshRate(
        ...
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM, "WFDMMSrc Resolution changed");
            if( m_pVideoSource)
            {
               m_result = m_pVideoSource->Configure(m_pVidEncConfig,
                    m_nBuffersSreenSrcToMux,
        ...
        if(WFD_negotiated_capability->transport_capability_config.eRtpPortType !=
...
OMX_ERRORTYPE WFDMMSource::Play()
{
    ...
    OMX_BUFFERHEADERTYPE** ppOverlayBuffers = NULL;
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "WFDMMSource::Play");
    if( m_eState == MMWFDSRC_STATE_PLAY)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
            "WFDMMSource::already in play state, calling play callback WFD_STATUS_SUCCESS");
        if (m_wfd_mm_stream_play_cb)
        {
    ...
    // Get the allocated input buffers
#ifdef OVERLAY_CAPTURE
    if (m_result == OMX_ErrorNone && m_bMuxRunning == OMX_FALSE && m_pOverlaySource)
    {
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
            "WFDMMSource::Play giving buffers to Overlay source %ld",
            m_pMuxConfig->nOverlayBufferCount);

        ppOverlayBuffers = m_pOverlaySource->GetBuffers();
        if(NULL == ppOverlayBuffers)
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                "WFDMMSource::Play getbuffer from Overlay Source errored out");
            m_result = OMX_ErrorNotReady;
        }
    }
#endif
...
void WFDMMSource::LinkSpeedCheckTimerCb(void * handle)
{
    ...
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM,"LinkSpeed %ld", nLinkSpeed);
...
OMX_ERRORTYPE WFDMMSource::Pause()
{
...
#ifdef OVERLAY_CAPTURE
    if (m_pOverlaySource != NULL)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
            "WFDMMSource::Pause waiting for Overlay source to Pause");
        m_result = m_pOverlaySource->pauseCapture();
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
            "WFDMMSource::Pause overlay source to finished");
    }
#endif
    ...
    if(m_pTEO != NULL)
    {
        m_pTEO->stop();
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
            "WFDMMSource::Pause TEO to finished");
    }
...
OMX_ERRORTYPE WFDMMSource::Stop()
{
    ...
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
            "WFDMMSource::Stop waiting for Overlay  source to stopcapture");
        m_result = m_pOverlaySource->stopCapture();
    }
#endif
    ...
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
            "WFDMMSource::Stop waiting for m_pTEO to stop");
        m_pTEO->stop();
    }
...
int WFDMMSource::WFDMMSourceThread( void )
{
    ...
    while ( bRunning )
    {
        ...
        if ( 0 == MM_SignalQ_Wait( m_signalQ, (void **) &pEvent ) )
        {
            ...
            case WFD_MM_SOURCE_STREAMING_START:
                {
                    WFDMMLOGH("WFD_MM_SOURCE_STREAMING_START in WFDMMSource \n");
                    EnableStreamingResources();

                }
                break;
            case WFD_MM_SOURCE_STREAMING_STOP:
                {
                    WFDMMLOGH("WFD_MM_SOURCE_STREAMING_STOP in WFDMMSource \n");
                    DisableStreamingResources();
                }
                    break;
                    ...
                    WFD_MM_HANDLE wfdmmhandle,WFD_runtime_cmd_t eCommand)
{
    ...
    switch (eCommand)
    {
      ...
      case WFD_MM_CMD_ENABLE_STREAMING_FEATURE:
      {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                     "WFDMMSource:: Enable Streaming feature");
        if(m_pVideoSource)
        {
          retval = m_pVideoSource->DirectStreamingSupport(true);
        }
        break;
      }
      case WFD_MM_CMD_DISABLE_STREAMING_FEATURE:
      {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                     "WFDMMSource:: Disable Streaming feature");
        if(m_pVideoSource)
        {
          retval = m_pVideoSource->DirectStreamingSupport(false);
        }
        break;
      }
      default:
                                        ...
                                        void *ptr )
{
    ...
    if(ptr && !b_IsStopped)
    {
        ...
        pBuffer->nTimeStamp      += pWFDMMSource->m_nAudioTimeAdjust;
...
void WFDMMSource::SourceDeliveryFn(OMX_BUFFERHEADERTYPE* pBuffer, void *ptr)
{
    ...
    if(ptr && !b_IsStopped)
    {
        ...
        if (pBuffer->nFilledLen != 0)
        {
            ...
            if(!b_IsStopped)
            {
                OMX_ERRORTYPE eError = OMX_ErrorNone;
#ifdef SYSTRACE_PROFILE
                ATRACE_ASYNC_BEGIN("Mux Frame", static_cast<int32_t>
                    (reinterpret_cast<uintptr_t>(pBuffer)));
#endif
                MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_MEDIUM,
                             "WFDMMSource::SourceDeliveryFn  %lld flags = %d ",
                             ((OMX_BUFFERHEADERTYPE*)pBuffer)->nTimeStamp,
                              ((OMX_BUFFERHEADERTYPE*)pBuffer)->nFlags);
                eError = pWFDMMSource->m_pSink->DeliverInput(
                    (OMX_BUFFERHEADERTYPE *)pBuffer);
                pWFDMMSource->m_result = eError;
                if (eError != OMX_ErrorNone)
    ...
    /*==============================================================================
         ...
         FUNCTION:         OverlayDeliveryFn

         DESCRIPTION:
*//**       @brief         Call back from the OverlaySource
...
*//*==========================================================================*/
void WFDMMSource::OverlayDeliveryFn(OMX_BUFFERHEADERTYPE* pBuffer, void* ptr)
{
#ifdef OVERLAY_CAPTURE
    if(ptr && !b_IsStopped)
    {
        OMX_ERRORTYPE eError = OMX_ErrorNone;
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM, "WFDMMSource::OverlayDeliveryFn");
        WFDMMSource* pWFDMMSource = (WFDMMSource*) ptr;

        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM, "WFDMMSource::Mux DeliverInput %ld",
            pBuffer->nFilledLen );

        pBuffer->nInputPortIndex = PORT_INDEX_OVERLAY;

#if 0
    /* Dump to file and check */
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM, "OverlayDeliveryFn: DUMP_TO_FILE");
    FILE *fPNGTempFile = fopen("/data/media/pngAtWFDMMSource.png","wb");
    fwrite((pBuffer->pBuffer + 18),sizeof(char),pBuffer->nFilledLen,fPNGTempFile);
    fclose(fPNGTempFile);
#endif

        /* Check for nFilledLen */
        if(pBuffer->nFilledLen == 0)
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM,
                "OverlayDeliveryFn: Zero sized buffer. Give back to OverlaySrc");
            if(pWFDMMSource->m_pOverlaySource)
            {
                pWFDMMSource->m_pOverlaySource->SetFreeBuffer(
                    (OMX_BUFFERHEADERTYPE *)pBuffer);
            }
            return;
        }
        ...
        if (eError != OMX_ErrorNone)
        {
          if (pWFDMMSource->m_pOverlaySource)
          {
             pWFDMMSource->m_result = OMX_ErrorNone;
             MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM,
                         "WFDMMSource::OverlayDeliveryFn:Failed. Giving back buffer to OverlaySource");
             pWFDMMSource->m_pOverlaySource->SetFreeBuffer(
                    (OMX_BUFFERHEADERTYPE *)pBuffer);
          }
        }
    ...
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                  "WFDMMSource::OverlayDeliveryFn After Stop, Return");
        WFDMMSource* pWFDMMSource = (WFDMMSource*) ptr;
                                             ...
                                             OMX_IN OMX_BUFFERHEADERTYPE* pBuffer)
{
        ...
        if(pBuffer->nInputPortIndex == 1)//VIDEO)
        {
            ...
            {
                if(pBuffer->pOutputPortPrivate != (OMX_PTR)0xeaeaeaea)
                {
                    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
...
#ifdef OVERLAY_CAPTURE
        if(pBuffer->nInputPortIndex == PORT_INDEX_OVERLAY && pWFDMMSource->m_pOverlaySource)//OVERLAY)
        {
            // Deliver free overlay buffer to overlay source
            pWFDMMSource->m_result =
                pWFDMMSource->m_pOverlaySource->SetFreeBuffer(
                (OMX_BUFFERHEADERTYPE *)pBuffer);
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM,
                "WFDMMSource::WFDMMSourceThread Overlay buffer for fill");
        }
#endif
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM,
...
OMX_ERRORTYPE WFDMMSource::ExtractVideoParameters()
{
    ...
    if(m_pVidEncConfig != NULL)
    {
           ...
           /*&& (m_pNegotiatedCap->video_config.video_config.num_h264_profiles > 0)*/)
        {
            ...
            WFD_extended_codec_config_t *pConfigEx = NULL;
            if(m_pNegotiatedCapEx/* && m_pNegotiatedCapEx->active*/)
                pConfigEx = m_pNegotiatedCapEx->extended_video_config.extended_video_config.codec;
            ...
            if(!pConfigEx)
            {
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "No valid Extended setting");
                /* Not returning from here as we have got valid H.264 setting above
                   and can proceed with it.*/
            }

            /* Currently valid as for DS also H.264 is only supported */
            ...
            */
            if(pConfigEx)
            {
                (void)getResolutionRefreshRate(
                    pConfigEx->supported_cea_mode,
                    pConfigEx->supported_vesa_mode,
                    pConfigEx->supported_hh_mode);
            }
            else
            {
                (void)getResolutionRefreshRate(
            ...
            if(pConfigEx)
            {
                if(pConfigEx->profile != 0)
                    m_pVidEncConfig->nProfile = pConfigEx->profile;
                else
                    m_pVidEncConfig->nProfile =1;
                if(pConfigEx->level !=0)
                    m_pVidEncConfig->nLevel = pConfigEx->level;
                else
                    m_pVidEncConfig->nLevel = 2;

                MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
                    "WFDMMSource::ExtractVideoParameters Ex Profile = %d, Level = %d",
                    m_pVidEncConfig->nProfile,m_pVidEncConfig->nLevel);
                if(pConfigEx->min_slice_size)
                {
                    m_pVidEncConfig->nResyncMarkerSpacing =
                        (OMX_S32)pConfigEx->min_slice_size;
                ...
                MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
                    "WFDMMSource::ExtractVideoParameters Ex nResyncMarkerSpacing = %ld,"
                     "eResyncMarkerType = %d",  m_pVidEncConfig->nResyncMarkerSpacing,
                     m_pVidEncConfig->eResyncMarkerType);

                MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_HIGH,
                    "WFDMMSource::ExtractVideoParameters Ex %ld %ld %ld",
...
/*==============================================================================
         ...
         FUNCTION:         ExtractOverlayParameters

         DESCRIPTION:
*//**       @brief         Extract Overlay parameters from the pNegotiatedCap
...
*//*==========================================================================*/
OMX_ERRORTYPE WFDMMSource::ExtractOverlayParameters()
{
    if(!m_pOverlayConfig)
    {
        m_pOverlayConfig = (OverlaySrcConfigType*)
            MM_Malloc(sizeof(OverlaySrcConfigType));
    }

    if(!m_pOverlayConfig)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
            "WFDMMSource::WFDMMSource m_pOverlayConfig failed");
        return OMX_ErrorInsufficientResources;
    }

    memset(m_pOverlayConfig, 0, sizeof(OverlaySrcConfigType));

    if(m_pAuxCap)
    {
        uint8 supBM = m_pAuxCap->supportedBitMap;
        getAuxResolutionFromBitmap(supBM);
        m_pOverlayConfig->IsHdcp = false;
        MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_MEDIUM,
                "WFDMMSource:: ExtractOverlayParameters h[%d]w[%d]",
                m_pOverlayConfig->frameHeight, m_pOverlayConfig->frameWidth);
    }
    else
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
            "WFDMMSource::ExtractOverlayParameters m_pOverlayConfig filling defaults");
        m_pOverlayConfig->frameHeight = 480;
        m_pOverlayConfig->frameWidth = 800;
        m_pOverlayConfig->IsHdcp = false;
    }

    if (m_pMuxConfig) {
        m_pMuxConfig->sOverlayParams.format = MUX_OVERLAY_VENDOR_SPECIFIC;
    }
    return OMX_ErrorNone;
}
...
/*==============================================================================
         FUNCTION:         getAuxResolutionFromBitmap

         DESCRIPTION:
*//**       @brief         Get Resolution for cea mode to be used for overlay
                           resources
*//**
...
*//*
         PARAMETERS:
*//**       @param        cea_mode - cea_mode value
                          pConfig - pointer to OverlaySrcConfigType
...
@par     SIDE EFFECTS:
*//*==========================================================================*/
void WFDMMSource::getAuxResolutionFromBitmap(uint8 supportedBM)
{
    if(m_pOverlayConfig == NULL)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
            "getAuxResolutionFromBitmap: m_pOverlayConfig is NULL");
        return;
    }
    int mode = supportedBM & 0x3;
    int value = (supportedBM>>3)&0x1F;

    /* if there is no matching mode between
       source and sink, else it will set to default
       value if there is no proper value.*/
    m_pOverlayConfig->frameWidth  = 0;
    m_pOverlayConfig->frameHeight = 0;
    ...
    if(mode == 0)
    {
        ...
        {
        case 0:
            m_pOverlayConfig->frameWidth  = 640;
            m_pOverlayConfig->frameHeight = 480;
            break;
        case 1:
        case 2:
            m_pOverlayConfig->frameWidth  = 720;
            m_pOverlayConfig->frameHeight = 480;
            break;
        case 3:
        case 4:
            m_pOverlayConfig->frameWidth  = 720;
            m_pOverlayConfig->frameHeight = 576;
            break;
        case 5:
        case 6:
        case 10:
        case 11:
        case 15:
            m_pOverlayConfig->frameWidth  = 1280;
            m_pOverlayConfig->frameHeight = 720;
            break;
        case 7:
        case 8:
        case 9:
        ...
        case 13:
        case 14:
            m_pOverlayConfig->frameWidth  = 1920;
            m_pOverlayConfig->frameHeight = 1080;
            break;
        default:
            m_pOverlayConfig->frameWidth  = 640;
            m_pOverlayConfig->frameHeight = 480;
    ...
    else if(mode == 1)
    {
        switch(value)
        {
            ...
            case 1:
                m_pOverlayConfig->frameWidth  = 800;
                m_pOverlayConfig->frameHeight = 600;
                break;
            case 2:
            case 3:
                m_pOverlayConfig->frameWidth  = 1024;
                m_pOverlayConfig->frameHeight = 768;
                break;
            case 4:
            case 5:
                m_pOverlayConfig->frameWidth  = 1152;
                m_pOverlayConfig->frameHeight = 864;
                break;
            case 6:
            case 7:
                m_pOverlayConfig->frameWidth  = 1280;
            ...
            case 9:
                m_pOverlayConfig->frameWidth  = 1280;
                m_pOverlayConfig->frameHeight = 800;
                break;
            case 10:
            case 11:
                m_pOverlayConfig->frameWidth  = 1360;
                m_pOverlayConfig->frameHeight = 768;
                break;
            case 12:
            case 13:
                m_pOverlayConfig->frameWidth  = 1366;
                m_pOverlayConfig->frameHeight = 768;
                break;
            case 14:
            case 15:
                m_pOverlayConfig->frameWidth  = 1280;
                m_pOverlayConfig->frameHeight = 1024;
                break;
            case 16:
            case 17:
                m_pOverlayConfig->frameWidth  = 1400;
                m_pOverlayConfig->frameHeight = 1050;
                break;
            case 18:
            case 19:
                m_pOverlayConfig->frameWidth  = 1440;
                m_pOverlayConfig->frameHeight = 900;
                break;
            case 20:
            case 21:
                m_pOverlayConfig->frameWidth  = 1600;
                m_pOverlayConfig->frameHeight = 900;
                break;
            case 22:
            case 23:
                m_pOverlayConfig->frameWidth  = 1600;
                m_pOverlayConfig->frameHeight = 1200;
                break;
            case 24:
            case 25:
                m_pOverlayConfig->frameWidth  = 1680;
                m_pOverlayConfig->frameHeight = 1024;
                break;
            case 26:
            case 27:
    ...
    else if(mode == 2)
    {
        switch(value)
        {
            ...
            case 2:
            case 3:
                m_pOverlayConfig->frameWidth  = 854;
                m_pOverlayConfig->frameHeight = 480;
                break;
            case 4:
            case 5:
                m_pOverlayConfig->frameWidth  = 864;
                m_pOverlayConfig->frameHeight = 480;
                break;
            case 6:
            case 7:
                m_pOverlayConfig->frameWidth  = 640;
                m_pOverlayConfig->frameHeight = 360;
                break;
            case 8:
            case 9:
                m_pOverlayConfig->frameWidth  = 960;
                m_pOverlayConfig->frameHeight = 540;
                break;
            case 10:
            case 11:
                m_pOverlayConfig->frameWidth  = 848;
                m_pOverlayConfig->frameHeight = 480;
                break;
            default:
                m_pOverlayConfig->frameWidth  = 800;
    ...
    MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_MEDIUM,
        "getAuxResolutionFromBitmap: Setting overlay width[%d] height[%d]",
         m_pOverlayConfig->frameWidth, m_pOverlayConfig->frameHeight);
}
                                           ...
                                           uint32 hh_mode )
{
    ...
    if(cea_mode)
    {
        ...
        case 131072:
            m_pVidEncConfig->nFrameWidth = 3840;
            m_pVidEncConfig->nFrameHeight = 2160;
            m_pVidEncConfig->nFramerate = 30;
            output_width = 3840;
            output_height = 2160;
            m_nMaxBitrate = 40 * ONE_MBPS;
        ...
        case 262144:
            ...
            output_width = 3840;
            output_height = 2160;
            m_nMaxBitrate = 60 * ONE_MBPS;
            m_nMinBitrate = 10 * ONE_MBPS;
            break;

        case 524288:
            m_pVidEncConfig->nFrameWidth = 4096;
            m_pVidEncConfig->nFrameHeight = 2160;
            m_pVidEncConfig->nFramerate = 30;
            output_width = 4096;
            output_height = 2160;
    ...
    else if(vesa_mode)
    {
        ...
        case 536870912:
            ...
            break;
        case 1073741824:
            m_pVidEncConfig->nFrameWidth = 2560;
            m_pVidEncConfig->nFrameHeight = 1600;
            m_pVidEncConfig->nFramerate = 30;
            output_width = 2560;
            output_height = 1600;
            m_nMaxBitrate = 40 * ONE_MBPS;
...
*//**       @param        pThis - this pointer
                          ...
                          pData  - More information about event
...
void WFDMMSource::eventHandler(OMX_U32 nModuleId,
                      WFDMMSourceEventType nEvent,
                      OMX_ERRORTYPE nStatus, void* pData)
{
    (void)nModuleId;
    (void)nStatus;
    switch(nEvent)
    ...
    case WFDMMSRC_RTP_SESSION_START:
        {//TODO fix the warning below
            MUX_RTPinfo *rtpInfo =  (MUX_RTPinfo *)pData;
    ...
    case WFDMMSRC_ERROR:
        ...
        else if(nModuleId == WFDMM_OCAP_MODULE_ID)
        {
            if(sCallBacks.update_event_cb)
            {
                sCallBacks.update_event_cb(WFD_EVENT_MM_OVERLAY, WFD_STATUS_RUNTIME_ERROR, NULL);
            }
        }
        break;
    case WFDMMSRC_RTCP_RR_MESSAGE:
        if (m_bNotifyRtcpRR)
        {
            // Miracast Source app is interested in RTCP RR notifications
            if (sCallBacks.update_event_cb)
            {
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "RTCP RR message");
                sCallBacks.update_event_cb(
                   WFD_EVENT_MM_RTP,
    ...
    case WFDMMSRC_AUDIO_PROXY_CLOSE_DONE:
      ...
      break;
    case WFDMMSRC_STREAMING_REQ:
        if(sCallBacks.update_event_cb)
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                        "eventHandler: WFD_STATUS_STREAMING_REQ");


            memcpy(&m_sDSCodecInfo, (void*)pData, sizeof(m_sDSCodecInfo));

            sCallBacks.update_event_cb(WFD_EVENT_MM_STREAMING,
                                           WFD_STATUS_STREAMING_REQ,
                                           reinterpret_cast<void*>(pData));
            }
            break;
    case WFDMMSRC_STREAMING_START_SUCCESS:
        {
            WFDMMLOGH("WFDMMSRC_STREAMING_START_SUCCESS in eventHandler");
            if(pData)
            {
                m_nAudioTimeAdjust = *(OMX_TICKS*)pData;
            }
            if(m_WFDMMSourceStartStreamingSignal)
            {
                MM_Signal_Set(m_WFDMMSourceStartStreamingSignal);
            }
        }
            break;
    case WFDMMSRC_STREAMING_STOP_SUCCESS:
        {
            WFDMMLOGH("WFDMMSRC_STREAMING_STOP_SUCCESS in eventHandler");
        ...
        break;
    case WFDMMSRC_STREAMING_STOP_DONE_SUCCESS:
        {
            WFDMMLOGH("WFDMMSRC_STREAMING_STOP_DONE_SUCCESS in eventHandler");
            if(m_WFDMMSourceStopStreamingSignal)
            {
                MM_Signal_Set(m_WFDMMSourceStopStreamingSignal);
            }
        }
        break;

    case WFDMMSRC_STREAMING_VIDEO_STREAM_FLUSH:
        {
            if (m_pSink)
            {
                m_pSink->Flush();
...
void WFDMMSource::UpdateStreamingQueryResponse(bool bStreamingStatus)
{
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
                 "UpdateStreamingQueryResponse =%d", bStreamingStatus);
    if(m_pVideoSource)
    {
        m_pVideoSource->UpdateWFDMMStreamingQueryResponse(bStreamingStatus);
    }
}
OMX_ERRORTYPE WFDMMSource::EnableStreamingResources()
{
    /*--------------------------------------------------------------------------
     perform functionalities for startStreaming
     start overlay VDS Source
    ----------------------------------------------------------------------------
    */
    WFDMMLOGH("EnableStreamingResources \n");

    WFDMMLOGH("Update new params to MUXER before moving to Streaming");
    updateMuxVideoParams(true);

#ifdef OVERLAY_CAPTURE
    if (m_result == OMX_ErrorNone && m_pOverlaySource)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
            "WFDMMSource::EnableStreamingResources: starting Overlay source");
        m_result = m_pOverlaySource->startCapture();
    }
#endif
    ...
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
            "WFDMMSource::EnableStreamingResources: starting TouchEvent Observer");
        m_pTEO->start();
    }
...
OMX_ERRORTYPE WFDMMSource::DisableStreamingResources()
{
    /*--------------------------------------------------------------------------
     perform functionalities for stopStreaming
     stop overlay VDS Source
    ----------------------------------------------------------------------------
    */

    WFDMMLOGH("DisableStreamingResources \n");
    ...
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
            "WFDMMSource::DisableStreamingResources: Stop TouchEvent Observer");
        m_pTEO->stop();
    }

    WFDMMLOGH("Update new params to MUXER before moving to Mirroring");

    updateMuxVideoParams(false);
...
/*==============================================================================
         ...
         FUNCTION:         updateMuxVideoParams

         DESCRIPTION:
*//**       @brief         Updates Video-related params for MUX
...
*//*==========================================================================*/
OMX_ERRORTYPE WFDMMSource::updateMuxVideoParams(bool bStreamingActive)
{
    OMX_ERRORTYPE result = OMX_ErrorNone;

    if(!m_pMuxConfig || !m_pSink)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
             "WFDMMSource:: Insufficient resources for updateMuxVideoParams");
        return OMX_ErrorInsufficientResources;
    }
    ...
    if(bStreamingActive)
    {
        MuxerConfigType* pTempMuxConfig = (MuxerConfigType*)MM_Malloc(sizeof(MuxerConfigType));

        if(!pTempMuxConfig)
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "updateMuxVideoParams Malloc failed");
            return OMX_ErrorInsufficientResources;
        }

        memset(pTempMuxConfig, 0, sizeof(MuxerConfigType));
        memcpy(pTempMuxConfig, m_pMuxConfig, sizeof(MuxerConfigType));

        if(strncmp(m_sDSCodecInfo.codecType, "H.264",6) == 0)
        {
            pTempMuxConfig->svideoParams.format = MUX_VIDEO_H264;
        }
        else if(strncmp(m_sDSCodecInfo.codecType, "MPEG4",6) == 0)
        {
            pTempMuxConfig->svideoParams.format = MUX_VIDEO_MPEG4;
        }
        else
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                "WFDMMSource:: Incorrect Codec Type for MUX's video param updates");
            pTempMuxConfig->svideoParams.format = MUX_VIDEO_INVALID;
            MM_Free(pTempMuxConfig);
            return OMX_ErrorUndefined;
        }

        pTempMuxConfig->svideoParams.height     = (OMX_S16)m_sDSCodecInfo.height;
        pTempMuxConfig->svideoParams.width      = (OMX_S16)m_sDSCodecInfo.width;
        pTempMuxConfig->svideoParams.eH264Level = (OMX_VIDEO_AVCLEVELTYPE)m_sDSCodecInfo.level;
        pTempMuxConfig->svideoParams.eH264Profile = (OMX_VIDEO_AVCPROFILETYPE)m_sDSCodecInfo.profile;

        result = m_pSink->Update(pTempMuxConfig);
    ...
    {
        //Moving to mirroring. Update Mirroring configuration to MUX
        result = m_pSink->Update(m_pMuxConfig);
    }

    if(result != OMX_ErrorNone)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                    "WFDMMSource:: Failed to Update video params");
    }
