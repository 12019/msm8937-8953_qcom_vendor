...
OMX_BOOL AudioSource::m_bIsProxyReset = OMX_FALSE;
...
/*==============================================================================
         ...
         FUNCTION:         AudioSystemCb

         DESCRIPTION:
*//**       @brief         callback from audio system binder interface
*//**
@par     DEPENDENCIES:
                            None
*//*
...
@par     SIFE EFFECTS:
                            None
*//*==========================================================================*/
void AudioSystemCb(android::status_t status)
{
  if(status == android::DEAD_OBJECT)
  {
      MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "Audio System Died");
      AudioSource::m_bIsProxyReset = OMX_TRUE;
  }
}
...
*//*==========================================================================*/
 ...
 :
    ...
    m_pPcmHandle(NULL)
  {
      ...
      android::AudioSystem::setErrorCallback(AudioSystemCb);
      return;
  }
...
*//*==========================================================================*/
  AudioSource::~AudioSource()
  {
    ...
    {
         MM_Delete_Array(m_pHDCPInputBuffers);
    }
      ...
      void *appData)
  {
          ...
          pEventHandlerFn != NULL)
      {
...
#if !defined(PLATFORM_MSM_COBALT)
          m_nAudiodevice = 8;
#else
          m_nAudiodevice = 7;
#endif
          MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
                       "Using audio device id %u", m_nAudiodevice);
           ...
           (HDCP_STATUS_SUCCESS == m_pHdcpHandle->m_eHdcpSessionStatus))
          {
              ...
              result = AllocateAudioHDCPBuffers(uBufferSize);
...
/*=============================================================================
         ...
         FUNCTION:         AllocateAudioHDCPBuffers

         DESCRIPTION:
*//**       @brief         This is the WFDMMAudioSource Allocate HDCP Buffers
                           function for allocating input buffers to HDCP module for Audio encryption .
*//**
...
*//*=========================================================================*/
  OMX_ERRORTYPE AudioSource::AllocateAudioHDCPBuffers(unsigned int uBufferSize)
  {

      MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "AudioSource::AllocateAudioHDCPBuffers");
      ...
      m_pHDCPInputBuffers = MM_New_Array(struct buffer, m_nBuffers);
      MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM,
  ...
  OMX_ERRORTYPE AudioSource::Audio_Handler (void * audioHandle)
  {
      ...
      if (pThis->bKeepGoing == OMX_TRUE )
      {
          ...
          if(pThis->m_bFlushPending == OMX_TRUE && OMX_TRUE == pThis->m_bIsProxyOpened)
          {
            ...
            {
              if(0 != pcm_mmap_read(pThis->m_pPcmHandle, pThis->m_pAudioDataBfr,
              ...
              {
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "FLushing old frames");
              }
            }
          }
               ...
               m_bIsProxyReset == OMX_TRUE))
         {
           ...
           m_bIsProxyReset = OMX_FALSE;
           pThis->m_bIsProxyAvailabile = OMX_FALSE;
         }
         ...
         else
         {
           if (pThis->m_pPcmHandle != NULL && bDataAvailable == OMX_FALSE &&
              0 != pcm_mmap_read(pThis->m_pPcmHandle, pThis->m_pAudioDataBfr,
                          (unsigned int ) pThis->m_nAudioBufsize))
           {
             if(result == OMX_ErrorNone)
             {
...
// 2. Append Private Data as Extra Data to the end of Elementary Stream Buffer
          ...
          pThis->m_pHdcpHandle->m_eHdcpSessionStatus == HDCP_STATUS_SUCCESS )
      {
        ...
        {
          MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_LOW,
              ...
              if (pThis->pBufferHdr->nFilledLen)
              {
                     ...
                     (OMX_U64)nTimeStamp - (OMX_U64)pThis->m_nCurrTime > nDriftCutoff)
                  {
                     MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
                       ...
                       TIMESTAMP = %lld ms ACTUAL = %lld", nTimeStamp, pThis->m_nCurrTime);
...
OMX_ERRORTYPE AudioSource::ConfigureAudioProxyDevice()
{
  ...
  {
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                 "ConfigureAudioProxyDevice: pcm_open failed\n");
    return OMX_ErrorDynamicResourcesUnavailable;
  }
  else if(!pcm_is_ready(m_pPcmHandle))
  {
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                 "ConfigureAudioProxyDevice: pcm handle is not valid\n");
