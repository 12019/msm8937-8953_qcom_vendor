...
#define OMX_INIT_STRUCT(_s_, _name_)            \
  ...
  Muxer::Muxer(EmptyDoneCallbackType pEmptyDoneFn,
    ...
    m_pSourcePort(NULL)
   {
    ...
    memset(&omxPortOverlay, 0, sizeof(omxPortOverlay));
       ...
       */
       ...
       OMX_INIT_STRUCT(&omxPortOverlay, OMX_PARAM_PORTDEFINITIONTYPE);
       omxPortOverlay.nPortIndex = (OMX_U32) PORT_INDEX_OVERLAY; // input
       ret = OMX_GetParameter(m_hMuxer,
               OMX_IndexParamPortDefinition,
               (OMX_PTR) &omxPortOverlay);

       if(ret != OMX_ErrorNone)
       {
           MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                       "::Muxer Failed to get Overlay Port Def");
       }

       m_nAudioBuffers = omxPortAudio.nBufferCountMin;
       m_nVideoBuffers = omxPortVideo.nBufferCountMin;
       m_nOverlayBuffers = omxPortOverlay.nBufferCountMin;
                                 ...
                                 OMX_U32 nModuleId)
  {
     ...
     OMX_OTHER_FORMATTYPE eOverlayFormat = OMX_OTHER_FormatVendorReserved;
     ...
     else
     {
        ...
        m_nOverlayBuffers = pConfig->nOverlayBufferCount;

        MM_MSG_PRIO4(MM_GENERAL, MM_PRIO_HIGH, "Entered configure : \
            Num Audio buffers = %ld Num Video buffers = %ld \
            Num Overlay buffers = %ld Mux format = %d", m_nAudioBuffers,
            m_nVideoBuffers, m_nOverlayBuffers, pConfig->eFormat);
     ...
     if( (result == OMX_ErrorNone) && (pConfig->sEncryptParam.nStreamEncrypted == OMX_TRUE))
     {
        ...
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "Muxer::Configure calling OMX_GetExtensionIndex");
        if( OMX_GetExtensionIndex(m_hMuxer,
     ...
     //////////////////////////////////////////
        ...
        if (result == OMX_ErrorNone)
        {
         ...
         //////////////////////////////////////////
         // Index param Overlay Init
         //////////////////////////////////////////
        if (result == OMX_ErrorNone)
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "Overlay CODEC Settings");
           //////////////////////////////////////////
           // Overlay init parameters
           //////////////////////////////////////////
           switch(pConfig->sOverlayParams.format)
           {
           case MUX_OVERLAY_VENDOR_SPECIFIC:
              {
                 MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM, "Overlay Format Vendor Specific Setting");
                 eOverlayFormat = OMX_OTHER_FormatVendorReserved;
                 break;
              }
           default:
               MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "Overlay Format Disable Settings");
               OMX_SendCommand(m_hMuxer, OMX_CommandPortDisable, PORT_INDEX_OVERLAY,0);
               m_bOverlayPortEnabled =  OMX_FALSE;
               eOverlayFormat = OMX_OTHER_FormatMax;
               break;
           }
        }
     }
     ...
     if (result == OMX_ErrorNone)
     {
        ...
        if (m_bOverlayPortEnabled) {
            ...
            if(result != OMX_ErrorNone)
            {
                   MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "Overlay PortDef FAILED");
            }

            if(omxPortOverlay.eDomain == OMX_PortDomainOther)
            {
               m_nOverlayBuffers = MAX(m_nOverlayBuffers, omxPortOverlay.nBufferCountMin) ;
               MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM, "Num Overlay buffers portdef = %ld", omxPortOverlay.nBufferCountMin);
               omxPortOverlay.format.other.eFormat = eOverlayFormat;
               omxPortOverlay.nBufferCountActual   = m_nOverlayBuffers;
            }
            result = OMX_SetParameter(m_hMuxer,
               OMX_IndexParamPortDefinition,
               (OMX_PTR) &omxPortOverlay);
        }
        MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_HIGH, "Num Audio buffers = %ld Num Video buffers = %ld Num Generic buffers = %ld",
                                 m_nAudioBuffers, m_nVideoBuffers, m_nOverlayBuffers);
   ...
   //     }
        ...
        m_nOverlayBufferSize = (OMX_S32) omxPortOverlay.nBufferSize;
     }
  ...
  /////////////////////////////////////////////////////////////////////////////
   ...
   if(portIndex==(OMX_U32)PORT_INDEX_AUDIO )
    ...
    else if(portIndex==(OMX_U32)PORT_INDEX_OVERLAY)
    {
        m_bOverlayUseBuffer = bInUseBuffer;
        m_pOverlayBuffers   = pInBuffers;
        if(pInBuffers[0]->nAllocLen != omxPortOverlay.nBufferSize ||
          omxPortOverlay.nBufferCountActual != nBuffers)
        {
            omxPortOverlay.nBufferCountActual = nBuffers;
            omxPortOverlay.nBufferSize = pInBuffers[0]->nAllocLen;
            m_nOverlayBuffers = nBuffers;
            result = OMX_SetParameter(m_hMuxer,
               OMX_IndexParamPortDefinition,
               (OMX_PTR) &omxPortOverlay);
            if(result != OMX_ErrorNone)
  ...
  OMX_ERRORTYPE Muxer::DeliverInput(OMX_BUFFERHEADERTYPE* pBuffer)
  {
    ...
    else if( pBuffer->nInputPortIndex ==  PORT_INDEX_OVERLAY)
    {
       MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "WFDMMSource::MuxerOverlayEmptyBufferDone %ld",pBuffer->nFilledLen );
    }
...
//       MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "WFDMMSource::MuxerEmptyBufferDone %d",pBuffer->nFilledLen );
  ...
  OMX_ERRORTYPE Muxer::GetInNumBuffers(OMX_U32 nPortIndex, OMX_U32 *nBuffers)
  {
    if(nPortIndex != PORT_INDEX_VIDEO &&
       ...
       nPortIndex != PORT_INDEX_OVERLAY)
    {
      return OMX_ErrorBadPortIndex;
    }

    *nBuffers =
        ( (nPortIndex == PORT_INDEX_AUDIO ) ? m_nAudioBuffers
                                            : ((nPortIndex == PORT_INDEX_VIDEO) ? m_nVideoBuffers
                                                                                : m_nOverlayBuffers) );
  ...
  OMX_ERRORTYPE Muxer::SetNumBuffers(OMX_U32 nPortIndex, OMX_U32 nBuffers)
  {
      ...
      else if(nPortIndex == PORT_INDEX_OVERLAY)
      {
          if(nBuffers < (unsigned int)m_nOverlayBuffers)
          {
              return OMX_ErrorBadParameter;
          }
          if(nBuffers != (unsigned int)m_nOverlayBuffers )
          {
              eRet = OMX_GetParameter(m_hMuxer, OMX_IndexParamPortDefinition,&omxPortOverlay);

              if(eRet == OMX_ErrorNone)
              {
                  omxPortOverlay.nBufferCountActual = nBuffers;
                  eRet = OMX_SetParameter(m_hMuxer,OMX_IndexParamPortDefinition,&omxPortOverlay);
              }
          }
      }
      else
  ...
  OMX_BUFFERHEADERTYPE** Muxer::GetBuffers(OMX_U32 nPortIndex)
  {
    ...
    if (m_eState == OMX_StateExecuting)
    {
        ppBuffers = ((nPortIndex == (OMX_U32)PORT_INDEX_AUDIO )? m_pAudioBufferHdrs
                                                                : ((nPortIndex == (OMX_U32)PORT_INDEX_VIDEO) ? m_pVideoBufferHdrs
                                                                                                              : m_pOverlayBufferHdrs));
    ...
    if(!m_hMuxer)
    {
     ...
     if( m_bAudioPortEnabled)
     {
         ...
         if (m_bAudioUseBuffer == OMX_TRUE)
         {
            ...
            {
               MM_MSG_PRIO4(MM_GENERAL, MM_PRIO_HIGH, "USE BUFFER INPUT: allocating audio buffer : %p %p %ld %p", m_hMuxer,&m_pAudioBufferHdrs[i],m_pAudioBuffers[i]->nAllocLen,m_pAudioBuffers[i]->pBuffer  );
     ...
     if(m_bVideoPortEnabled)
     {
         ...
         if (m_bVideoUseBuffer == OMX_TRUE)
         {
            ...
            {
               MM_MSG_PRIO4(MM_GENERAL, MM_PRIO_HIGH, "USE BUFFER INPUT: allocating Video buffer: %p %p %ld %p", m_hMuxer,&m_pVideoBufferHdrs[i],m_pVideoBuffers[i]->nAllocLen,m_pVideoBuffers[i]->pBuffer  );
     ...
     if(m_bOverlayPortEnabled)
     {
         m_pOverlayBufferHdrs = new OMX_BUFFERHEADERTYPE*[m_nOverlayBuffers];
         if (m_bOverlayUseBuffer == OMX_TRUE)
         {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM, "Level 1USE BUFFER INPUT: allocating Overlay buffer");
            for (i = 0; i < m_nOverlayBuffers; i++)
            {
               MM_MSG_PRIO4(MM_GENERAL, MM_PRIO_HIGH, "USE BUFFER INPUT: allocating Overlay buffer: %p %p %ld %p", m_hMuxer,&m_pOverlayBufferHdrs[i],m_pOverlayBuffers[i]->nAllocLen,m_pOverlayBuffers[i]->pBuffer);

               result = OMX_UseBuffer(m_hMuxer,
                  &m_pOverlayBufferHdrs[i],
                  PORT_INDEX_OVERLAY, // port index
                  (void*)this,
                  m_pOverlayBuffers[i]->nAllocLen,
                  m_pOverlayBuffers[i]->pBuffer);
               if (result != OMX_ErrorNone)
               {
                  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "error using Overlay buffer");
                  break;
               }
            }
         }
  ...
  OMX_ERRORTYPE Muxer::FreeBuffers()
  {
    ...
    if(!m_hMuxer)
    {
     ...
     if (m_pOverlayBuffers)
     {
        for (i = 0; i < m_nOverlayBuffers; i++)
        {
           MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,"freeing Overlay buffer %d", i);
           ...
           if (result != OMX_ErrorNone)
           {
              MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,"error freeing Overlay buffer = %d", result);
              return result;
           }

        }

        delete [] m_pOverlayBufferHdrs;
        m_pOverlayBufferHdrs = NULL;
     }
     return result;
  }
  ...
  OMX_ERRORTYPE Muxer::Update(MuxerConfigType *pConfig)
  {
      ...
      //Check for codec property changes
      if(m_sConfig.svideoParams.eH264Profile != pConfig->svideoParams.eH264Profile ||
         m_sConfig.svideoParams.eH264Level != pConfig->svideoParams.eH264Level ||
         m_sConfig.svideoParams.height != pConfig->svideoParams.height ||
         m_sConfig.svideoParams.width != pConfig->svideoParams.width ||
         m_sConfig.svideoParams.format != pConfig->svideoParams.format)
      {
          if(m_eState == OMX_StateExecuting)
          {
              m_eState = OMX_StateIdle;
          ...
          if(UpdateVideoParams(pConfig) != OMX_ErrorNone)
          {
              return OMX_ErrorResourcesLost;
          }
          memcpy(&m_sConfig, pConfig, sizeof(m_sConfig));
  ...
  /////////////////////////////////////////////////////////////////////////////
  /////////////////////////////////////////////////////////////////////////////
  OMX_ERRORTYPE Muxer::UpdateVideoParams(MuxerConfigType *pVideoInfo)
  {
      OMX_ERRORTYPE result = OMX_ErrorNone;
      OMX_VIDEO_CODINGTYPE eVideoCoding   = OMX_VIDEO_CodingUnused;
      OMX_PORT_PARAM_TYPE portParam;
     ...
     if(!m_hMuxer)
     {
      ...
      if (result == OMX_ErrorNone)
      {
         ...
         // Video init parameters
         //////////////////////////////////////////
         switch(pVideoInfo->svideoParams.format)
         {
         case MUX_VIDEO_MPEG4:
            {
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM, "Video CODEC Mpeg 4 Settings");
               ...
               //Mpeg4VideoParam.bACPred = OMX_TRUE;
               Mpeg4VideoParam.eLevel = pVideoInfo->svideoParams.eMpeg4Level;
               Mpeg4VideoParam.eProfile =  pVideoInfo->svideoParams.eMpeg4Profile;
               //Mpeg4VideoParam.nTimeIncRes = 1000;
               Mpeg4VideoParam.nPortIndex = portParam.nStartPortNumber;
         ...
         case MUX_VIDEO_H264:
            {
               ...
               OMX_INIT_STRUCT(&AVCVideoParam, OMX_VIDEO_PARAM_AVCTYPE);
               AVCVideoParam.nPortIndex = portParam.nStartPortNumber;
               AVCVideoParam.eProfile = pVideoInfo->svideoParams.eH264Profile;
               AVCVideoParam.eLevel =  pVideoInfo->svideoParams.eH264Level;
         ...
         case MUX_VIDEO_H263:
            {
               ...
               OMX_INIT_STRUCT(&H263VideoParam, OMX_VIDEO_PARAM_H263TYPE);
               H263VideoParam.nPortIndex = portParam.nStartPortNumber;
               H263VideoParam.eProfile = pVideoInfo->svideoParams.eH263Profile;
               H263VideoParam.eLevel =  pVideoInfo->svideoParams.eH263Level;
      ...
      if(m_bVideoPortEnabled)
      {
          ...
          if(omxPortVideo.eDomain == OMX_PortDomainVideo)
          {
             m_nVideoBuffers = MAX((unsigned)m_nVideoBuffers, omxPortVideo.nBufferCountMin) ;
             MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM, "Num Video buffers potdef = %ld", omxPortVideo.nBufferCountMin);
             omxPortVideo.format.video.nFrameWidth = pVideoInfo->svideoParams.width;
             omxPortVideo.format.video.nFrameHeight = pVideoInfo->svideoParams.height;
             //omxPortVideo.format.video.nBitrate = pConfig->svideoParams.bitrate;
             //omxPortVideo.format.video.xFramerate = pConfig->svideoParams.frame_rate << 16;
             omxPortVideo.nBufferCountActual = m_nVideoBuffers;
             omxPortVideo.format.video.eCompressionFormat = eVideoCoding;
