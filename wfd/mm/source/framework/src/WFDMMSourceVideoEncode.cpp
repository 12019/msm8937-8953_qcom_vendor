...
#define ENCODER_PERF_MODE_POWER_SAVE 2
#define ENCODER_PRIORITY_REALTIME 0
#define ENCODER_PRIORITY_NON_REALTIME 1
...
(WFDMMSourceStatistics* pStats):
    m_pWFDMMSrcStats(pStats)
{
    initData();
              ...
              void* pAppData)
{
    ...
    result = configureCodecParam(pConfig);
    WFD_OMX_ERROR_CHECK(result,"Unable to configure encoder parameters");

    WFDMMLOGH("Go to Idle state...");
...
OMX_ERRORTYPE WFDMMSourceVideoEncode::configureCodecParam(
              VideoEncStaticConfigType* pConfig )
{
    OMX_ERRORTYPE result = OMX_ErrorNone;
    ...
    int nIRCyclicSupport  = -1;
    int nIRCyclicMicroBlocks  = -1;
    ...
    WFDMMLOGM2("Video Resoultion %ld, %ld", m_sConfig.nFrameWidth,
                                            m_sConfig.nFrameHeight);
    ...
    WFDMMLOGM2("Video Resoultion Out %ld, %ld", m_sConfig.nOutputFrameWidth,
                 m_sConfig.nOutputFrameHeight);
    ...
    */
    ...
    {
        WFDMMLOGH("Enabling Turbo Mode in encoder");
...
// TODO: Does not compile as it is
#if 1
    getCfgItem(CYCLIC_IR_KEY, &nIRCyclicSupport);

    if(nPeriodicIDRSupport == 0 && m_sConfig.nFramerate > 0
       && nIRCyclicSupport > 0)
    {
        /*----------------------------------------------------------------------
          Cyclic Intra refresh will be active if PeriodicIDRValid is not active
          and CyclicIntraRefresh is active. number of macro block refresh is
          depends on the value of CIRNumberOfMacroBlocks in configuration file
          otherwise default microblocks set to
        ...
        */
        ...
        getCfgItem(CYCLIC_IR_NUM_MACRO_BLK_KEY, &nIRCyclicMicroBlocks);
        if(nIRCyclicMicroBlocks > 0)
        {
            nMacroBlocks = nIRCyclicMicroBlocks;
        }
        sIntraReferesh.nPortIndex = m_nOutputPortIndex; //Output port
        sIntraReferesh.eRefreshMode = OMX_VIDEO_IntraRefreshCyclic;
        sIntraReferesh.nCirMBs = nMacroBlocks;
        WFDMMLOGE1("Setting cyclic Intra Refresh mode with %ld MBs",
    ...
    else if(nPeriodicIDRSupport == 0 && m_sConfig.nFramerate > 0)//Avoid DIV by 0 error
    {
        /*----------------------------------------------------------------------
    ...
    */
    ...
    /*-----------------------------------------------------------------
     Set encoder in Power save mode
    -----------------------------------------------------------------
    */

    QOMX_EXTNINDEX_VIDEO_PERFMODE perf_mode;
    OMX_INIT_STRUCT(&perf_mode,QOMX_EXTNINDEX_VIDEO_PERFMODE);
    perf_mode.nPerfMode = ENCODER_PERF_MODE_POWER_SAVE;
              /*1: MAX_QUALITY 2: POWER_SAVE*/
    WFDMMLOGH("Setting power save mode to encoder");
    result = OMX_SetConfig(m_hEncoder,
    ...
    /*-----------------------------------------------------------------
     Set Encoder priority in RT mode. When WFD moves to pausing state
      set encoder in NRT mode and set RT mode again while resuming.
      Intent here is to keep encoder in non real time mode when it is
      actually not running.
    ------------------------------------------------------------------
    */
    OMX_PARAM_U32TYPE enc_priority;
    OMX_INIT_STRUCT(&enc_priority, OMX_PARAM_U32TYPE);
    enc_priority.nU32 = ENCODER_PRIORITY_REALTIME;
    WFDMMLOGH("Setting Real time Priority for encoder");
    result = OMX_SetConfig(m_hEncoder,
    ...
    /*--------------------------------------------------------------------------
     Enabling low_latency mode to configure encoder in 1D1S mode. Going forward
     we will set this parameter by default to the encoder. It is upto firmware
     to decide whether to honour this request (and move to 1D1S) or discard it
     as a no-op (no error reporting to client) based on its capability.

     1D1S: Configures encoder in one dimensional, single-stage mode where it
     processes single YUV row at a time with certain encoder blocks running in
     sequential order. This results in corruption-free bitstream which was
     being observed in 2D2S mode due to greater number of skip macro blocks.
    ----------------------------------------------------------------------------
    */
    ...
    WFDMMLOGH("Enabled low latency mode");
...
/*=============================================================================
         ...
         DESCRIPTION:
*//**       @brief           reconfigure encoder copmponent
*//**
@par     DEPENDENCIES:
                            None
*//*
...
OMX_ERRORTYPE WFDMMSourceVideoEncode::reconfigure(VideoEncStaticConfigType* pConfig)
{
    if (!ISSTATECLOSED)
    {
        WFDMMLOGE("Reconfigure Encoder is not supported in this state");
        return OMX_ErrorInvalidState;
    }
    if (!pConfig)
    {
        WFDMMLOGE("Reconfigure Encoder Bad Param");
        return OMX_ErrorBadParameter;
    }
    WFDMMLOGE("Reconfigure Encoder to support DRC");
    ...
    result = GoToLoadedState();
    WFD_OMX_ERROR_CHECK(result,"Failed to go to loaded state");

    for(int i = 0; i < m_nNumOutputBuffers; i++)
    {
        result = m_pOutBufQ->Pop(&pBufferHdr,sizeof(pBufferHdr),(OMX_S32)100);
        WFD_OMX_ERROR_CHECK(result,"Failed to pop input buffers");
    }
    for(int i = 0; i < m_nNumInputBuffers; i++)
    {
        result = m_pInpBufQ->Pop(&pBufferHdr,sizeof(pBufferHdr),(OMX_S32)100);
        WFD_OMX_ERROR_CHECK(result,"Failed to pop outputbuffer");
    }
    SETSTATEINIT;
    if (m_hEncoder)
    {
        WFDMMLOGE("DRC Free encoder Handle...");
        OMX_FreeHandle(m_hEncoder);
        m_hEncoder = NULL;
    }
    WFDMMLOGE("DRC Creating encoder...");
    //////////////////////////////////////////
    ...
    result = configureCodecParam(pConfig);
    WFD_OMX_ERROR_CHECK(result,"Cannot configure encoder parameters");
    ...
    ----------------------------------------------------------------------------
    */
    result = GoToExecutingState();

    WFD_OMX_ERROR_CHECK(result,"Error in GoToExecutingState");
...
OMX_ERRORTYPE WFDMMSourceVideoEncode::createCaptureSource()
{
    ...
    */
    ...
    m_hVideoCapture = MM_New_Args(WFDMMSourceVideoCapture,(m_pWFDMMSrcStats));
...
OMX_ERRORTYPE WFDMMSourceVideoEncode::DeAllocateBuffers()
{
    ...
    for (i = 0; i < m_nNumOutputBuffers; i++)
    {
        if(m_pOutputBuffers[i])
        {
            ...
            tempExtra = static_cast<buff_hdr_extra_info*>
                        ((m_pOutputBuffers[i])->pAppPrivate);
            if(tempExtra)
            {
                MM_Free(tempExtra);
            }
      ...
      OMX_IN OMX_PTR pEventData)
{
    ...
    if (eEvent == OMX_EventCmdComplete)
    {
        if ((OMX_COMMANDTYPE) nData1 == OMX_CommandStateSet)
        {
            ...
            WFDMMLOGH2("Pushed cmd with %d and %d", cmd.eEvent,cmd.eCmd);
        }
        else if ((OMX_COMMANDTYPE) nData1 == OMX_CommandFlush)
        {
            WFDMMLOGM("Event callback with flush status");
            CmdType cmd;
            cmd.eEvent = OMX_EventCmdComplete;
            cmd.eCmd = OMX_CommandFlush;
            cmd.sCmdData = 0;
            cmd.eResult = result;
            WFDMMLOGH("Pushing OMX_EventCmdComplete OMX_CommandFlush");
            result = pMe->m_pCommandQ->Push(&cmd, sizeof(cmd));
        }
        else
        {
    ...
    {
        WFDMMLOGE2("async error: nData1 %lx, nData2 %lx", nData1, nData2);
        CmdType cmd;
        ...
        if(result == OMX_ErrorHardware || result == OMX_ErrorInsufficientResources)
        {
            WFDMMLOGE("Received hardware error / Insufficient resources");
    ...
    {
        WFDMMLOGM("Got buffer flag event");
    }
    else
    {
      ...
      OMX_BUFFERHEADERTYPE* pBufferHdr)
{
    ...
    WFDMMLOGM1("WFDMMSourceVideoEncode::DeliverInput with %p",pBufferHdr);
    ...
    WFDMMLOGL1("Difference in DeliverInputTS = %lld",
...
#ifdef SYSTRACE_PROFILE
    ATRACE_ASYNC_BEGIN("Encode frame", static_cast<int32_t>(pBufferHdr->nTimeStamp));
#endif
    ...
    if(WFD_LIKELY(m_pWFDMMSrcStats))
    {
        ...
        -------------------------------------------------------------------
        */
        m_pWFDMMSrcStats->recordEncIP(pBufferHdr);
    }
      ...
      OMX_IN OMX_BUFFERHEADERTYPE* pBufferHdr)
{
    ...
    WFDMMLOGM5(" EmptyDoneCallback nFlags = %lx, nAllocLen= %ld,\
      ...
      OMX_IN OMX_BUFFERHEADERTYPE* pBufferHdr)
{
    ...
    WFDMMLOGM5("FillDoneCallback:nFlags = %lx, nAllocLen= %ld,\
        ...
        pBufferHdr->nFlags,pBufferHdr->nAllocLen, pBufferHdr->nFilledLen,
        pBufferHdr->nOffset,pBufferHdr->nTimeStamp);
    WFDMMLOGL1("Difference in FillDoneTS = %lld",
...
#ifdef SYSTRACE_PROFILE
    ATRACE_ASYNC_END("Encode frame", static_cast<int32_t>(pBufferHdr->nTimeStamp));
#endif

    if(WFD_LIKELY(pMe->m_pWFDMMSrcStats))
    {
        pMe->m_pWFDMMSrcStats->recordEncOP(pBufferHdr);
    }
...
OMX_ERRORTYPE WFDMMSourceVideoEncode::Pause()
{
    ...
    /*--------------------------------------------------------------------------
     Set Encoder in NRT mode as it will not be active during pause
    ----------------------------------------------------------------------------
    */
    OMX_PARAM_U32TYPE enc_priority;
    OMX_INIT_STRUCT(&enc_priority, OMX_PARAM_U32TYPE);
    enc_priority.nU32 = ENCODER_PRIORITY_NON_REALTIME;
    WFDMMLOGH("Setting Non-Real time Priority for encoder while pausing");
    result = OMX_SetConfig(m_hEncoder,
               (OMX_INDEXTYPE)OMX_IndexConfigPriority,
               (OMX_PTR)&enc_priority);
    WFD_OMX_ERROR_CHECK(result, "Cannot set OMX_IndexConfigPriority");
...
@par     SIDE EFFECTS:
                            None
*//*==========================================================================*/
OMX_ERRORTYPE WFDMMSourceVideoEncode::Flush()
{
    OMX_ERRORTYPE result = OMX_SendCommand(m_hEncoder,
                             OMX_CommandFlush,
                             -1,
                             NULL);
...
OMX_ERRORTYPE WFDMMSourceVideoEncode::Resume()
{
    ...
    */
    for(int i = 0; i < m_nNumOutputBuffers; i++)
    {
        ...
        if(tempExtra)
            tempExtra->bPushed = OMX_FALSE;
        WFDMMLOGH1("Calling OMX_FillThisBuffer %p",pBufferHdr->pBuffer);
        eError = OMX_FillThisBuffer(m_hEncoder,pBufferHdr);
        WFD_OMX_ERROR_CHECK(eError,"Error in OMX_FillThisBuffer");
        mWfdVencStats.nFTB++;
    }
    ...
    */
    ...
    SETSTATEOPENED;
      ...
      /*----------------------------------------------------------------------
      Set Encoder in RT mode back while resuming
      ------------------------------------------------------------------------
      */
    OMX_PARAM_U32TYPE enc_priority;
    ...
    OMX_INIT_STRUCT(&enc_priority, OMX_PARAM_U32TYPE);
    enc_priority.nU32 = ENCODER_PRIORITY_REALTIME;
    WFDMMLOGH("Setting Real time Priority for encoder while resuming");
    result = OMX_SetConfig(m_hEncoder,
               (OMX_INDEXTYPE)OMX_IndexConfigPriority,
               (OMX_PTR)&enc_priority);
    WFD_OMX_ERROR_CHECK(result, "Cannot set OMX_IndexConfigPriority");
    ...
    */
    RequestIntraVOP();
    m_hVideoCapture->resumeCapture(&m_sConfig);
EXIT:
    return eError;
...
)
{
    ...
    tempExtra = static_cast<buff_hdr_extra_info*>(pBufferHdr->pAppPrivate);
