...
#undef LOG_TAG
#endif
#define LOG_TAG "WFDMMSRCTESTAPP"
...
int main (int argc, char *argv[])
{
  ...
  //Currently not supporting overlay for test app
  WFD_MM_Aux_capability_t *WFD_aux_capability = (WFD_MM_Aux_capability_t *) MM_Malloc(sizeof(WFD_MM_Aux_capability_t));
  if(WFD_aux_capability != NULL)
  {
    WFD_aux_capability->count = 0;
  }
  ...
  if( WFD_negotiated_capability != NULL)
  {
    ...
    wfd_mm_create_session(&handle, wfd_device_type, WFD_negotiated_capability,
                          WFD_aux_capability, &pCallBacks);
