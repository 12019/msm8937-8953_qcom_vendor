...
jbyteArray JNICALL Java_com_qualcomm_qti_securemsm_mdtp_ServerSigner_ASN1_MdtpMsgBuilder_nativeBuildSetStateMsg(
		JNIEnv *env, jobject obj, jboolean jActive, jboolean jFirmwareLockSelect, jboolean jSimLockSelect,jbyteArray jIdToken) {
	...
	if (SIGNER_OK == mdtp_build_set_state_msg(jActive, jFirmwareLockSelect, jSimLockSelect, &idToken_buf, &msg_buf)) {
...
jbyteArray JNICALL Java_com_qualcomm_qti_securemsm_mdtp_ServerSigner_ASN1_MdtpMsgBuilder_nativeBuildSetLockMsg(
		JNIEnv * env, jobject obj, jboolean jLock, jboolean jFirmwareLockSelect, jboolean jSimLockSelect, jbyteArray jIdToken) {
	mdtp_buf_t idToken_buf = {0,0};
	mdtp_buf_t msg_buf = {0,0};
	int res;
	jbyteArray jresponse = NULL;
	...
	if (SIGNER_OK == mdtp_build_set_lock_msg(jLock, jFirmwareLockSelect, jSimLockSelect, &idToken_buf, &msg_buf)) {
		jresponse = (*env)->NewByteArray(env, msg_buf.len);
		(*env)->SetByteArrayRegion(env, jresponse, 0, msg_buf.len, (jbyte*)msg_buf.data);
		free(msg_buf.data);
	}
