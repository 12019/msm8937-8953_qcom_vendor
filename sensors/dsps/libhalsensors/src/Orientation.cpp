...
void Orientation::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        la_sample.orientation.status =  msensor_status;
		
        HAL_LOG_ERROR( "%s:hzf sensor Msensor accuracy = %d, osensro accuracy =%d", __FUNCTION__,msensor_status , curr_sample->accuracy );
