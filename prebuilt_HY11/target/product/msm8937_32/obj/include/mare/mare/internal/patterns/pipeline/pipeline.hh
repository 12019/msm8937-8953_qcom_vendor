...
#include <mare/internal/patterns/pipeline/pipelineskeleton.hh>

namespace mare {
namespace internal {

template<mare::mem_order const mem_order>
struct comp_result_w_total_iters;

template<>
struct comp_result_w_total_iters<mare::mem_order_relaxed>  {
...
template<>
struct comp_result_w_total_iters<mare::mem_order_seq_cst>  {
  static void comp(size_t& result, size_t total_iters)  {
    if(total_iters > 0)
      result = result > total_iters ? total_iters : result;
  }
...
public:
  ...
  pipeline_stage_instance(pipeline_stage_skeleton_base* stage_ptr,
    ...
    _doc(stage_ptr->_doc),
    _id(stage_ptr->_id),
    _iter_ahead(stage_ptr->_doc == 1 ? _num_init_pipeline_tasks : stage_ptr->_doc),
    _lag(stage_ptr->_lag),
    _lag_succ(0),
  ...
  {
    MARE_INTERNAL_ASSERT(_iter_ahead != 0, "Stage degree of concurrency cannot be 0.");
  ...
  void apply(size_t in_first_idx,
    ...
    auto total_iters = _total_iters.load(mare::mem_order_relaxed);
  ...
  MARE_DELETE_METHOD(pipeline_stage_instance(pipeline_stage_instance const& other));
  MARE_DELETE_METHOD(pipeline_stage_instance(pipeline_stage_instance&& other));
  MARE_DELETE_METHOD(pipeline_stage_instance& operator=(pipeline_stage_instance const& other));
  MARE_DELETE_METHOD(pipeline_stage_instance& operator=(pipeline_stage_instance&& other));

  friend class pipeline_instance_base;
  friend class ::mare::pipeline_context_base;
};
...
#ifdef MARE_USE_PIPELINE_LOGGER
...
public:
...
#endif
  {
    ...
    _group = mare::create_group("pipieline instance.");
  };
...
MARE_GCC_IGNORE_END("-Weffc++");
  ...
  void perform_gpu_stage(
    ...
    auto cpu_tptr = mare::create_task(
      ...
      if(get_load_on_current_thread() == 0) {
         _active_tasks ++;
  #ifdef MARE_USE_PIPELINE_LOGGER
         _num_tasks.fetch_add(1, mare::mem_order_relaxed);
  #endif
  ...
  size_t gcd_stage_iter_rate(size_t i) {
    ...
    MARE_INTERNAL_ASSERT(a != 0, "Stage iter rate cannot be zero.");
    MARE_INTERNAL_ASSERT(b != 0, "Stage iter rate cannot be zero.");
  ...
  void get_predecessor_iter_info(size_t stage_id,
                                 ...
                                 mare::mem_order mem_order) const {
  ...
  void pipeline_task_aggressive_fetch(size_t               init_stage,
    ...
    while(1) {
      ...
      for(size_t i = first_search_stage; i > 0; i --) {
        ...
        MARE_INTERNAL_ASSERT(!curr_stage->is_parallel() || fetch_work <= curr_stage->_doc,
          "fetch too much work.");
      ...
      if(new_active_tasks == 0  && !_stages[_num_stages-1]->template is_finished<mem_order>()) {
        ...
        _active_tasks.store(1, mare::mem_order_relaxed);
  ...
  void launch_stb(size_t num_iterations,
    ...
    if(mem_order == mare::mem_order_relaxed)
...
#ifdef MARE_USE_PIPELINE_LOGGER
    MARE_ALOG("\033[31m *** creating %zu task, max %zu at one time *** \033[0m\n",
  ...
  MARE_DELETE_METHOD(pipeline_instance_base(pipeline_instance_base const& other));
  MARE_DELETE_METHOD(pipeline_instance_base(pipeline_instance_base&& other));
  MARE_DELETE_METHOD(pipeline_instance_base& operator=(pipeline_instance_base const& other));
  MARE_DELETE_METHOD(pipeline_instance_base& operator=(pipeline_instance_base&& other));
...
MARE_GCC_IGNORE_END("-Weffc++");
  ...
  mare_shared_ptr<pipeline_skeleton<>> _skeleton_shared_ptr;

public:

  explicit pipeline_instance(mare_shared_ptr<pipeline_skeleton<>> p):
  ...
  void launch(size_t num_iterations,
    ...
    if(launch_type == without_sliding_window) {
      MARE_API_ASSERT(!_skeleton->_has_sliding_window,
        "Cannot launch without sliding window because some stages are using sliding windows.");
    }

    if(num_iterations == 0) {
      _launch_with_iterations = false;
      launch_stb<mare::mem_order_seq_cst>(0, launch_type);
    }
    else {
      _launch_with_iterations = true;
      launch_stb<mare::mem_order_relaxed>(num_iterations, launch_type);
    }
  }
  ...
  MARE_DELETE_METHOD(pipeline_instance(pipeline_instance const& other));
  MARE_DELETE_METHOD(pipeline_instance(pipeline_instance&& other));
  MARE_DELETE_METHOD(pipeline_instance& operator=(pipeline_instance const& other));
  MARE_DELETE_METHOD(pipeline_instance& operator=(pipeline_instance&& other));
...
private:
  ...
  mare_shared_ptr<pipeline_skeleton<UserData>> _skeleton_shared_ptr;
  UserData*                                    _user_data;

public:

  explicit pipeline_instance(mare_shared_ptr<pipeline_skeleton<UserData>> p):
    pipeline_instance_base(static_cast<pipeline_skeleton_base*>(c_ptr(p))),
    _skeleton_shared_ptr(p),
    _user_data(nullptr) {}
