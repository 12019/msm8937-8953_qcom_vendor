...
#include <mare/internal/task/cputaskinternal.hh>
...
void blocking(BlockingFunction&& bf, CancelFunction&& cf)
{
  ...
  if(t != nullptr) {
    MARE_API_THROW(t->is_blocking(),
        "mare::blocking can only be called from within a blocking task or from outside a task");

    mare::internal::task_internal::blocking_code_container<BlockingFunction, CancelFunction>
...
private:
  ...
  virtual bool do_execute(task_bundle_dispatch* tbd = nullptr) {
    MARE_UNUSED(tbd);
    MARE_UNREACHABLE("Completed tasks cannot execute.");
    return false;
  }

  virtual void destroy_body_and_args(){
    MARE_UNREACHABLE("Cannot destroy the body of a completed task.");
  }

  MARE_DELETE_METHOD(value_cputask(value_cputask const&));
  MARE_DELETE_METHOD(value_cputask(value_cputask&&));
  MARE_DELETE_METHOD(value_cputask& operator=(value_cputask const&));
  MARE_DELETE_METHOD(value_cputask& operator=(value_cputask&&));
...
protected:
  ...
  template<typename Body>
  ::mare::task_ptr<ReturnType> prepare_args_but_do_not_store_retval(Body& b) {
    return b();
  }
...
class cputask_arg_layer<ReturnType(Arg1, Rest...)> : public cputask_return_layer<ReturnType> {
  ...
  void set_data_dependency_impl(cputask_return_layer<PredReturnType>* pred) {
    ...
    MARE_INTERNAL_ASSERT( pred != nullptr, "Unexpected null pointer.");
  ...
  struct bind_by_value {
    static void bind_impl(ArgType&& value, cputask_arg_layer<ReturnType(Arg1, Rest...)>* curr_task) {
      static_assert(is_by_value_t<ArgType&&>::value == true,
                    "Argument has to be bound by mare::by_value.");
  ...
  struct bind_by_data_dep {
    static void bind_impl(ArgType&& value, cputask_arg_layer<ReturnType(Arg1, Rest...)>* curr_task) {
      static_assert(is_by_data_dep_t<ArgType&&>::value == true,
                    "Argument has to be bound by mare::by_data_dependency.");
      ...
      static_assert(is_mare_task20_ptr<user_arg_nocvref>::has_return_value == true,
                    "Argument has to be a mare api20 task_ptr with return type information");
      using pred_return_type = typename is_mare_task20_ptr<user_arg_nocvref>::type;

      auto pred_ptr =
        static_cast<cputask_return_layer<pred_return_type>*>(::mare::internal::get_cptr(value._t));
  ...
  struct bind_as_data_dep {
    static void bind_impl(ArgType&& value, cputask_arg_layer<ReturnType(Arg1, Rest...)>* curr_task) {
      ...
      auto pred_ptr = static_cast<cputask_return_layer<pred_return_type>*>(::mare::internal::get_cptr(value));
      curr_task->set_data_dependency_impl<Pos, pred_return_type>(pred_ptr);
    }
  };
  ...
  void bind(ArgType&& value) {
    MARE_CONSTEXPR_CONST bool is_by_value    = is_by_value_t<ArgType&&>::value;
    MARE_CONSTEXPR_CONST bool is_by_data_dep = is_by_data_dep_t<ArgType&&>::value;
    ...
    MARE_CONSTEXPR_CONST bool can_as_value    = can_bind_as_value<user_arg, expected>::value;
    MARE_CONSTEXPR_CONST bool can_as_data_dep = can_bind_as_data_dep<user_arg, expected>::value;

    MARE_CONSTEXPR_CONST bool by_value    = is_by_value && can_as_value;
    MARE_CONSTEXPR_CONST bool by_data_dep = is_by_data_dep && can_as_data_dep;
    MARE_CONSTEXPR_CONST bool as_value    = !is_by_value && can_as_value;
    MARE_CONSTEXPR_CONST bool as_data_dep = !is_by_data_dep && can_as_data_dep;

    MARE_CONSTEXPR_CONST bool undecided   =
      (as_value && as_data_dep && !by_value && !by_data_dep) == true;
    MARE_CONSTEXPR_CONST bool error       =
      (as_value || as_data_dep || by_value || by_data_dep) == false;

    static_assert(undecided == false,
                  "Ambiguous argument binding. Please specifiy by using mare::by_value() or mare::by_data_dependency().");
  ...
  struct integer_sequence {
    ...
    static MARE_CONSTEXPR_CONST size_t get_size() {
  ...
  template<typename Body, uint32_t ...Number>
  ::mare::task_ptr<ReturnType> expand_args_but_do_not_store_retval(Body& b, integer_sequence<Number...>) {
  ...
  using buffer_acquire_set_t = buffer_acquire_set< num_buffer_ptrs_in_tuple<std::tuple<Arg1, Rest...>>::value >;
  ...
  void acquire_buffer(buffer_acquire_set_t& bas, BufferPtr& b, std::true_type, bool do_not_dispatch) {
    ...
    if(do_not_dispatch == true) {
      ...
      MARE_INTERNAL_ASSERT(acquired_arena != nullptr, "Error. Acquired arena is nullptr");
      auto mm_a = static_cast<mare::internal::mainmem_arena*>(acquired_arena);
      MARE_INTERNAL_ASSERT(mm_a != nullptr, "Error. Acquired main mem arena is nullptr");
      mm_a->acquire(executor_device::cpu);
      reinterpret_cast<mare::internal::buffer_ptr_base&>(b).allocate_host_accessible_data();
    }
  }
...
protected:
  ...
  template<typename Body>
  void prepare_args(Body& b) {
    MARE_INTERNAL_ASSERT(_set_arg_tracker.are_all_set() == true, "Arguments are not ready");
  ...
  buffer_acquire_set_t acquire_buffers() {
    ...
    bas.blocking_acquire_buffers(this,
                                 mare::internal::executor_device::cpu);
...
public:
  ...
  void bind_all(Arg1Type&& arg1, RestType&&...rest) {
    ...
    MARE_API_THROW(success, "Argument %d was already set", error_pos);
...
private:
  ...
  MARE_DELETE_METHOD(cputask(cputask const&));
  MARE_DELETE_METHOD(cputask(cputask&&));
  MARE_DELETE_METHOD(cputask& operator=(cputask const&));
  MARE_DELETE_METHOD(cputask& operator=(cputask&&));
};
