...
public:
  ...
  linkedlist_pool(size_t pool_size, size_t object_size, owner_pool_t* pool):
    ...
    _element_size(sizeof (element_header) + object_size),
    _owner_pool(pool),
    _total_deallocs(0),
    _buffer(nullptr),
    _next_pool(nullptr),
    ...
    _buffer = reinterpret_cast<char*>(mare_aligned_malloc(128, _pool_size * _element_size));
    if (_buffer == nullptr) {
      throw std::bad_alloc();
    }
    initialize_buffer();
  ...
  MARE_DELETE_METHOD(linkedlist_pool(linkedlist_pool const&));
  MARE_DELETE_METHOD(linkedlist_pool& operator=(linkedlist_pool const&));
  MARE_DELETE_METHOD(linkedlist_pool(linkedlist_pool const&&));
  MARE_DELETE_METHOD(linkedlist_pool& operator=(linkedlist_pool const&&));
