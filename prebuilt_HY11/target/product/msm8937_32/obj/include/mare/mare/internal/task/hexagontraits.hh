...
template <typename T>
struct is_buffer_ptr<::mare::buffer_ptr<T> > : public std::true_type {};
...
struct match_buffer_against_two_Params<TP, index, mare::buffer_ptr<T>>
{
...
template< typename Fn>
struct check<::mare::hexagon_kernel<Fn>>
{
};

template <typename Fn , typename ...Args>
struct check<::mare::hexagon_kernel<Fn>, Args...>
{
...
struct add_buffer_to_acquire_set<BufferAcquireSet, TP, tp_index, Fn, fn_index, true> {
  add_buffer_to_acquire_set(BufferAcquireSet& bas, TP& tp) {
    ...
    auto acquire_mode =  buffer_read_only ? mare::internal::bufferpolicy::acquire_r : mare::internal::bufferpolicy::acquire_rw;
...
template<typename T>
struct buffertraits<::mare::buffer_ptr<T>> {
...
struct translate_single_TP_arg_to_hexagon_tuple<Fn, fn_index, TP, index, BufferAcquireSet, true> {
  ...
  template<typename T>
  element_type * get_data(::mare::buffer_ptr<T> b, BufferAcquireSet& bas) {

    MARE_INTERNAL_ASSERT(bas.get_acquire_status() == true, "Buffers should be already acquired");

    auto acquired_arena = bas.find_acquired_arena(b);

    MARE_INTERNAL_ASSERT(acquired_arena != nullptr, "Invalid arena for buffer");
    MARE_INTERNAL_ASSERT(acquired_arena->get_type() == ::mare::internal::arena_t::ION_ARENA,
                         "Arenas for hexagon should be ION type");

    auto ion_arena = static_cast<mare::internal::ion_arena*>(acquired_arena);
    MARE_INTERNAL_ASSERT(ion_arena != nullptr, "Invalid ion arena for buffer");

    ion_arena->acquire(executor_device::hexagon);

    auto ion_arena_ptr = ion_arena->get_ptr();
    MARE_INTERNAL_ASSERT(ion_arena_ptr != nullptr, "ION arena should be not null");

    return static_cast<element_type *>(ion_arena_ptr);
  }

  template<typename T>
  size_t get_size(::mare::buffer_ptr<T> b) {
    return b.size();
  }
...
struct translate_TP_args_to_hexagon<Fn, fn_index, TP, index, BufferAcquireSet, false> {
  ...
  static bool constexpr is_index_arg_a_buffer_ptr =
                            ::mare::internal::is_buffer_ptr<element_type>::value;
