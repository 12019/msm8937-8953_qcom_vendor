  ...
  public:
    ...
    inline bool allocate(size_t& id, size_t pool_count) {
      while (true) {
        ...
        element_type current_bitmap = _bitmap.load(mare::mem_order_relaxed);

        size_t bit_number = mare_ffs(~current_bitmap);
        ...
        current_bitmap = _bitmap.fetch_or(mask, mare::mem_order_acq_rel);
    ...
    inline void deallocate(size_t id) {
      size_t bit_index = id;
      _bitmap.fetch_and(~(1 << bit_index), mare::mem_order_acq_rel);
    }

    element_type get_ids() const {
      return _bitmap.load(mare::mem_order_relaxed);
    }
...
public:
  ...
  explicit slab_allocator(size_t pool_byte_size = 1024, size_t pool_count = 10):
    ...
    _buffer_size(_pool_byte_size * _pool_count) {
      ...
      MARE_INTERNAL_ASSERT(pool_count <= pool_id_assigner::get_max_ids(),
                           "Allocating too many bump pools");
      MARE_INTERNAL_ASSERT(pool_count > 0,
  ...
  concurrent_bump_pool* allocate_pool(size_t ref_count = 0) {
    ...
    MARE_INTERNAL_ASSERT(false, "No pool left!");
    return nullptr;
  }
  ...
  MARE_DELETE_METHOD(slab_allocator(slab_allocator const&));
  MARE_DELETE_METHOD(slab_allocator& operator=(slab_allocator const&));
  MARE_DELETE_METHOD(slab_allocator(slab_allocator const&&));
  MARE_DELETE_METHOD(slab_allocator& operator=(slab_allocator const&&));
...
concurrent_bump_pool::deallocate_object_impl(char* object) {
  ...
  size_t ref_count = _ref_count.fetch_sub(1, mare::mem_order_relaxed);
  if (ref_count == 1) {
    _pointer = _buffer;
...
concurrent_bump_pool::deallocate() {
  if (!_concurrent)
    MARE_FATAL("An RCed pool must not be deallocated explicitly!");
  _pointer = _buffer;
  if (_allocator != nullptr)
    _allocator->deallocate_pool(this);
}
