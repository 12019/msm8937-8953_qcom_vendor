...
void pdivide_and_conquer_internal(group_ptr g,
                                  ...
                                  Fn4&& merge,
                                  bool is_base_n) {
  MARE_INTERNAL_ASSERT(n->_subproblems.empty(),
      "work node should have no subproblems just yet");
  if (is_base_n) {
    MARE_INTERNAL_ASSERT(c == nullptr, "if base, continuation should be null");
    ...
    for (auto i : split(n->_problem)) {
      ...
      auto cont_child = is_base_i ? nullptr : mare::create_task([=]() {

        MARE_INTERNAL_ASSERT(!node_i->_subproblems.empty(),
      ...
      auto task_child = mare::create_task([=] {
  ...
  if (is_base_p) {
    ...
    for (auto i : split(p)) {
      ...
      auto cont_child = is_base_i ?
        nullptr : mare::create_task([=]{merge(i);});
...
void pdivide_and_conquer_internal(group_ptr g,
                                  ...
                                  Fn3&& split,
                                  bool is_base_p) {
  MARE_INTERNAL_ASSERT(g != nullptr, "group must not be null");
  if (is_base_p) {
    base(p);
  } else {
    for (auto i : split(p)) {
...
template <typename Problem, typename Solution, typename Fn1,
         typename Fn2, typename Fn3, typename Fn4>
mare::task_ptr<Solution> pdivide_and_conquer(group_ptr g, Problem p, Fn1&& is_base,
    Fn2&& base, Fn3&& split, Fn4&& merge) {
  if (is_base(p)) {
    return mare::create_value_task<Solution>(base(p));
  } else {

    auto n = std::make_shared<internal::dnc_node<Problem, Solution>>(p);
    auto cont = mare::create_task([n, p, merge] {
...
void pdivide_and_conquer(group_ptr g, Problem p, Fn1&& is_base,
  ...
  MARE_INTERNAL_ASSERT(g == nullptr || async == true,
      "g should be valid only for async invocations");
  if (is_base(p)) {
    base(p);
  } else {
    auto c = mare::create_task([merge, p] {merge(p);});
...
template <typename Problem, typename Solution, typename Fn1,
         typename Fn2, typename Fn3, typename Fn4>
mare::task_ptr<Solution> pdivide_and_conquer_async(Fn1&& is_base,
    Fn2&& base, Fn3&& split, Fn4&& merge, Problem p) {
  auto g = create_group();
  auto t = mare::create_task([g, p, is_base, base, split, merge]{
      return mare::internal::pdivide_and_conquer<Problem, Solution>(g, p, is_base, base, split, merge);
  });
...
template <typename Problem, typename Fn1, typename Fn2,
         typename Fn3, typename Fn4>
mare::task_ptr<> pdivide_and_conquer_async(Fn1&& is_base,
    Fn2&& base, Fn3&& split, Fn4&& merge, Problem p) {
  auto g = mare::create_group();
  auto t = mare::create_task([g, p, is_base, base, split, merge]{
      mare::internal::pdivide_and_conquer(g, p, is_base, base, split, merge, true);
  });
  auto gptr = internal::c_ptr(g);
  gptr->set_representative_task(internal::c_ptr(t));
  return t;
...
pdivide_and_conquer_async(Fn1&& is_base, Fn2&& base, Fn3&& split, Problem p) {
  ...
  auto g = mare::create_group();
  auto t = mare::create_task([g, p, is_base, base, split]{
      mare::internal::pdivide_and_conquer(g, p, is_base,
        base, split, true);
  });
  auto gptr = internal::c_ptr(g);
  gptr->set_representative_task(internal::c_ptr(t));
