...
#include <mare/internal/log/eventids.hh>
#include <mare/internal/log/eventcontext.hh>
...
public:
  ...
  static bool remove_one_for_event(handler_id id) {
    auto list = get_user_handler_list<Event>();
    if (!list) {
      MARE_ALOG ("Invalid event handler id %d\n", id);
      return false;
    }
...
public:
  ...
  bool unregister_handler(handler_id id) {
    ...
    MARE_ALOG ("Invalid event handler id.\n");
    return false;
  }
...
private:
  ...
  MARE_DELETE_METHOD(user_handler_list(user_handler_list const&));
  MARE_DELETE_METHOD(user_handler_list(user_handler_list&&));
  MARE_DELETE_METHOD(user_handler_list& operator=(user_handler_list const&));
  MARE_DELETE_METHOD(user_handler_list& operator=(user_handler_list&&));
};
