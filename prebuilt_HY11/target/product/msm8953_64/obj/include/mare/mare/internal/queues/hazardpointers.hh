...
public:
  ...
  void scan(Record* headHPRecord) {
    ...
    MARE_INTERNAL_ASSERT(headHPRecord->nrThreads()>0, "nrThreads should be > 0");
#ifdef _MSC_VER
  ...
  bool tryClaim() {
    ...
    if(b) {
      ...
      __sync_synchronize();
      if(_owner != 0) {
        MARE_FATAL("Thread got claim to already claimed record: %" MARE_FMT_TID, _owner);
      }
      _owner = thread_id();
    }
#endif
...
#ifdef MARE_DEBUG
    if(_owner != thread_id()) {
      MARE_FATAL("FATAL: Thread released record that is not his: %" MARE_FMT_TID "/%" MARE_FMT_TID
                , _owner, thread_id()
                );
    }
    _owner = 0;
...
public:
  ...
  void threadInit() {
    MARE_INTERNAL_ASSERT(!_myhprec, "Please call threadInit() only on an uninitialized thread");
  ...
  void threadFinish() {
    MARE_INTERNAL_ASSERT(_myhprec, "Please call threadInit() on each thread using hp::Manager");
    _myhprec->retire();
    _myhprec = nullptr;
  }
...
private:
  ...
  void retireNode(tlsptr<Record<TN,K>>& myhprec, TN* node) {
    MARE_INTERNAL_ASSERT(myhprec, "Please call threadInit() on each thread using hp::Manager");
    myhprec->push(node);
