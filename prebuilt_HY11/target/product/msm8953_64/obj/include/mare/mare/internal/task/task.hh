...
#include <mare/internal/storage/taskstorage.hh>
#include <mare/internal/task/exception_state.hh>
#include <mare/internal/task/task_misc.hh>
...
private:
  ...
  using exception_state_ptr = mare::internal::exception_state::exception_state_ptr;
  using blocking_code_container_base = mare::internal::task_internal::blocking_code_container_base;
...
public:
  ...
  bool is_blocking() const {
    return has_attr(_attrs, mare::internal::legacy::attr::blocking);
  }

  bool is_big() const {
    return has_attr(_attrs, mare::internal::legacy::attr::big);
  }

  bool is_little() const {
    return has_attr(_attrs, mare::internal::legacy::attr::little);
  }
  ...
  bool is_gpu() const {
    return has_attr(_attrs, mare::internal::legacy::attr::gpu);
  }

  bool is_cpu() const {
    return has_attr(_attrs, mare::internal::legacy::attr::cpu);
  }

  bool
  is_hexagon() const {
    return has_attr(_attrs, mare::internal::legacy::attr::hexagon);
  }
  ...
  bool request_ownership(scheduler& sched) {
    ...
    return std::atomic_compare_exchange_strong_explicit(
                                    ...
                                    &expected,
                                    &sched,
                                    mare::mem_order_acq_rel,
                                    mare::mem_order_relaxed);
  }

  void relinquish_scheduler() {
    _scheduler.store(nullptr, mare::mem_order_release);
  }
  ...
  state_snapshot get_snapshot(mare::mem_order order =
                                mare::mem_order_acquire) const {
  ...
  scheduler* get_scheduler(mare::mem_order order) {
  ...
  group* get_current_group(mare::mem_order mem_order =
                           mare::mem_order_acquire) const {
  ...
  std::vector<task*>& get_alternatives() {
    MARE_INTERNAL_ASSERT(is_poly(), "must call get_alternatives only on a poly task");
    return *_alternatives;
  }

  task* get_alternative(size_t variant) {
    MARE_INTERNAL_ASSERT(variant > 0, "variants start from 1");
    MARE_INTERNAL_ASSERT(_alternatives->size() >= variant,
  ...
  inline size_t get_suitable_alternative(::mare::internal::task_domain td);
...
private:
  ...
  void set_current_group_unlocked(group* g) {
    ...
    _current_group.store(g, mare::mem_order_relaxed);
  }
  ...
  void set_scheduler(scheduler& sched) {
    ...
    _scheduler.store(&sched, mare::mem_order_relaxed);
  }

  group* move_current_group_unlocked(mare::mem_order mem_order =
                                     mare::mem_order_acquire) {
    auto g = get_current_group(mem_order);
    set_current_group_unlocked(nullptr);
    MARE_INTERNAL_ASSERT(get_current_group() == nullptr,
  ...
  bool is_task_executable_in_domain(::mare::internal::task_domain td);
  ...
  void throw_exceptions() {
    MARE_INTERNAL_ASSERT(is_exceptional(), "must be called only if exceptional");
    auto guard = get_lock_guard();
    _exception_state->rethrow();
    MARE_UNREACHABLE("exception should have been thrown");
  }

  void set_exception_unlocked(std::exception_ptr& eptr) {
    MARE_INTERNAL_ASSERT(eptr != nullptr, "cannot set empty exception");
    if (_exception_state == nullptr) {
  ...
  void set_finish_after_state(finish_after_state_ptr& tfa, StubTask const& s) {
    MARE_INTERNAL_ASSERT((tfa == nullptr),
  ...
  friend class mare_shared_ptr<task>;
...
task::unref() {
  ...
  MARE_INTERNAL_ASSERT(snapshot.is_ready_running_or_finished(is_cancelable()) ||
...
task::execute_sync(task_bundle_dispatch* tbd, size_t variant)
{
  ...
  std::atomic_thread_fence(mare::mem_order_acquire);
  execute(tbd, variant);
}

inline void
task::execute_trigger_task(bool g_was_canceled) {
  MARE_INTERNAL_ASSERT(is_trigger(), "Task %p is not a trigger task: %s",
                       this,
                       to_string().c_str());

  MARE_INTERNAL_ASSERT(!get_snapshot().is_launched(),
                       "Can't set trigger task %p to COMPLETED because it's"
                       "already launched: %s",
                       this,
                       to_string().c_str());

  MARE_INTERNAL_ASSERT(!get_snapshot().is_finished(),
                       "Can't set trigger task %p to COMPLETED because it's"
                       "already launched: %s",
                       this,
                       to_string().c_str());

  MARE_INTERNAL_ASSERT(!is_cancelable(),
...
{
  MARE_INTERNAL_ASSERT(succ != nullptr, "null task_ptr succ.");

  MARE_API_THROW(!succ->get_snapshot().is_launched(),
  ...
  MARE_INTERNAL_ASSERT(succ->get_snapshot().is_launched() == false,
...
{
  MARE_INTERNAL_ASSERT(succ != nullptr, "null task_ptr succ.");
  MARE_INTERNAL_ASSERT(succ->get_snapshot().is_launched(),
      "Successor task %p should already be launched: %s",
      succ,
      succ->to_string().c_str());
  MARE_INTERNAL_ASSERT(succ->get_snapshot().is_ready_running_or_finished(succ->is_cancelable()),
  ...
  MARE_INTERNAL_ASSERT(get_snapshot().is_launched(),
      ...
      this,
      to_string().c_str());
  MARE_INTERNAL_ASSERT(get_snapshot().is_ready_running_or_finished(succ->is_cancelable()),
...
current_scheduler() {
  ...
  if (auto t = current_task())
    return t->get_scheduler(mare::mem_order_relaxed);
  return nullptr;
}
...
current_task_and_scheduler() {
  ...
  if (auto t = current_task())
    return std::make_tuple(t, t->get_scheduler(mare::mem_order_relaxed));
  return std::make_tuple<task*, scheduler*>(nullptr, nullptr);
}

inline bool
task::is_task_executable_in_domain(::mare::internal::task_domain td)
{
...
size_t
task::get_suitable_alternative(::mare::internal::task_domain td) {
  MARE_INTERNAL_ASSERT(is_poly(), "can be called only for a poly task");
  ...
  MARE_INTERNAL_ASSERT(alt != alternatives.end(),
...
task::finish_after_stub_task_body(task* t)
{
  ...
  MARE_INTERNAL_ASSERT(sched != nullptr,
...
yield()
{
  if (auto task = current_task()) {
    ...
    MARE_INTERNAL_ASSERT(t_ptr != nullptr, "null task_ptr");
    t_ptr->yield();
  }
}
...
release_manager::release(task* t) {
  MARE_INTERNAL_ASSERT(debug_is_ok_to_release(t) == true, "Cannot release task t");
#if !defined(MARE_NO_TASK_ALLOCATOR)
