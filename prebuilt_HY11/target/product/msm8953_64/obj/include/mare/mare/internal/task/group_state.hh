...
public:
  ...
  bool is_leaf() const {
    MARE_INTERNAL_ASSERT(_state != UNINITIALIZED_STATE,
...
class group_state : public group_state_base {
  ...
  friend class mare::internal::group;
  friend class ::GroupStateSuite;
...
public:
  ...
  group_state_snapshot get_snapshot(mare::mem_order order =
                                   mare::mem_order_relaxed) const {
  ...
  bool set_cancel(group_state_snapshot& state){
    ...
    if (state.is_canceled())
      return false;
    state = _state.fetch_or(canceled, mare::mem_order_acq_rel);
  ...
  group_state_snapshot lock() {
    ...
    while(true) {
      ...
      do{
        prior_state = get_snapshot(mare::mem_order_relaxed);
    ...
    MARE_UNREACHABLE("lock returns only if successful");
  }

  group_state_snapshot unlock() {

    group_state_snapshot state(get_snapshot(mare::mem_order_relaxed));

    while(true){

      MARE_INTERNAL_ASSERT(state.is_locked(),
