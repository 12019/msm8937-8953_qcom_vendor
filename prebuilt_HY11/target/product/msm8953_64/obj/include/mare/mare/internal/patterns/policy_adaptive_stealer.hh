...
#include <mare/internal/patterns/policy_adaptive_worker.hh>
...
{
  static void stealer_task_body(mare_shared_ptr<Strategy> stg_ptr, size_t task_id){
...
struct stealer_wrapper<T, Strategy, Policy::SCAN>
{
  ...
  static void stealer_task_body(mare_shared_ptr<Strategy> stg_ptr,
                                size_t task_id)
  {
    MARE_INTERNAL_ASSERT(current_task() != nullptr,
        "current task pointer unexpectedly invalid");
    ...
    if (task_id == 0) {
      ...
      stg_ptr->set_local_sum_updated(mare::mem_order_release);
    }

    while(!stg_ptr->is_local_sum_updated(mare::mem_order_acquire)) {}
