...
#include <mare/internal/legacy/taskfactory.hh>
...
{
  auto t_ptr = ::mare::internal::c_ptr(task);
  MARE_API_ASSERT(t_ptr, "null task_shared_ptr");
  t_ptr->wait();
}
...
{
  auto pred_ptr = ::mare::internal::c_ptr(pred);
  auto succ_ptr = ::mare::internal::c_ptr(succ);

  MARE_API_ASSERT(pred_ptr != nullptr, "null task_shared_ptr");
  MARE_API_ASSERT(succ_ptr != nullptr, "null task_shared_ptr");
...
{
  ::mare::internal::legacy::launch_dispatch(task);
}

template<typename Body>
inline void launch(internal::group_shared_ptr const& a_group, Body&& body, bool notify = true)
{
  ::mare::internal::legacy::launch_dispatch<Body>(a_group, std::forward<Body>(body), notify);
}

};

template<typename F>
static ::mare::internal::task_shared_ptr create_stub_task(F f, task* pred = nullptr) {
  auto attrs = legacy::create_task_attrs(attr::stub, attr::non_cancelable);
  if (pred == nullptr)
    attrs = ::mare::internal::legacy::add_attr(attrs, attr::yield);

  auto stub = ::mare::internal::legacy::create_task(with_attrs(attrs, f));
  if (pred)
...
inline task_shared_ptr create_trigger_task() {
  ...
  return task_shared_ptr(t, mare::internal::task_shared_ptr::ref_policy::no_initial_ref);
}
