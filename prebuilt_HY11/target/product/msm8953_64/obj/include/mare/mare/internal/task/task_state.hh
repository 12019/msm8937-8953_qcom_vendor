...
class state_snapshot : public state_base {
  ...
  friend class ::mare::internal::task_internal::state;
...
public:
  ...
  bool is_finished() const {
    MARE_INTERNAL_ASSERT(_snapshot != value::uninitialized,
...
private:
  ...
  state_snapshot set_finished_and_lock_impl(raw_type finished_state,
    ...
    MARE_INTERNAL_ASSERT(finished_state == completed || finished_state == canceled,
    ...
    while(true) {
      ...
      do {
        snapshot = get_snapshot(mare::mem_order_relaxed);
  ...
  state_snapshot set_finished_and_unref_and_unlock(bool must_cancel, task* t) {
    ...
    while(true) {
      ...
      MARE_INTERNAL_ASSERT(snapshot.is_locked(),
...
public:
  ...
  state_snapshot get_snapshot(mare::mem_order order = mare::mem_order_acquire) const {
    return state_snapshot(_state.load(order));
  }

  void add_predecessor(task* t, bool dynamic_dep = false) {

    state_snapshot snapshot (get_snapshot(mare::mem_order_relaxed));

    while(true) {

      if(((dynamic_dep == false) && (snapshot.is_launched() == true))
          || (snapshot.is_finished() == true)) {
        MARE_FATAL("Can't add predecessors to task %p State: %s",
                    t,
                    get_task_description(t).c_str());
      }
      ...
      if (atomic_compare_exchange_weak_explicit(&_state,
        ...
        if(snapshot.get_num_predecessors() == pred_cntr_max) {
          MARE_FATAL("Task has reached the maximum number of predecessors");
        }
        return;
      }
  ...
  void remove_predecessor(state_snapshot& updated_state) {
    updated_state.reset(_state.fetch_sub(one_pred, mare::mem_order_acq_rel) - one_pred);
  }
  ...
  bool set_launched(state_snapshot& snapshot, bool increase_ref_count) {
    ...
    while (true) {
      ...
      if ((snapshot.is_launched()) || (snapshot.is_bound() == false)) {
        snapshot.reset(get_snapshot(mare::mem_order_acquire).get_raw_value());
        return false;
      }
  ...
  bool set_running(bool is_anonymous, bool is_cancelable, task *t) {
    ...
    MARE_INTERNAL_ASSERT(get_snapshot().get_num_predecessors() == 0,
                         "Can't transition task %p to running because it has "
                         "predecessors: %s.",
                         t,
                         get_task_description(t).c_str());

    MARE_INTERNAL_ASSERT(get_snapshot().is_bound() == true,
    ...
    if (is_anonymous) {
      MARE_INTERNAL_ASSERT(get_snapshot().get_raw_value() == 1,
                           "Anonymous task %p has unexpected state: %s ",
                           t,
                           get_task_description(t).c_str());
      _state.store(running_anonymous, mare::mem_order_relaxed);
      return true;
    }
  ...
  state_snapshot set_in_utcache(task* t) {
    MARE_INTERNAL_ASSERT(get_snapshot().is_finished() == false,
    ...
    MARE_UNUSED(t);
    return state_snapshot(_state.fetch_or(in_utcache, mare::mem_order_acq_rel));
  }

  void reset_in_utcache() {
    _state.fetch_and(in_utcache_mask, mare::mem_order_acq_rel);
  }

  void set_bound(task* t) {
    MARE_INTERNAL_ASSERT(get_snapshot().is_bound() == false,
                         "Task %p is already bound: %s",
                         t,
                         get_task_description(t).c_str());

    MARE_UNUSED(t);
    _state.fetch_and(unbound_mask, mare::mem_order_acq_rel);
  }

  bool set_cancel_request(state_snapshot& snapshot) {
    snapshot.reset(_state.fetch_or(cancel_req, mare::mem_order_acq_rel));
  ...
  state_snapshot ref() {
    auto old_value = _state.fetch_add(one_ref, mare::mem_order_acq_rel);
    MARE_INTERNAL_ASSERT(old_value != ref_cntr_max,
                         "Maximum number of references exceeded.");
    return state_snapshot(old_value);
  }

  state_snapshot unref() {
    auto old_value = _state.fetch_sub(one_ref, mare::mem_order_acq_rel);
    MARE_INTERNAL_ASSERT(old_value != 0, "Can't unref task with no references.");
    return state_snapshot(old_value);
  }
