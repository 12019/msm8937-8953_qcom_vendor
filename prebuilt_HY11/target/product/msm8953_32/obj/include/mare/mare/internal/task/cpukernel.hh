...
public:
  ...
  using collapsed_task_type = ::mare::task_ptr<collapsed_signature>;
  using non_collapsed_task_type = ::mare::task_ptr<non_collapsed_signature>;
  ...
  static MARE_CONSTEXPR_CONST size_type arity =user_code_traits::arity::value;

  explicit cpu_kernel(UserCode&& user_code) :
    _user_code(std::forward<UserCode>(user_code)),
    _attrs(legacy::create_task_attrs(::mare::internal::legacy::attr::cpu)) {
  ...
  cpu_kernel& operator=(cpu_kernel&& other) {
    ...
    _user_code = std::move(other._user_code);
    _attrs = other._attrs;
    other._attrs = legacy::create_task_attrs(::mare::internal::legacy::attr::cpu);
    return *this;
  }

  template<typename Attribute>
  void set_attr(Attribute const& attr) {
    _attrs = ::mare::internal::legacy::add_attr(_attrs, attr);
  }

  template<typename Attribute>
  bool has_attr(Attribute const& attr) const {
    return ::mare::internal::legacy::has_attr(_attrs, attr);
  }
  ...
  ::mare::internal::legacy::task_attrs get_attrs() {
    return _attrs;
  }

private:
  user_code_type_in_task _user_code;
  ::mare::internal::legacy::task_attrs _attrs;
};
