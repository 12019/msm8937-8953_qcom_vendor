...
#include <mare/internal/util/index.hh>
...
public:
  ...
  inline size_t num_elems(size_t i) const {
    MARE_INTERNAL_ASSERT(_e[i] - _b[i], "begin == end");
...
public:
  ...
  range(size_t b0, size_t e0, size_t s0) {
    MARE_API_ASSERT(((b0 <= e0) && (s0 > 0)), "Invalid range parameters");
    _b[0] = b0;
    _e[0] = e0;
    _s[0] = s0;
  }
  ...
  void print() const {
    MARE_ALOG("(begin:end:stride) [%zu:%zu:%zu]", _b[0], _e[0], _s[0]);
  }
};
...
public:
  ...
  range(size_t b0, size_t e0, size_t s0, size_t b1, size_t e1, size_t s1) {
    MARE_API_ASSERT(((b0 <= e0) && (b1 <= e1) && (s0 > 0) && (s1 > 0)),
    ...
    MARE_API_ASSERT(((num_elems(0) > 0) && (num_elems(1) > 0)),
                    "Invalid range parameters");
  }
  ...
  void print() const {
    MARE_ALOG("(begin:end:stride) [%zu:%zu:%zu], [%zu:%zu:%zu]",
...
public:
  ...
  void print() const {
    MARE_ALOG("(begin:end:stride) [%zu:%zu:%zu], [%zu:%zu:%zu], [%zu:%zu:%zu]",
