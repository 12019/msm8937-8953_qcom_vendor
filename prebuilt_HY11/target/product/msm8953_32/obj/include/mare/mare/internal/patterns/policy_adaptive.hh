...
#include <mare/internal/patterns/workstealtree/wstree.hh>
...
public:
  ...
  adaptive_strategy_base(internal::group_shared_ptr g,
                         Fn&& f,
...
private:
  ...
  MARE_DELETE_METHOD(adaptive_strategy_base(adaptive_strategy_base const&));
  MARE_DELETE_METHOD(adaptive_strategy_base(adaptive_strategy_base&&));
  MARE_DELETE_METHOD(adaptive_strategy_base&
...
private:
  ...
  MARE_DELETE_METHOD(tree_ops_base(tree_ops_base const&));
  MARE_DELETE_METHOD(tree_ops_base(tree_ops_base&&));
  MARE_DELETE_METHOD(tree_ops_base& operator=(tree_ops_base const&));
  MARE_DELETE_METHOD(tree_ops_base& operator=(tree_ops_base&&));
};
...
public:
  ...
  adaptive_steal_strategy(internal::group_shared_ptr g,
                          size_type first,
                          size_type last,
                          const T& identity,
                          Fn&& f,
...
private:
  ...
  MARE_DELETE_METHOD(adaptive_steal_strategy
...
public:
  ...
  void inc_task_counter(mare::mem_order order = mare::mem_order_seq_cst) {
    _task_counter.fetch_add(1, order);
  }

  uint8_t get_task_counter(mare::mem_order order = mare::mem_order_seq_cst) {
    return _task_counter.load(order);
  }

  void set_local_sum_updated(mare::mem_order order = mare::mem_order_seq_cst) {
    _lsum_update_done.store(true, order);
  }

  bool is_local_sum_updated(mare::mem_order order = mare::mem_order_seq_cst) {
