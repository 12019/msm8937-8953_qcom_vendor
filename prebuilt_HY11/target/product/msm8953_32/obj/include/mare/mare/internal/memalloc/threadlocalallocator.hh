...
#include <mare/internal/memalloc/defaulttaskallocator.hh>
#include <mare/internal/memalloc/linkedlistpool.hh>
...
class thread_local_fixed_pool_set: public pool {
  ...
  public:
    ...
    MARE_DELETE_METHOD(spin_lock(spin_lock &));
    MARE_DELETE_METHOD(spin_lock(spin_lock &&));
    MARE_DELETE_METHOD(spin_lock& operator=(spin_lock const &));
    MARE_DELETE_METHOD(spin_lock& operator=(spin_lock&&));
    ...
    void lock() {
      ...
      while(acquired == false) {
        ...
        while(_held.load(mare::mem_order_relaxed) == true){}

        bool old_value = false;
        acquired = _held.compare_exchange_strong(old_value, true,  mare::mem_order_acquire);
      }
    }

    void unlock(){
      _held.store(false, mare::mem_order_release);
    }
  };
...
public:
  ...
  void remove_pool(linkedlist_pool* old_pool) {
    MARE_INTERNAL_ASSERT(old_pool != &_first_pool,
  ...
  void add_pool(linkedlist_pool* new_pool) {
    MARE_INTERNAL_ASSERT(new_pool->_next_pool == nullptr, "New pool not initialized properly!");
...
private:
  ...
  MARE_DELETE_METHOD(thread_local_fixed_pool_set(thread_local_fixed_pool_set const&));
  MARE_DELETE_METHOD(thread_local_fixed_pool_set&
...
public:
  ...
  MARE_DELETE_METHOD(thread_local_allocator(thread_local_allocator const&));
  MARE_DELETE_METHOD(thread_local_allocator&
