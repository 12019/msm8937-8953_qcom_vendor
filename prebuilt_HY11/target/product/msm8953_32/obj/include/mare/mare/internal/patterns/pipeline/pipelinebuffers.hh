...
#include <mare/internal/memalloc/pipelinelinearpool.hh>
#include <mare/internal/memalloc/pipelineslaballocator.hh>
#include <mare/internal/patterns/pipeline/pipelinepoolbucket.hh>
...
public:
  ...
  MARE_DELETE_METHOD(stagebuffer(stagebuffer const& other));
  MARE_DELETE_METHOD(stagebuffer(stagebuffer&& other));
  MARE_DELETE_METHOD(stagebuffer& operator=(stagebuffer const& other));
  MARE_DELETE_METHOD(stagebuffer& operator=(stagebuffer&& other));
...
public:
  ...
  virtual void allocate_pool() {
    MARE_UNREACHABLE("Cannot allocate pool for ringbuffer.");
  }

  T& operator[](size_t id) {
    MARE_INTERNAL_ASSERT(id < _size, "Out of range buffer access.");
    return _buffer[id];
  };
  ...
  MARE_DELETE_METHOD(ringbuffer(ringbuffer const& other));
  MARE_DELETE_METHOD(ringbuffer(ringbuffer&& other));
  MARE_DELETE_METHOD(ringbuffer& operator=(ringbuffer const& other));
  MARE_DELETE_METHOD(ringbuffer& operator=(ringbuffer&& other));
};
...
class dynamicbuffer:public stagebuffer{
 ...
 static mare::mem_order const pool_mem_order = mare::mem_order_relaxed;
...
private:
  static MARE_CONSTEXPR_CONST size_type s_bucket_size = 64;
#ifdef MARE_PIPELINE_DEBUG_BUFFER
  size_t                                                  _num_alloc;
  size_t                                                  _num_dalloc;
  size_t                                                  _num_reset;
#endif
...
public:
...
#ifdef MARE_PIPELINE_DEBUG_BUFFER
   MARE_ALOG("DYNAMIC BUFFER %p, %zu out of %zu alloc from the slab, %zu resets",
...
#if defined(MARE_PIPELINE_DEBUG_BUFFER) && defined(DEBUG_PIPELINE)
        MARE_ALOG(
  ...
  virtual void reset_pool() {
    ...
    MARE_INTERNAL_ASSERT(_pool_buckets[bucket_id] != nullptr,
  ...
  MARE_DELETE_METHOD(dynamicbuffer(dynamicbuffer const& other));
  MARE_DELETE_METHOD(dynamicbuffer(dynamicbuffer&& other));
  MARE_DELETE_METHOD(dynamicbuffer& operator=(dynamicbuffer const& other));
  MARE_DELETE_METHOD(dynamicbuffer& operator=(dynamicbuffer&& other));
};
