...
public:
  ...
  void set_arg(size_t , T ) {
    MARE_UNIMPLEMENTED("GL path does not yet support value args");
  }

  void set_arg_local_alloc(size_t, size_t) {
    MARE_UNIMPLEMENTED("GL path does not yet support local alloc args");
  }

  template<size_t Dims>
  GLsync launch(const ::mare::range<Dims>& r,
                const ::mare::range<Dims>& lr);
