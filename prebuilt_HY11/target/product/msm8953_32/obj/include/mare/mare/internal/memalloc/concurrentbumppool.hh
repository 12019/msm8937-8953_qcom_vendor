...
#include <mare/internal/memalloc/pool.hh>
...
public:
  ...
  concurrent_bump_pool(size_t pool_byte_size, char* pool_buffer):
    ...
    _ref_count(0),
    _concurrent(false) {
    MARE_INTERNAL_ASSERT(pool_byte_size > 0,
  ...
  bool allocate (size_t ref_count = 0) {
    ...
    if (ref_count > 0) {
      _ref_count.store(ref_count, mare::mem_order_release);
      _concurrent = true;
    }
    else {
      _concurrent = false;
  ...
  virtual std::string to_string() {
    std::string str = strprintf("Concurrent Bump pool info: pool[%zu] %p allocs %zu RC %zu pointer = %p size = %zu",
                                ...
                                _ref_count.load(mare::mem_order_relaxed), _pointer, _pool_byte_size);
    return str;
  }

  MARE_DELETE_METHOD(concurrent_bump_pool(concurrent_bump_pool const&));
  MARE_DELETE_METHOD(concurrent_bump_pool(concurrent_bump_pool const&&));
  MARE_DELETE_METHOD(concurrent_bump_pool& operator=(concurrent_bump_pool const&));
  MARE_DELETE_METHOD(concurrent_bump_pool&& operator=(concurrent_bump_pool const&&));
