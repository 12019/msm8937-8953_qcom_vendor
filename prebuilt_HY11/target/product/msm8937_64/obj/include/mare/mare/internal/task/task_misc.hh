...
#include <mare/internal/task/task_shared_ptr.hh>
#include <mare/internal/task/task_state.hh>
...
class lock_guard {
  ...
  friend class ::mare::internal::task;
...
class successor_list {
  ...
  public:
    ...
    static MARE_CONSTEXPR_CONST size_type s_max_entries = 8;
    ...
    bucket(task* t, void* dst, bool move, bucket* next) :
      ...
      MARE_INTERNAL_ASSERT(next != nullptr, "Invalid null pointer.");
    ...
    void insert(task* t, void* dst, bool move = false) {
      MARE_INTERNAL_ASSERT(can_add_dependency() == true, "No entries available in bucket.");
      MARE_INTERNAL_ASSERT(_dependencies[_offset].get_succ() == nullptr, "Bucket already taken.");
    ...
    dependency& operator[](size_type index)  {
      MARE_INTERNAL_ASSERT(index < s_max_entries, "Out of bounds: %d", static_cast<int>(index));
      return _dependencies[index];
    }

  private:

    MARE_DELETE_METHOD(bucket(bucket const&));
    MARE_DELETE_METHOD(bucket(bucket&&));
    MARE_DELETE_METHOD(bucket& operator=(bucket const&));
    MARE_DELETE_METHOD(bucket& operator=(bucket&&));
  ...
  class state {
    ...
    static MARE_CONSTEXPR_CONST raw_type s_bucket_mask = 1;
    static MARE_CONSTEXPR_CONST raw_type s_empty_mask = 2;
  ...
  public:
    ...
    void set_first_bucket(bucket *b) {
      MARE_INTERNAL_ASSERT((reinterpret_cast<uintptr_t>(b) & s_bucket_mask) == 0,
    ...
    bucket* get_first_bucket() {
      MARE_INTERNAL_ASSERT(has_buckets(), "Can't return a bucket becasue there isn't any.");
    ...
    MARE_DELETE_METHOD(state());
    MARE_DELETE_METHOD(state(state const&));
    MARE_DELETE_METHOD(state(state&&));
    MARE_DELETE_METHOD(state& operator=(state const&));
    MARE_DELETE_METHOD(state& operator=(state&&));
...
private:
  ...
  static MARE_CONSTEXPR_CONST size_type s_bucket_size = bucket::s_max_entries;
  static MARE_CONSTEXPR_CONST size_type s_max_inlined_tasks = 2;
...
public:
  ...
  ~successor_list() {
    MARE_INTERNAL_ASSERT(is_empty(), "Reset list before deleting it.");
    MARE_INTERNAL_ASSERT(!has_buckets(), "Overflow buckets exist.");
  }
...
private:
  ...
  void apply_for_each_and_clear(F&& f) {
    ...
    using traits = typename ::mare::internal::function_traits<F>;
    ...
    MARE_INTERNAL_ASSERT(!is_empty(), "List shouldn't be empty by now");
  ...
  void for_each_entry(F&& f) {
    ...
    using traits = ::mare::internal::function_traits<F>;
  ...
  MARE_DELETE_METHOD(successor_list(successor_list const& other));
  MARE_DELETE_METHOD(successor_list& operator=(successor_list const& other));
...
public:
  ...
  MARE_DELETE_METHOD(blocking_code_container(blocking_code_container const&));
  MARE_DELETE_METHOD(blocking_code_container& operator=(blocking_code_container const&));
  MARE_DELETE_METHOD(blocking_code_container(blocking_code_container&&));
  MARE_DELETE_METHOD(blocking_code_container& operator=(blocking_code_container&&));
};
