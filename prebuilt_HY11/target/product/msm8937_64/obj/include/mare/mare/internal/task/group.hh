...
#include <mare/internal/memalloc/groupallocator.hh>
#include <mare/internal/task/exception_state.hh>
#include <mare/internal/task/group_signature.hh>
#include <mare/internal/task/group_state.hh>
#include <mare/internal/task/group_misc.hh>
#include <mare/internal/task/group_shared_ptr.hh>
#include <mare/internal/task/lattice.hh>
...
protected:
  ...
  group(group_misc::id::type id,
    ...
    MARE_INTERNAL_ASSERT(id != group_misc::id_generator::default_meet_id,
...
public:
  ...
  void set_representative_task(task* t) {
    MARE_INTERNAL_ASSERT(t->get_current_group() != this, "Representative task "
  ...
  bool cancel_if_group_member(task* t) {
    MARE_INTERNAL_ASSERT(t, "invalid task pointer checked for group "
                         "membership in %p", this);

    auto g = t->get_current_group(mare::mem_order_relaxed);
  ...
  bool is_canceled(mare::mem_order mem_order=mare::mem_order_acquire) const {
    MARE_UNUSED(mem_order);
  ...
  inline void task_canceled() {
    auto eptr = std::make_exception_ptr(mare::canceled_exception());
    set_exception(eptr);
  }

  inline void task_canceled(std::exception_ptr& eptr) {
    MARE_INTERNAL_ASSERT(eptr != nullptr, "eptr cannot be nullptr");
    set_exception(eptr);
  }
  ...
  bool is_empty(mare::mem_order mem_order=mare::mem_order_seq_cst) const {
...
public:
  ...
  virtual ~group() {
    ...
    MARE_INTERNAL_ASSERT(is_empty() == true,
        "Non-empty group: %s" ,
        to_string().c_str());

    auto xxx = use_count();
    if (xxx != 0) {
      MARE_ALOG("%d %s",xxx, to_string().c_str() );
      mare_breakpoint();
    }

    #ifdef MARE_CHECK_INTERNAL
      _state.lock();
      MARE_INTERNAL_ASSERT(!_trigger_task, "trigger task should be nullptr");
      _state.unlock();
    #endif
...
private:
  ...
  void propagate_inc_task_counter() {
    ...
    MARE_INTERNAL_ASSERT(!get_lattice_node()->get_parent_sees_tasks(),
  ...
  void launch_trigger_task() {
    ...
    if (t_ptr != nullptr) {
      ...
      t_ptr->execute_trigger_task(is_canceled(mare::mem_order_relaxed));
    }
  }
...
public:
...
#if !defined(MARE_NO_GROUP_ALLOCATOR)
  ...
  static group* create_meet_group(group_misc::group_signature& signature,
                                  group_misc::meet_db* db) {
    MARE_INTERNAL_ASSERT(db != nullptr,
...
{
  if (is_leaf()) {
    if (_tasks.fetch_add(1, mare::mem_order_seq_cst) != 0) {
...
{
  if (is_leaf()) {
    if (_tasks.fetch_sub(1, mare::mem_order_seq_cst) != 1) {
...
task::leave_groups_unlocked(bool must_cancel)
{
  ...
  auto g = get_current_group(mare::mem_order_relaxed);
  ...
  MARE_INTERNAL_ASSERT(get_current_group(mare::mem_order_relaxed) == nullptr,
...
task::set_running()
{
  ...
  auto g = get_current_group(mare::mem_order_relaxed);
  if (g && g->is_canceled(mare::mem_order_relaxed))
    return false;
...
task::is_canceled(bool has_cancel_request)
{
  ...
  if (auto g = get_current_group(mare::mem_order_relaxed))
...
task::join_group_and_set_in_utcache(group* g) {
  MARE_INTERNAL_ASSERT(is_locked(),
                       "Task must be locked: %s",
                       to_string().c_str());
  MARE_INTERNAL_ASSERT(g != nullptr, "Nullptr");
  ...
  MARE_INTERNAL_ASSERT(!prev_state.in_utcache(),
                       "Task already in utcache: %s",
                       to_string().c_str());

  MARE_UNUSED(prev_state);
...
task::abort_on_cancel()
{
  if (!is_anonymous()) {
    ...
    auto snapshot = get_snapshot(mare::mem_order_acquire);
    MARE_INTERNAL_ASSERT(snapshot.is_running(),
  ...
  auto g = get_current_group(mare::mem_order_relaxed);
  MARE_INTERNAL_ASSERT(!is_anonymous() || g,
...
#include <mare/internal/log/coreloggerevents.hh>
