...
#include <mare/internal/queues/bounded_lfqueue.hh>
#include <mare/internal/util/debug.hh>
#include <mare/internal/util/macros.hh>
#include <mare/internal/util/memorder.hh>

#define MARE_LFQ_FORCE_SEQ_CST 0

#if MARE_LFQ_FORCE_SEQ_CST
#define MARE_LFQ_MO(x) mare::mem_order_seq_cst
#else
#define MARE_LFQ_MO(x) x
#endif
...
public:
  ...
  MARE_DELETE_METHOD(lfq_node(lfq_node const&));
  MARE_DELETE_METHOD(lfq_node(lfq_node &&));
  MARE_DELETE_METHOD(lfq_node& operator=(lfq_node const&));
  MARE_DELETE_METHOD(lfq_node& operator=(lfq_node &&));

  lfq_node<T>* get_next(){ return _next.load(MARE_LFQ_MO(mare::mem_order_relaxed)); }

  bool set_next(lfq_node<T>* const& new_val){

    lfq_node<T>* old_val = nullptr;
    if(_next.compare_exchange_strong(old_val, new_val, MARE_LFQ_MO(mare::mem_order_relaxed))){
      return true;
    }
...
public:
 ...
 explicit lfq(size_t log_size = 12):
  ...
  MARE_DELETE_METHOD(lfq(lfq const&));
  MARE_DELETE_METHOD(lfq(lfq &&));
  MARE_DELETE_METHOD(lfq& operator=(lfq const&));
  MARE_DELETE_METHOD(lfq& operator=(lfq &&));
  ...
  size_t push(value_type const& v){
    ...
    while(true){
      auto current_blfq_node = _tail.load(MARE_LFQ_MO(mare::mem_order_relaxed));

      auto next_blfq_node =  current_blfq_node->get_next();

      if(next_blfq_node != nullptr){
        _tail.compare_exchange_strong(current_blfq_node, next_blfq_node, MARE_LFQ_MO(mare::mem_order_relaxed));
        continue;
      }

      auto sz = current_blfq_node->_c.push(v);
      if(sz != 0){
      ...
      sz = new_blfq_node->_c.push(v);
      MARE_INTERNAL_ASSERT(sz != 0, "Push into a local lfq_node. Should always succeed");

      if(current_blfq_node->set_next(new_blfq_node)){

        _tail.compare_exchange_strong(current_blfq_node, new_blfq_node, MARE_LFQ_MO(mare::mem_order_relaxed));
        return (sz + (full_nodes_encountered * current_blfq_node->get_max_node_size()));
      }
      else{
  ...
  size_t pop(value_type& r){
    while(true){
      auto current_blfq_node = _head.load(MARE_LFQ_MO(mare::mem_order_relaxed));
      auto sz = current_blfq_node->_c.pop(r);
      if(sz != 0){
        return sz;
      }
      ...
      sz = current_blfq_node->_c.pop(r);
      if(sz != 0){
        return sz;
      }

      _head.compare_exchange_strong(current_blfq_node, next_blfq_node, MARE_LFQ_MO(mare::mem_order_relaxed));
    }
  }
