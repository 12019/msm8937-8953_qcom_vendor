...
bool ws_node<T>::try_own(node_type* n)
{
  ...
  MARE_INTERNAL_ASSERT(n != nullptr, "Call try_own from a nullptr!");

  log::fire_event(log::events::ws_tree_worker_try_own());

  if (n->_progress.load(mare::mem_order_relaxed) != UNCLAIMED)
    return false;
                                       ...
                                       const T& init)
{
  ...
  MARE_INTERNAL_ASSERT(n != nullptr, "Call try_steal from a nullptr!");
  ...
  do {
    ...
    iter = n->_progress.load(mare::mem_order_relaxed);
    MARE_INTERNAL_ASSERT(iter != UNCLAIMED, "Can't steal an unclaimed node");
...
{
  MARE_INTERNAL_ASSERT(_root != nullptr, "Invalid root to split");
  ...
  while(nctx != 1) {
    ...
    for(size_type i = 0; i < _prealloc_leaf; ++i, ++pos){
      ...
      if (nctx < 4) {
        n->get_left(mare::mem_order_relaxed)->set_assigned();
        n->get_right(mare::mem_order_relaxed)->set_assigned();

        if (i == 0) {
          n->get_left(mare::mem_order_relaxed)->
                        inc_progress(_blk_size, mare::mem_order_relaxed);
        }
      }
    }
...
void ws_tree<T>::split_tree_before_stealing(size_type nctx)
{
  ...
  while(nctx != 1) {
    ...
    for(size_type i = 0; i < _prealloc_leaf; ++i, ++pos){
      ...
      if (nctx < 4 && i == 0) {
        n->get_left(mare::mem_order_relaxed)->inc_progress(_blk_size, mare::mem_order_relaxed);
      }
    }
...
typename ws_tree<T>::node_type* ws_tree<T>::find_work_prealloc(size_type tid)
{
  ...
  for (size_type i = 0; i < _prealloc_level; ++i, temp >>= 1) {
    MARE_INTERNAL_ASSERT(n != nullptr, "Fail to pre-allocate tree");
    if (!(temp & 0x1))
      n = n->get_left(mare::mem_order_relaxed);
    else
      n = n->get_right(mare::mem_order_relaxed);
  }
                             ...
                             const T& init)
{
  ...
  if (n->is_leaf() && !n->is_assigned()) {
#ifdef MARE_ADAPTIVE_PFOR_DEBUG
    n->increase_traversal(1, mare::mem_order_relaxed);
#endif
    ...
    if (steal_result == try_steal_result::SUCCESS) {
      ...
      return n->get_right(mare::mem_order_relaxed);
    }
    ...
    MARE_INTERNAL_ASSERT(steal_result == try_steal_result::ALREADY_STOLEN,
                         "Invalid node state.");
  }
  ...
  if (n->get_left_traversal() <= n->get_right_traversal()) {
    ...
    first = n->get_left(mare::mem_order_relaxed);
    second = n->get_right(mare::mem_order_relaxed);
  } else {
    n->inc_right_traversal();
    first = n->get_right(mare::mem_order_relaxed);
    second = n->get_left(mare::mem_order_relaxed);
  }
