...
struct is_body_arg1_based_on_b<T, 2, B> {
  ...
  static_assert(std::is_reference<arg1_type>::value,
    "the second argument of a cpu stage lambda/function shuold always be a reference to mare::stage_input<T>.");
...
template<typename T>
struct strip_stage_input_type<mare::stage_input<T>&> {
  using type = T;
};
...
template<>
struct is_parallel_stage<mare::parallel_stage> {
  static constexpr bool value = true;
};
...
template<>
struct is_serial_stage<mare::serial_stage> {
  static constexpr bool value = true;
};
...
template<>
struct is_iteration_lag<mare::iteration_lag> {
  static constexpr bool value = true;
};
...
template<>
struct is_iteration_rate<mare::iteration_rate> {
  static constexpr bool value = true;
};
...
template<>
struct is_sliding_window_size<mare::sliding_window_size> {
  static constexpr bool value = true;
};

template<typename T>
struct is_mare_pipeline_callable {
private:
  template <typename U >
...
struct is_mare_pipeline_callable<FReturnType(&)(FArgs...)> {
  static constexpr bool value = true;
};
...
struct callable_parser  {
  ...
  static constexpr bool is_callable =
    is_mare_pipeline_callable<type>::value || is_mare_pipeline_callable<unref_type>::value;
...
struct check_add_cpu_stage_params {
  ...
  static_assert(
                is_body_arg1_based_on_b<Body, function_traits<Body>::arity::value, mare::stage_input_base>::value,
...
struct get_ringbuffer_type  {
  using type =
    typename std::conditional<std::is_same<T, void>::value,
                              mare::internal::stagebuffer,
                              mare::internal::ringbuffer<T>>::type;
};
...
struct get_dynamicbuffer_type  {
  using type =
    typename std::conditional<std::is_same<T, void>::value,
                              mare::internal::stagebuffer,
                              mare::internal::dynamicbuffer<T>>::type;
};
...
template<size_t N>
struct apply_launch {
  template<typename CPINST, typename LAUNCHTYPE, typename TUPLE, typename... T>
  static void apply(CPINST* cpinst, LAUNCHTYPE& launch_type, TUPLE& tp, T&... t) {
    apply_launch<N-1>::apply(cpinst, launch_type, tp, std::get<N-1>(tp), t...);
  }
};
...
struct apply_launch<0> {
  template<typename CPINST, typename LAUNCHTYPE, typename TUPLE, typename... T>
  static void apply(CPINST* cpinst, LAUNCHTYPE& launch_type, TUPLE&, T&... t) {
    cpinst->launch(t..., launch_type);
  }
};
...
template<typename... Args>
struct is_gpu_kernel<mare::gpu_kernel<Args...>&> {
  static constexpr bool value = true;
};
...
template<typename ...Args>
struct is_mare_pipeline_callable<mare::gpu_kernel<Args...>&> {
  static constexpr bool value = false;
};
...
template <typename T>
struct is_mare_range : std::false_type {
  static constexpr bool value = false;
};

template <size_t Dims>
struct is_mare_range<mare::range<Dims>> : std::true_type {
  static constexpr bool value = true;
};
...
template<typename T>
struct strip_buffer_dir<mare::out<mare::buffer_ptr<T>>>  {
 using type = mare::buffer_ptr<T>;
};

template<typename T>
struct strip_buffer_dir<mare::in<mare::buffer_ptr<T>>>  {
 using type = mare::buffer_ptr<T>;
};
...
template<typename... Args>
struct get_gpu_kernel_args_tuple<mare::gpu_kernel<Args...>> {
...
struct check_add_gpu_stage_params {
  ...
  static_assert(
    ...
    "before lambda's return tuple should have at least two elements, i.e. a mare::range, and one input for the gpu kernel");

  using before_return_type_first_element = typename std::tuple_element<0, before_return_type>::type;
  static_assert(
    is_mare_range<before_return_type_first_element>::value,
    "before lambda's return tuple should be a std::tuple whose first element is a mare::range");
