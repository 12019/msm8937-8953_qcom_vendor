...
private:
  ...
  public:
    ...
    void set_next(node_slab* next, mare::mem_order order) {
      _next.store(next, order);
    }

    node_slab* get_next(mare::mem_order order) {
  ...
  node_slab* find_slab(const size_type pos, node_slab* first_slab) {
    ...
    while(ck != nullptr) {
      if (ck->has(pos))
        return ck;
      ck = ck->get_next(mare::mem_order_acquire);
    }
    return nullptr;
  }
  ...
  node_slab* grow(size_type pos, node_slab* current_slab) {
    ...
    current_slab->set_next(new_slab, mare::mem_order_relaxed);

    _slab.store(new_slab, mare::mem_order_release);
  ...
  pointer get_next_impl() {
    ...
    auto pos = _pos.fetch_add(size_type(1), mare::mem_order_relaxed);
    auto slab = _slab.load(mare::mem_order_relaxed);
    ...
    slab = _slab.load(mare::mem_order_acquire);
...
public:
  ...
  explicit node_pool(size_type max_sharers) :
    ...
    _pos(0),
    _slab(&_inlined_slab) {
    MARE_INTERNAL_ASSERT(max_sharers < Size_, "Too many sharers for pool.");
  }
  ...
  ~node_pool() {
    auto next_slab = _inlined_slab.get_next(mare::mem_order_acquire);
    while (next_slab) {
      auto tmp = next_slab->get_next(mare::mem_order_relaxed);
      delete next_slab;
      next_slab = tmp;
    }
  };

  MARE_DELETE_METHOD(node_pool(node_pool const&));
  MARE_DELETE_METHOD(node_pool(node_pool&&));
  MARE_DELETE_METHOD(node_pool& operator=(node_pool const&));
  MARE_DELETE_METHOD(node_pool& operator=(node_pool&&));
};
