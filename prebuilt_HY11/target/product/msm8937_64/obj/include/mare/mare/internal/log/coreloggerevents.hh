...
MARE_ADD_TO_MEMBUF_MAP(group_wait_for_ended);
...
MARE_ADD_TO_MEMBUF_MAP(join_ut_cache);
...
MARE_ADD_TO_MEMBUF_MAP(task_finished);
...
MARE_ADD_TO_MEMBUF_MAP(task_launched_into_gpu);
#endif
...
struct task_executes_handler : public single_task_event_handler {
  ...
  explicit task_executes_handler(event&&e ) :
    single_task_event_handler(std::forward<event>(e)),
    _group(e.get_task()->get_current_group(mare::mem_order_relaxed)),
    _group_id(e.get_task()->get_current_group(mare::mem_order_relaxed)?
              e.get_task()->get_current_group(mare::mem_order_relaxed)->get_log_id(): null_object_id<group>()),
...
MARE_ADD_TO_MEMBUF_MAP(task_wait_inlined);
...
MARE_ADD_TO_MEMBUF_MAP(trigger_task_scheduled);
