...
#include <mare/internal/patterns/pipeline/pipelinefactory.hh>
...
public:
  ...
  MARE_DELETE_METHOD(pipeline_stage_skeleton_base(
     pipeline_stage_skeleton_base&& other));
  MARE_DELETE_METHOD(pipeline_stage_skeleton_base& operator=(
...
public:
  ...
  virtual void apply_after(
      ...
      pipeline_launch_type ,
      void* ) {
      MARE_INTERNAL_ASSERT(false, "shoud never be called for a cpu stage");
  };
...
struct helper_create_pipeline_stage_gpu_task<Range, GK, TP, -1, N...> {
  ...
  static task_ptr<> create_gpu_stage_task(Range& range, GK& gk, TP& tp) {
    return mare::create_task(gk, range, std::get<N+1>(tp)...);
  }
};
...
public:
  ...
  virtual void apply(
     ...
     auto p = apply_before(in_first_idx, in_size, inbuf, iter_id, max_iter, pstage, pinst, launch_type);
     std::get<0>(p)->launch();
     std::get<0>(p)->wait_for();

     apply_after(out_idx, outbuf, iter_id, max_iter, pstage, pinst, launch_type, std::get<1>(p));
  }
...
MARE_GCC_IGNORE_END("-Weffc++");
  ...
  MARE_DELETE_METHOD(pipeline_skeleton_base(pipeline_skeleton_base&& other));
  MARE_DELETE_METHOD(pipeline_skeleton_base& operator=(pipeline_skeleton_base const& other));
  MARE_DELETE_METHOD(pipeline_skeleton_base& operator=(pipeline_skeleton_base&& other));
...
public:
  ...
  pipeline_skeleton(pipeline_skeleton const& other):
    ...
    ref_counted_object<pipeline_skeleton<UserData...>, mareptrs::default_logger>() {}
    ...
    sliding_window_size const& sw) {
      ...
      _stages.push_back(
        new pipeline_gpu_stage_skeleton<BeforeBody, GKBody, AfterBody, UserData...>(
          ...
          has_lag, _num_stages ++, lag, rate, sw, stage));
  ...
  MARE_DELETE_METHOD(pipeline_skeleton(pipeline_skeleton&& other));
  MARE_DELETE_METHOD(
