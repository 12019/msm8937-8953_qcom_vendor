...
private:
  ...
  static void count(events::task_created&&) {
    s_num_tasks.fetch_add(1, mare::mem_order_relaxed);
  }
  ...
  static void count(events::task_executes&& e) {
    if (e.is_blocking()) {
      s_num_blocking_tasks_exec.fetch_add(1, mare::mem_order_relaxed);
    } else if (e.is_gpu()) {
      s_num_gpu_tasks_exec.fetch_add(1, mare::mem_order_relaxed);
    } else if (e.is_inline()) {

      s_num_inline_tasks_exec.fetch_add(1, mare::mem_order_relaxed);
    } else {
  ...
  static void count(events::task_stolen&& e) {
    ...
    s_num_tasks_stolen[qid].fetch_add(1, mare::mem_order_relaxed);
  }

  static void count(events::group_created&&) {
    s_num_groups.fetch_add(1, mare::mem_order_relaxed);
  }
