...
#else
        if (!s32)
            *peBufType = OMX_InterlaceInterleaveFrameTopFieldFirst;
#endif
    }
                                 ...
                                 const char *pcFn)
{
    ...
    offset = pstBuf->pBuf->extradata.offset;
    base = pstBuf->stExtra.pvBase;
    virt = base + offset;
    bytes = pstBuf->pBuf->extradata.filled_len;

    LOGD("dumping extradata: %s: offset=%d, base=%p, virt=%p, bytes=%u, fd=%p",
         cPath, offset, base, virt, bytes, fd);

    u32Ret = fwrite(virt, sizeof(char), bytes, fd);
    if(u32Ret != bytes)
    {
        LOGE("File write Y failed.");
        goto ERR_WRITE;
