...
static void vVpp_FramerateReset(t_StVppFrameRate *stpFrameRate)
{
    if(!stpFrameRate)
        return;

    memset(stpFrameRate, 0, sizeof(t_StVppFrameRate));
}
                              ...
                              t_StVppBuf *pstIntBuf)
{
    ...
    stpFrameRate = &pstCb->stFrameRate;
    ...
    if(u32Flags & VPP_BUFFER_FLAG_EOS)
        vVpp_FramerateReset(stpFrameRate);

    u32PrevTS = stpFrameRate->u32PrevTimestamp;
    u32PrevFR = stpFrameRate->u32PrevFrameRate;
    u32Index = stpFrameRate->u32Index;
    u32Count = stpFrameRate->u32Entries;

    if((u32Timestamp > u32PrevTS) && (u32Count > 0))
        u32FrameRate = US_TO_FPS_NUMERATOR / (u32Timestamp - u32PrevTS);
    else
        u32FrameRate = DEFAULT_FRAME_RATE; // Default if timestamps aren't increasing or first frame

    u32FrameRate = RANGE_CHECK(u32FrameRate, MIN_FRAME_RATE, MAX_FRAME_RATE);

    stpFrameRate->u32PrevFrameRate = u32FrameRate;

    if(u32Count > 0)
    {
        if(u32Index == 0)
            u32TempIndex = FRAMERATE_HISTO_SZ - 1;
        else
        ...
        if(u32Count == 1)
        {
            ...
            stpFrameRate->u32aFrameRates[u32TempIndex] = u32FrameRate;
        }
        for(i = 0; i < u32Count; i++)
        {
            u32AvgFrameRate += stpFrameRate->u32aFrameRates[u32TempIndex];
            if(u32TempIndex == 0)
                u32TempIndex = FRAMERATE_HISTO_SZ - 1;
            else
                u32TempIndex--;
        ...
        if(((ABS_DIFF(u32FrameRate, u32PrevFR)) > FRAME_RATE_DELTA_MAX) &&
           ((ABS_DIFF(u32FrameRate, u32Temp)) > FRAME_RATE_DELTA_MAX))
        {
    ...
    stpFrameRate->u32aFrameRates[u32Index] = u32FrameRate;
    stpFrameRate->u32PrevTimestamp = u32Timestamp;
    stpFrameRate->u32Index++;
    if(stpFrameRate->u32Index >= FRAMERATE_HISTO_SZ)
        stpFrameRate->u32Index = 0;
    if(stpFrameRate->u32Entries < FRAMERATE_HISTO_SZ)
        stpFrameRate->u32Entries++;

    stpFrameRate->u32AvgFrameRate = u32AvgFrameRate;
    if(((ABS_DIFF(u32AvgFrameRate, stpFrameRate->u32ProgrammedFrameRate)) >=
       FRAME_RATE_UPDATE_THRESHOLD) && (u32Count > 0))
    {
        u32Ret = u32VppUsecase_UpdateFramerate(ctx, pstUc, u32AvgFrameRate);
        if(u32Ret != VPP_OK)
        {
            stpFrameRate->u32Bypass = 1;
            LOGD("New frame rate %dFPS not supported. In bypass.", u32AvgFrameRate);
        }
        else
        {
            stpFrameRate->u32Bypass = 0;
            LOGD("New frame rate %dFPS supported.", u32AvgFrameRate);
        }

        stpFrameRate->u32ProgrammedFrameRate = u32AvgFrameRate;
    }

    if(stpFrameRate->u32Bypass)
        VPP_FLAG_SET(pstIntBuf->u32InternalFlags, VPP_BUF_FLAG_BYPASS);

    LOGI("Calculated frame rate: %d", u32AvgFrameRate);
...
void *vpp_init(uint32_t flags, struct vpp_callbacks cb)
{
    ...
    // Utils
    u32Ret = u32VppUtils_Init(pstCtx);
    if (u32Ret != VPP_OK)
    {
