...
static const t_StVppStatsConfig astHvxCoreStatsCfg[] = {
    VPP_PROF_DECL(HVX_CORE_STAT_REGBUF, 100, 1),
    VPP_PROF_DECL(HVX_CORE_STAT_UNREGBUF, 100, 1),
};
...
static void vVppIpHvxCore_GetVersionInfo(vpp_svc_cap_resource_list_t* pstCapabilityResources)
{
    int32_t s32Ret, i;

    VPP_RET_VOID_IF_NULL(pstCapabilityResources);

    for (i = 0; i < pstCapabilityResources->resourceLen; i++)
    {
        s32Ret = vpp_svc_get_buildts_id(pstCapabilityResources->resource[i].vpp_func_id,
                                        (unsigned char *)pstCapabilityResources->resource[i].build_ts,
                                        MAX_TS_LEN);
        if (s32Ret == AEE_SUCCESS)
        {
            LOGI("HVX func_id[%d] version: %s",
                 pstCapabilityResources->resource[i].vpp_func_id,
                 pstCapabilityResources->resource[i].build_ts);
        }
...
void vVppIpHvxCore_RegisterBuffer(t_StVppIpHvxCoreCb *pstHvxCore, t_StVppBuf *pstBuf)
{
    t_StVppCtx *pstCtx;

    if (!pstHvxCore || !pstBuf || !pstHvxCore->stBase.pstCtx)
        return;

    pstCtx = pstHvxCore->stBase.pstCtx;

#ifndef HVX_DO_NOT_REGISTER_BUFFER
    struct vpp_mem_buffer *pstExtMemBuf = &pstBuf->pBuf->pixel;
    t_StVppMemBuf *pstIntMemBuf = &pstBuf->stPixel;
    void *pv = NULL;

    if (VPP_FLAG_IS_SET(pstBuf->u32InternalFlags, VPP_BUF_FLAG_HVX_MAPPED))
        return;
    ...
    if (pstHvxCore->stBase.bSecureSession)
    {
        if (remote_register_fd)
        {
            ...
            pv = remote_register_fd(pstExtMemBuf->fd, pstExtMemBuf->alloc_len);
            ...
            if (pv == (void *)-1 || pv == NULL)
            {
                LOGE("unable to remote_register_fd, fd=%d, alloc_len=%u, "
                     "dummy_ptr=%p", pstExtMemBuf->fd, pstExtMemBuf->alloc_len, pv);
            }
            else
            {
                pstIntMemBuf->pvBase = pv;
                VPP_FLAG_SET(pstBuf->u32InternalFlags, VPP_BUF_FLAG_HVX_MAPPED);
                LOGI("registering secure buffer, fd=%d, sz=%u, new_ptr=%p",
                     pstExtMemBuf->fd, pstExtMemBuf->alloc_len, pv);
            }
        }
        else
            LOGE("unable to register secure buffer, fd=%d, ptr=%p, sz=%u",
                 pstExtMemBuf->fd, pv, pstExtMemBuf->alloc_len);
    ...
    else
    {
        if (remote_register_buf)
        {
            ...
            LOGI("registering buffer, fd=%d, ptr=%p, sz=%u", pstIntMemBuf->fd,
                 pstIntMemBuf->pvBase, pstIntMemBuf->u32MappedLen);
        }
        else
            LOGE("unable to remotely register buffer, fd=%d, ptr=%p, sz=%u",
...
void vVppIpHvxCore_UnregisterBuffer(t_StVppIpHvxCoreCb *pstHvxCore, t_StVppBuf *pstBuf)
{
    t_StVppCtx *pstCtx;
    ...
    if (remote_register_buf)
    {
        ...
        if (pstHvxCore->stBase.bSecureSession)
        {
            u32MappedLen = pstExtMemBuf->alloc_len;
        }
        ...
        VPP_FLAG_CLR(pstBuf->u32InternalFlags, VPP_BUF_FLAG_HVX_MAPPED);

        LOGI("unregistering buffer, fd=%d, ptr=%p, sz=%u", pstExtMemBuf->fd,
...
uint32_t u32VppIpHvxCore_BuffInInit(t_StVppIpHvxCoreCb *pstHvxCore, uint32_t u32NumBuffers)
{
    ...
    uint32_t u32PixDataAllocSize, u32FrameSize;
...
{
    if ((pstHvxCore) && (pstHvxCore->pstBufferdataIn->pixel_data))
...
{
    if ((pstHvxCore) && (pstHvxCore->pstBufferdataOut->pixel_data))
                              ...
                              uint32_t framesize_bytes)
{
    ...
    pstIonBuf = &pstHvxCore->stIonCtxBuf;
    ...
    vVppIpHvxCore_GetVersionInfo(pstCapabilityResources);
    ...
    vpp_diag_ctx_size = vpp_svc_get_diag_ctx_size(vpp_processing_flags, 0);

    if (vpp_ctx_size < 0 || vpp_diag_ctx_size < 0)
    {
        LOGE("get_ctx_sz returned invalid, ctx_sz=%d, diag_ctx_sz=%d",
             vpp_ctx_size, vpp_diag_ctx_size);
        u32Ret = VPP_ERR_HW;
        goto ERR_GET_CTX_SZ;
    }

    pstContext->user_dataLen = vpp_ctx_size + vpp_diag_ctx_size;
    u32Length = pstContext->user_dataLen;

    LOGD("framesize_bytes=%d, vpp_ctx_size=%d, vpp_diag_ctx_size=%d, u32Length=%d",
         framesize_bytes, vpp_ctx_size, vpp_diag_ctx_size, u32Length);

#ifdef USE_MALLOC
    if (0 == (pstContext->user_data = (unsigned char*)memalign(128, u32Length)))
    {
    ...
    uint32_t u32;
    void *pv = NULL;
    u32Length = (pstContext->user_dataLen + 127) & (~127); //128 aligned

    u32 = u32VppIon_Alloc(pstHvxCore->stBase.pstCtx, u32Length, pstHvxCore->stBase.bSecureSession,
                          pstIonBuf);
    if (u32 != VPP_OK)
    {
        LOGE("Error: context ion alloc failed, secure=%u, fd=%d, addr=%p, sz=%u",
             pstHvxCore->stBase.bSecureSession, pstIonBuf->fd_ion_mem,
             pstIonBuf->buf, u32Length);
        u32Ret = VPP_ERR_NO_MEM;
        goto ERR_ALLOC;
    }
    ...
    if (!pstHvxCore->stBase.bSecureSession)
    {
        // TODO: agreement with firmware that they will zero this buffer
        // themselves for all cases
        LOGI("non-secure mode, zeroing context buffer");
        memset(pstContext->user_data, 0, sizeof(u32Length));
    }

#endif

    //Prepare context for use
    rc = vpp_svc_prepare_ctx((unsigned char*)pstContext->user_data, u32Length, framesize_bytes,
                             vpp_processing_flags, vpp_ctx_size, vpp_diag_ctx_size);
    if(rc)
    {
...
#else
    vVppIpHvxCore_UnregisterIon(pstIonBuf, (void *)&pstContext->user_data);
...
uint32_t u32VppIpHvxCore_Close(t_StVppIpHvxCoreCb *pstHvxCore)
{
    ...
    if(pstHvxCore->pstContext->user_data)
        free(pstHvxCore->pstContext->user_data);
#else
    t_StVppIonBuf *pstIonBuf = &pstHvxCore->stIonCtxBuf;
    LOGD("freeing ion mem fd=%d", pstIonBuf->fd_ion_mem);
    vVppIpHvxCore_UnregisterIon(pstIonBuf, (void *)pstHvxCore->pstContext->user_data);
    u32VppIon_Free(pstHvxCore->stBase.pstCtx, pstIonBuf);
#endif
    vVppIpHvxCore_BuffInTerm(pstHvxCore);
    vVppIpHvxCore_BuffOutTerm(pstHvxCore);
