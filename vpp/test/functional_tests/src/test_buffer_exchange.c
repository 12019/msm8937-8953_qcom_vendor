    ...
    if(u32TestMask & FUNCTEST_SETCTRL)
    {
        ...
        if(u32 == VPP_OK)
        {
               ...
               (pstTestCtx->hqvCtrl.mode == HQV_MODE_MANUAL))
            {
                ...
                pstTestCtx->hqvCtrl.cade.mode = 5;  // Should reject this value
            ...
            else if((get_uc_algo_enabled(p, HQV_CONTROL_AIE)) && (pstTestCtx->hqvCtrl.mode == HQV_MODE_MANUAL))
            {
                ...
                pstTestCtx->hqvCtrl.aie.mode = 5;  // Should reject this value
...
TEST(BufferExchangeReconfigError_Testcase)
{
    ...
    if(u32TestMask & FUNCTEST_RECONFIG_ALL)
    {
        ...
        if(u32 == VPP_OK)
        {
            ...
            pstTestCtx->u32FramesOut = u32TempFrames;
            pstTestCtx->buf_pool->params.eOutputFileType = u32TempFileType;
            for(i=0; i < pFuncTestCtx->u32StreamCount; i++)
            {
                pFuncTestCtx->reconfig_streams[i].reconfig_pool->params.eOutputFileType = u32TempFileType;
            }
...
TEST(BufferExchangeReconfigBypass_Testcase)
{
    ...
    if((u32TestMask & FUNCTEST_BYPASS) || (u32TestMask & FUNCTEST_RECONFIG_ALL))
    {
        ...
        if(u32 == VPP_OK)
        {
            ...
            u32MaxW = u32GetMaxWidthForRes(u32MaxRes);
            u32MaxH = u32GetMaxHeightForRes (u32MaxRes);
            for(i=0; i < pFuncTestCtx->u32StreamCount; i++)
            {
                param.width = pFuncTestCtx->reconfig_streams[i].reconfig_pool_params.u32Width;
                param.height = pFuncTestCtx->reconfig_streams[i].reconfig_pool_params.u32Height;
