...
#ifdef FEATURE_DATA_LINUX_ANDROID
...
#ifdef FEATURE_DATA_LINUX_ANDROID
static int radish_should_keep_stats(void)
{
  #define PROP_NMR1_SDK 25
  if (server_cfg.prop_sdk_version < PROP_NMR1_SDK)
    return 1;
  return 0;
}
#else /* Non-Android */
...
)
{
  ...
  {
     if('\0' != *g_cur_iface && radish_should_keep_stats())
     {
...
int main(int argc, char *argv[])
{
  ...
  List *cfg_lst = NULL;
#if defined FEATURE_DATA_LINUX_ANDROID
  #define PROP_MAX_LEN 255
  const char PROP_SDK[] = "ro.build.version.sdk";
  const char PROP_SDK_DEFAULT[] = "0";
  char* str_err_end;
  char prop[PROP_MAX_LEN] = "";
#endif /* FEATURE_DATA_LINUX_ANDROID */
  ...
  /* AOSP Introduced IPv6 Tethering in NMR1, therefore must leave
   * Tether counters for their implementation when this platform
   * version is detected
   */
  property_get(PROP_SDK, prop, PROP_SDK_DEFAULT);
  server_cfg.prop_sdk_version = strtol(prop,&str_err_end,10);
  if (*str_err_end)
  {
    server_cfg.prop_sdk_version = 0;
  }
