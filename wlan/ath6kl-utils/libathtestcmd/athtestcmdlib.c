...
void qca6174SetDBS(char *dbs_val)
{
	...
	int dbs_value = PHY_RF_MODE_NON_DBS_PHY1;
...
int qca6174TxCommand(const char *txtype)
{
	...
	if (Params->mode == TCMD_CONT_TX_OFF) {
		...
		tlv2AddParms(4, PARM_TXPOWER, RATE_POWER_MAX_INDEX, 0,
				(uint8_t *)&Params->txPower);
		tlv2AddParms(4, PARM_PKTSZ, RATE_POWER_MAX_INDEX, 0,
				(uint8_t *)&Params->pktLength);
...
int qca6174RxPacketStart(char *rx_type)
{
	...
	if ( rowIndex != 0 ) {
		...
		tlv2AddParms(4, PARM_BSSID, ATH_MAC_LEN, 0,
				(uint8_t *)&Params->bssid);
                printf("\n rx cmd: set bssid 0x%02x, 0x%02x, 0x%02x,"
