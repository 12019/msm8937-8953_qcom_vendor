    ...
    if (gtm->enable_adrc) {
      ...
      if (gtm->use_gtm_fix_curve == 0 ){
        ...
        if (!gtm->is_aec_update_valid) {
          ...
          if( (isp_sub_module->hdr_mode == CAM_SENSOR_HDR_IN_SENSOR) && gtm->ext_func_table &&
            gtm->ext_func_table->gtm_trigger_params){
            ret = gtm->ext_func_table->gtm_trigger_params(module, isp_sub_module, event);
            if (ret == FALSE){
