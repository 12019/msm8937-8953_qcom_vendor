  ...
  chromatix_GIC_v2_type *chromatix_gic, float multifactor)
{
  ...
  if (ratio > 0.0f) {
    ...
    denoise_scale = INTERPOLATE(gic_start->denoise_scale,
      gic_end->denoise_scale, ratio, 8);

    for (i = 0; i < GIC_NOISE_STD_LUT_SIZE_CHROMATIX; i++) {
      tmp_val = (float)
        INTERPOLATE(
          (float)gic_start->noise_std_lut_level0[i],
          (float)gic_end->noise_std_lut_level0[i], ratio, 0);
      tmp_val = CLAMP(tmp_val, 0.0f, GIC2_NOISE_OFFSET_MAX_VAL_CHROMATIX);
      /* 16 * tmp * denoise_scale / 256 */
      tmp_val = (tmp_val * denoise_scale) / 16.0f;
      mod->noiseStdTable[i] = CLAMP(Round(tmp_val), GIC2_NOISE_STD_LUT_LEVEL0_MIN,
        GIC2_NOISE_STD_LUT_LEVEL0_MAX);
    }
  } else {
    gic = &chromatix_gic->gic_config[mod->trigger_index];

    noise_offset = (float)gic->thin_line_noise_offset * multifactor;
    filter_str  = FLOAT_TO_Q(8, gic->filter_strength);
    denoise_scale =  FLOAT_TO_Q(8, gic->denoise_scale);
  ...
  noise_offset = MIN(noise_offset, GIC2_THINLINE_OFF_MAX);
  mod->ISP_GICCfgCmd.noiseOffset = FLOAT_TO_Q(0, noise_offset);

  filter_str = MIN(filter_str, GIC2_FILTER_STRENGTH_MAX);
  /* Flip filter strength to have same effect like ABF/wavelet */
  mod->ISP_GICCfgCmd.filterStrength = FLOAT_TO_Q(0, GIC2_FILTER_STRENGTH_MAX - filter_str);
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  gic_config_internal(gic, chromatix_gic, multifactor);
...
FILL_METADATA:
 if (output->metadata_dump_enable == 1) {
   ...
   memcpy(gic_dmi_info->isp_meta_dump,
      gic->packed_noiseStdTable, sizeof(gic->packed_noiseStdTable));
 }
  ...
  chromatix_parms_type *chroma_ptr)
{
  ...
  gic_config_internal(mod, chromatix_gic, multifactor);
