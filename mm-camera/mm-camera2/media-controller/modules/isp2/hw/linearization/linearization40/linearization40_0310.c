 ...
 **/
static boolean linearization40_v2_copy_table_to_float(
  chromatix_linearization_v2_type * input, Linearization_high_res_Lut_t *output)
{
  int i;
  for (i = 0; i < 8; i++) {
...
)
{
  ...
  if (pchromatix_L_v2->linearization_v2_enable) {
    ...
    tgr_idx2 = ((linearization->trigger_index + 1) < MAX_LIGHT_TYPES_FOR_SPATIAL) ?
