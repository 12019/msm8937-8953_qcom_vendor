  ...
  void *data)
{
  ...
  if (isp_sub_module->trigger_update_pending == TRUE) {
    ...
    if (isp_sub_module->led_calibration_mode == CAM_LED_CALIBRATION_MODE_OFF) {
      ...
      memcpy(&final_output, &output2, sizeof(Linearization_high_res_Lut_t));
    }
...
FILL_METADATA:
  ...
  ret = linearization40_calc_clamp(linearization, &final_output, sub_module_output);
  if (FALSE == ret) {
    ISP_ERR("failed");
    goto ERROR;
  }
  ...
  chromatix_VFE_common_type *pchromatix_common)
{
  ...
  if (!linearization40_update_right_base_tables(linearization,
   pchromatix_common)) {
  ...
  void *data)
{
  ...
  if (!linearization40_get_trigger_idx(isp_sub_module,
