  ...
  chromatix_parms_type *chromatix_ptr)
{
  ...
  chromatix_color_conversion_type *cv_led1 = NULL, *cv_led2 = NULL,
                                   cv_flash;
  ...
  if (led1_value_mainflash > 0 && led2_value_mainflash > 0 ) {
    ...
    led_mainflash_ratio =
      (float)led1_value_mainflash/(led1_value_mainflash +
        led2_value_mainflash);
    cv_led1 = &(chromatix_CV_ptr->LED1_color_conversion);
    cv_led2 = &(chromatix_CV_ptr->LED2_color_conversion);
    chroma_enhan_interpolate(cv_led1, cv_led2, &cv_flash, led_mainflash_ratio);
  } else {
    if (led2_value_mainflash == 0.0)
      memcpy(&cv_flash, &(chromatix_CV_ptr->LED1_color_conversion),
        sizeof(chromatix_color_conversion_type));
    if (led1_value_mainflash == 0.0)
      memcpy(&cv_flash, &(chromatix_CV_ptr->LED2_color_conversion),
        sizeof(chromatix_color_conversion_type));
  }
