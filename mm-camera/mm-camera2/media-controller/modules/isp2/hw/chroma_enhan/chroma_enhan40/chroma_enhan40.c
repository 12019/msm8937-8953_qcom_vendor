  ...
  chromatix_color_conversion_type* out, float ratio)
{
  ...
  out->chroma.k_cb = Round(LINEAR_INTERPOLATION(in1->chroma.k_cb,
    in2->chroma.k_cb, ratio));
  out->chroma.k_cr = Round(LINEAR_INTERPOLATION(in1->chroma.k_cr,
    in2->chroma.k_cr, ratio));
 out->luma.k = Round(LINEAR_INTERPOLATION(in1->luma.k, in2->luma.k, ratio));
