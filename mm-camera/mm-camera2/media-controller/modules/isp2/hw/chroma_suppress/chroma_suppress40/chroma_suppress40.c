...
static void chroma_suppress40_cmd_config(chroma_suppress40_t *chroma_suppress)
{
  ...
  Diff = Round(chroma_suppress->thresholds.cs_luma_threshold2 -
  ...
  Q_slope = (int)ceil(log((double)Diff) / log(2.0)) + 6;
  chroma_suppress->reg_mix_cmd_2.reg_cmd.cSupM1 = Round((float)(1 << Q_slope) / Diff);
