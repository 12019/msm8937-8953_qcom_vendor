  ...
  chromatix_abf34_core_type   *abf34_core_end, float ratio)
{
  ...
  temp = Round(LINEAR_INTERPOLATION(((float)CLAMP_ABF
  ...
  temp = Round(LINEAR_INTERPOLATION((float)abf34_core_start->bpc_minshft,
  ...
  temp = Round(LINEAR_INTERPOLATION(
  ...
  abf48_t *abf)
{
  ...
  if ((ratio == 1.0f) || ((abf->trigger_index + 1) == MAX_LIGHT_TYPES_FOR_SPATIAL))
 ...
 * Returns TRUE on success and FALSE on failure
 **/
static boolean abf48_prepare_table(abf48_t *abf)
{
  boolean                           ret = TRUE;
  int i, j;
  ...
  bx_temp = (int32_t)(abf->init_h_offset
    - abf->sensor_out_info.sensor_rolloff_config.output_width/2);
  bx_temp = Clamp(bx_temp,
    ABF34_BPC_BX_MIN, ABF34_BPC_BX_MAX);
  abf->v34RegCmd.cfg_cmd2.rnrCfg.bx = abf_adjust_signbit(bx_temp, 14);

  by_temp = (int32_t)(abf->init_v_offset
    - abf->sensor_out_info.sensor_rolloff_config.output_height/2);
 ...
 *  Return TRUE on success and FALSE on failure
 **/
static boolean abf48_calcualte_hdr_ratio(isp_sub_module_t
 ...
 *  Return TRUE on success and FALSE on failure
 **/
static boolean compute_abf34_denoise_adj(isp_sub_module_t
   *isp_sub_module, abf48_t *abf)
{
  ...
  abf48_t *abf)
{
  ...
  if ((abf3_hdr_adj_data->enable) && (abf->hdr_enable)) {
    compute_abf34_denoise_adj(isp_sub_module, abf);
  }
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  memcpy(abf_dmi_info->isp_meta_dump,
    &abf->abf34_parms.noise_std_lut_level,
    sizeof(noise_std2_lut_t));
  }
