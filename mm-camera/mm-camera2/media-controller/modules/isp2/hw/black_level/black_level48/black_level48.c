 ...
 **/
static void black_level48_update_params(black_level48_t *black_level,
  float BLSS_offset, int BLSS_is_v2, cam_sensor_hdr_type_t hdr_mode)
{
  ...
  black_level->Reg.black_level_cfg.scale =
    CLAMP(FLOAT_TO_Q(11, 1), 0, SET_FIRST_N_BITS(12));
 ...
 **/
static boolean black_level48_hw_config(isp_sub_module_t *isp_sub_mod,
  black_level48_t *black_level)
{
  ...
  black_level48_update_params(black_level, BLSS_offset,
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  if ((isp_sub_module->submod_trigger_enable == TRUE) &&
    ...
    ret = black_level48_hw_config(isp_sub_module, black_level);
    if (ret == FALSE) {
  ...
  black_level48_t *black_level)
{
  ...
  black_level48_update_params(black_level, blk_offset,
