    ...
    chromatix_parms_type *chromatix_ptr)
{
  ...
  p_cmd->interpClassifier.a_n = data->ak[0] << multifactor;
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  if (isp_sub_module->trigger_update_pending == TRUE) {
    ...
    p_cmd->interpClassifier.a_n = roundf(LINEAR_INTERPOLATION(
      normal_data->ak[0], lowlight_data->ak[0], ratio) * multi_factor);
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  demosaic48_set_chromatix_params(mod,
