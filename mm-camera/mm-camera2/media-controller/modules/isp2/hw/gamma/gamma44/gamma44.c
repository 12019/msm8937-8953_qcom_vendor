 ...
 *    contains the current GammaTable value
 */
int32_t gamma44_get_hi_lo_gamma_bits(uint16_t *table, int i)
{
 ...
 *
 */
int32_t gamma44_get_last_gamma_value(uint16_t *table)
{
  int32_t       hw_lut_entry = 0;
  ...
  gamma44_t* gamma, gamma_table_type gamma_table_type)
{
  ...
  case GAMMA_TABLE_LOWLIGHT:
    ...
    case GAMMA_TABLE_WHITE_BOARD:
      ...
      gamma44_get_downsampled_tables(&gamma->gamma_table_rgb,
 ...
 *  Return:   None
 **/
void gamma44_interp_gamma_table(uint16_t *dest,
    ...
    void *data3 __unused)
{
  ...
  if (gamma->gamma_ratio.lighting == TRIGGER_OUTDOOR)
    gamma44_interpolate(&gamma->default_gamma_tbl,
