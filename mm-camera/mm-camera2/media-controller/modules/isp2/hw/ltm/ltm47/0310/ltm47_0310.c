  ...
  ltm47_t *mod, int backlit_enable)
{
    ...
    for (i = 0; i < LTM_HW_LUT_SIZE + 1; i++) {
      ...
      if ((chromatix_ltm_normal &&
        ...
        shift_scale_backlit[i] = ((float)LINEAR_INTERPOLATION_LTM(
          ...
          backlit_trigger_idx + 1),
          backlit_ratio));
        mod->master_curve[i] = (int)Round((float)LINEAR_INTERPOLATION_LTM(
  ...
  for (i = 0; i < LTM_HW_LUT_SIZE + 1; i++) {
    if ((chromatix_ltm_normal &&
      ...
      chromatix_ltm_backlit->curves_from_core_section_en)) {
      if (backlit_enable) {
      mod->unpacked_master_scale[i] = (int)Round((float)
        LINEAR_INTERPOLATION_LTM(
      ...
      mod->unpacked_shift_scale[i] =
        ...
        mod->unpacked_master_scale[i] = (int)Round(master_scale_normal[i]);
  ...
  if (backlit_enable)
    ltm47_setup_dark_bright_region_index(chromatix_ltm_backlit,
      mod, mod->unpacked_master_scale);
  else
    ltm47_setup_dark_bright_region_index(chromatix_ltm_normal,
      mod, mod->unpacked_master_scale);
