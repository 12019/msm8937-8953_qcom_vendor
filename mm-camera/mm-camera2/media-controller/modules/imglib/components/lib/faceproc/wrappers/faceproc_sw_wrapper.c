...
static fpsww_config_t default_config;
static bool default_config_available = FALSE;
  ...
  faceproc_info_t *p_output, face_part_detect *p_pt_results)
{
  ...
  if ((uint32_t)p_output->fd_confidence >= p_hnd->config.filter_params.discard_face_below) {
    IDBG_MED("Do not discard confidence is fine %d  below %d",
      p_output->fd_confidence, p_hnd->config.filter_params.discard_face_below);
    return 0;
  }
...
/**
 * Function: wrapper_get_default_config
 *
 * Description: Gets the dfault configuration of sw faceproc library and saves into glabl config structure.
 *
 * Input parameters:
 *   p_hnd - faceproc sw wrapper handle
 *
 ...
 * Notes: None
 **/
static int wrapper_get_default_config(faceproc_sw_wrap_t *p_hnd)
{
  if (!p_hnd) {
  ...
  if (default_config_available == TRUE) {
    return IMG_SUCCESS;
  }

  int rc;

  rc = fpsww_co_get_config(p_hnd, &default_config.co_params);
  if (rc != IMG_SUCCESS) {
    IDBG_ERROR("Failed in getting CO config");
    return rc;
  }

  rc = fpsww_dt_get_config(p_hnd, &default_config.dt_params);
  if (rc != IMG_SUCCESS) {
  ...
  if (p_hnd->create_params.create_face_parts == TRUE) {
    rc = fpsww_pt_get_config(p_hnd, &default_config.pt_params);
    if (rc != IMG_SUCCESS) {
      IDBG_ERROR("Failed in getting PT config");
      return rc;
    }

    rc = fpsww_ct_get_config(p_hnd, &default_config.ct_params);
    if (rc != IMG_SUCCESS) {
      IDBG_ERROR("Failed in getting CT config");
      return rc;
    }

    rc = fpsww_sm_get_config(p_hnd, &default_config.sm_params);
    if (rc != IMG_SUCCESS) {
      IDBG_ERROR("Failed in getting SM config");
      return rc;
    }

    rc = fpsww_gb_get_config(p_hnd, &default_config.gb_params);
    if (rc != IMG_SUCCESS) {
  ...
  if (p_hnd->create_params.create_face_recog == TRUE) {
    rc = fpsww_fr_get_config(p_hnd, &default_config.fr_params);
    if (rc != IMG_SUCCESS) {
...
static int wrapper_filter_print_config(faceproc_sw_wrap_t *p_hnd, fpsww_filter_params_t *p_filter_params)
{
  ...
  IDBG_INFO("weights                       : %f(eye), %f(mouth), %f(nose), %f(face)",
    p_filter_params->weight_eyes, p_filter_params->weight_mouth, p_filter_params->weight_nose, p_filter_params->weight_face);
...
static int wrapper_print_current_config(faceproc_sw_wrap_t *p_hnd)
{
  ...
  IDBG_INFO("enable_facial_parts : %d", p_hnd->config.enable_facial_parts);
  IDBG_INFO("enable_contour      : %d", p_hnd->config.enable_contour);
  IDBG_INFO("enable_smile        : %d", p_hnd->config.enable_smile);
  IDBG_INFO("enable_gaze         : %d", p_hnd->config.enable_gaze);
  IDBG_INFO("enable_blink        : %d", p_hnd->config.enable_blink);
  IDBG_INFO("enable_recog        : %d", p_hnd->config.enable_recog);
...
//  uint32_t dens_offset = 0;
  ...
  if ((uint32_t)p_output->fd_confidence >= p_hnd->config.filter_params.discard_face_below) {
    IDBG_MED("[FD_FALSE_POS_DBG] Do not discard confidence is fine %d below %d",
      p_output->fd_confidence, p_hnd->config.filter_params.discard_face_below);
    return IMG_SUCCESS;
  }
  ...
  if (p_output->face_boundary.x + p_output->face_boundary.dx >
    p_frame->frame[0].plane[0].width - p_hnd->config.filter_params.sw_face_discard_border) {
    box_out_of_frame = 1;
  }

  if (p_output->face_boundary.y + p_output->face_boundary.dy >
    p_frame->frame[0].plane[0].height - p_hnd->config.filter_params.sw_face_discard_border) {
    box_out_of_frame = 1;
  }

  if (box_out_of_frame) {
    if (p_hnd->config.filter_params.sw_face_discard_out) {
      IDBG_HIGH("[FD_FALSE_POS_DBG] Discard out of border");
      return IMG_ERR_NOT_FOUND;
    } else {
      IDBG_HIGH("[FD_FALSE_POS_DBG] Allow out of border");
      return IMG_SUCCESS;
    }
  }
  ...
  int density = p_hnd->config.filter_params.sw_face_search_dens;
  if (face_tracked > 0) {
    density -= 5;
  }
  ...
  if (density <= 25) {
    dt_params.nSearchDensity = DENSITY_HIGHEST;
  } else if (density <= 30) {
    dt_params.nSearchDensity = DENSITY_HIGH;
  } else if (density <= 34) {
    dt_params.nSearchDensity = DENSITY_NORMAL;
  } else if (density <= 37) {
    dt_params.nSearchDensity = DENSITY_LOW;
  } else {
    dt_params.nSearchDensity = DENSITY_LOWEST;
  }
  ...
  if ((p_output->gaze_angle == FD_POSE_RIGHT_ANGLE) || (p_output->gaze_angle == FD_POSE_LEFT_ANGLE)) {
      an_still_angle[FPSWW_ANGLE_FULLPROFILE] = angle_mask;
  }

  if (p_old != NULL) { // in case it's not first latch
    if (p_old->sw_fd_result) {
      IDBG_MED("[FD_DBG_SW_OR] HWConf=%03d, HWRoll=%03d, HWPose=%03d, "
      ...
      if (abs(p_old->gaze_angle)==0) {
        an_still_angle[FPSWW_ANGLE_FRONT]       = facial_angle_mask(p_old->face_angle_roll, 3);
        an_still_angle[FPSWW_ANGLE_HALFPROFILE] = facial_angle_mask(p_old->face_angle_roll, 2);
      } else if (abs(p_old->gaze_angle)==45) {
        an_still_angle[FPSWW_ANGLE_FRONT]       = facial_angle_mask(p_old->face_angle_roll, 2);
        an_still_angle[FPSWW_ANGLE_HALFPROFILE] = facial_angle_mask(p_old->face_angle_roll, 3);
      } else if (abs(p_old->gaze_angle)==90) {
        // never happen -- error
        IDBG_ERROR("[FD_DBG_SW_OLD] Angle error=%03d", p_old->gaze_angle);
      } else {
        // error
        IDBG_ERROR("[FD_DBG_SW_OLD] Angle error=%03d", p_old->gaze_angle);
      }
  ...
  for (i = 0; i < num_faces; i++) {
    ...
    if (p_hnd->config.filter_params.sw_face_spread_tol != 0) {
      ...
      double FaceSqDist = (double)(DistX * DistX + DistY * DistY);
...
int faceproc_sw_wrapper_get_fd_info(void *p_handle, faceproc_info_t faceinfo[], int max_roi, int32_t *p_num_faces)
{
  ...
  *p_num_faces = num_faces;
  ...
  for (face_indx = 0; face_indx < num_faces; face_indx++) {
    ...
    rc = fpsww_dt_get_result_info(p_hnd, &detection_info, face_indx, FALSE);
    if (IMG_ERROR(rc)) {
    ...
    if (p_hnd->config.filter_params.lock_faces == TRUE) {
      rc = fpsww_dt_lock_unlock_face(p_hnd, detection_info.nID, TRUE);
      if (IMG_ERROR(rc)) {
    ...
    // Fill the face info
    faceinfo[face_indx].unique_id = abs(detection_info.nID);
    rc = wrapper_fill_face_info_from_dtresult(p_hnd, &faceinfo[face_indx], &detection_info);
    if (IMG_ERROR(rc)) {
      IDBG_ERROR("Failed in filling FacialParts info, rc=%d", rc);
      return rc;
    }
...
{
  return faceproc_sw_wrapper_set_config(p_handle, &default_config);
}
...
void *faceproc_sw_wrapper_create(fpsww_create_params_t *p_create_params)
{
  ...
  IDBG_HIGH("Create Params : engine=%d, create_face_parts=%d, create_face_recog=%d, "
    "max_face_count=%d, detection_mode=%d, handles=%d",
  ...
  rc = wrapper_get_default_config(p_hnd);
  if (rc != IMG_SUCCESS) {
    IDBG_ERROR("Failed in getting default config");
    goto error_config;
  }

  // Start with default config. Set default config as current config.
  p_hnd->config = default_config;
 ...
 *     IMG_xxx on failure
 **/
int faceproc_sw_wrapper_unload()
{
  int rc = IMG_SUCCESS;
  ...
  if (lib_counter == 0) {
    ...
    if (rc != IMG_SUCCESS) {
      IDBG_MED("Fail to unload faceproc sw library, rc=%d", rc);
      return rc;
    }
  }
