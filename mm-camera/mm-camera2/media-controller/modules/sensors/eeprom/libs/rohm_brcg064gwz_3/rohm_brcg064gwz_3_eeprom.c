 ...
 * Return: void.
 **/
static void brcg064gwz_3_format_pdafgain(sensor_eeprom_data_t *e_ctrl)
{
  ...
  pdaf_ptr = e_ctrl->eeprom_params.buffer + PDAF_OFFSET;

  pdafc_2d->VersionNum = 2;
  pdafc_2d->PD_conversion_coeff[0] =
    (unsigned short)(pdaf_ptr[0]|pdaf_ptr[1] << 8);
  /*memory map has 3 bytes as PD conv coeff*/
  pdaf_ptr += 3;

  SHIGH("PDAF Gain map version %d", (pdaf_ptr[0] | pdaf_ptr[1] << 8));
  pdaf_ptr += 2;
...
static void brcg064gwz_3_format_calibration_data(void *e_ctrl) {
  ...
  brcg064gwz_3_format_pdafgain(ctrl);
