 ...
 * Return: void.
 **/
static unsigned char imx298_sunny_checksum(unsigned char *buf,
        ...
        sensor_eeprom_data_t *e_ctrl)
{
  ...
  SLOW("AWB : checksum");
  imx298_sunny_checksum((unsigned char *)(e_ctrl->eeprom_params.buffer + AWB_FLAG_OFFSET),0,8,9);
         ...
         sensor_eeprom_data_t *e_ctrl)
{
  ...
  */
  ...
  af_present = TRUE;
  SLOW("AF : checksum");
  imx298_sunny_checksum((unsigned char *)(e_ctrl->eeprom_params.buffer + AF_FLAG_OFFSET),0,6,7);
  /* Get AF data */
        ...
        sensor_eeprom_data_t *e_ctrl)
{
  ...
  lsc_present = TRUE;
  SLOW("LSC : checksum");
  imx298_sunny_checksum((unsigned char *)(e_ctrl->eeprom_params.buffer + LSC_FLAG_OFFSET),0,1768,1769);
...
static int sunny_gt24c64_imx298_eeprom_get_raw_data(void *e_ctrl, void *data)
{
  ...
  if (e_ctrl && data){
    if(is_checksum_error)
    	memcpy(data, &g_reg_setting_colorbar, sizeof(g_reg_setting_colorbar));
    else    	
...
static void sunny_gt24c64_imx298_eeprom_format_spcdata(sensor_eeprom_data_t *e_ctrl)
{
  ...
  spc_present = TRUE;
  SLOW("SPC : checksum");
  imx298_sunny_checksum((unsigned char *)(e_ctrl->eeprom_params.buffer + SPC_FLAG_OFFSET),0,252,253);
...
static void sunny_gt24c64_imx298_eeprom_format_dccdata(sensor_eeprom_data_t *e_ctrl)
{
  ...
  SLOW("DCC : checksum");
  imx298_sunny_checksum((unsigned char *)(e_ctrl->eeprom_params.buffer + DCC_FLAG_OFFSET),0,96,97);
  ...
  for(i = 0;i < PD_KNOT_X * PD_KNOT_Y;i++) {
    ...
    pdaf_data->SlopeData[i] = (float)(buffer[pdaf_offset + i * 2 + 1] << 8 |
      buffer[pdaf_offset + i * 2]) / 1024.0;
    pdaf_data->OffsetData[i] = 0;
    SLOW("OTP SlopeData[%d]: u6.10: 0x%x, float: %f", i,
     (buffer[pdaf_offset + i * 2 +1] << 8 | buffer[pdaf_offset + i * 2]),
