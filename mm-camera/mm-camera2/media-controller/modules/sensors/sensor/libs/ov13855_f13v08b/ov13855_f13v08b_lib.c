...
============================================================================*/
#include <stdio.h>
#include "ov13855_f13v08b_lib.h"
#include "debug_lib.h"
#include <utils/Log.h>

/**
 * FUNCTION: ov13855_f13v08b_real_to_register_gain
 *
 * DESCRIPTION: Calcuate sensor analog gain register value
 **/
static unsigned int ov13855_f13v08b_real_to_register_gain(float real_gain)
{
    unsigned int reg_gain = 0;

    if (real_gain < (float)OV13855_F13V08B_MIN_AGAIN)
    {
        real_gain = (float)OV13855_F13V08B_MIN_AGAIN;
        SHIGH("ov13855_f13v08b Requested gain(%f) is lower than the sensor minimum(%d)",
            real_gain, OV13855_F13V08B_MIN_AGAIN);
    }
    else if (real_gain > (float)OV13855_F13V08B_MAX_AGAIN)
    {
        real_gain = (float)OV13855_F13V08B_MAX_AGAIN;
    }
...
/**
 * FUNCTION: ov13855_f13v08b_register_to_real_gain
 *
 * DESCRIPTION: Calcuate sensor real gain value
 **/
static float ov13855_f13v08b_register_to_real_gain(unsigned int reg_gain)
{
    float real_gain;

    if (reg_gain < OV13855_F13V08B_MIN_AGAIN_REG_VAL)
    {
        reg_gain = OV13855_F13V08B_MIN_AGAIN_REG_VAL;
        SHIGH("ov13855_f13v08b Requested gain(%d) is lower than the sensor minimum(%d)",
            reg_gain, OV13855_F13V08B_MIN_AGAIN_REG_VAL);
    }
    else if(reg_gain > OV13855_F13V08B_MAX_AGAIN_REG_VAL)
    {
        reg_gain = OV13855_F13V08B_MAX_AGAIN_REG_VAL;
    }
...
/**
 * FUNCTION: ov13855_f13v08b_digital_gain_calc
 *
 * DESCRIPTION: Calcuate the sensor digital gain
 **/
static unsigned int ov13855_f13v08b_digital_gain_calc(float real_gain, float sensor_real_gain)
{
    unsigned int reg_dig_gain = OV13855_F13V08B_MIN_DGAIN_REG_VAL;
    float real_dig_gain = OV13855_F13V08B_MIN_DGAIN;

    if(real_gain > OV13855_F13V08B_MAX_AGAIN)
    {
        real_dig_gain = real_gain / sensor_real_gain;
    }
    else
    {
        real_dig_gain = OV13855_F13V08B_MIN_DGAIN;
    }

    if(real_dig_gain > OV13855_F13V08B_MAX_DGAIN)
    {
        real_dig_gain = OV13855_F13V08B_MAX_DGAIN;
    }
    reg_dig_gain = (unsigned int)(real_dig_gain * 128);
    return reg_dig_gain;
}
 ...
 **/
int ov13855_f13v08b_calculate_exposure(float real_gain,
  ...
  __attribute__((unused)) float s_real_gain)
{
    ...
    exp_info->reg_gain = ov13855_f13v08b_real_to_register_gain(real_gain);
    exp_info->sensor_real_gain =
      ov13855_f13v08b_register_to_real_gain(exp_info->reg_gain);
    exp_info->sensor_digital_gain =
      ov13855_f13v08b_digital_gain_calc(real_gain, exp_info->sensor_real_gain);
    exp_info->sensor_real_dig_gain =
      (float)exp_info->sensor_digital_gain / OV13855_F13V08B_MAX_DGAIN_DECIMATOR;
...
/**
 * FUNCTION: ov13855_f13v08b_fill_exposure_array
 *
 * DESCRIPTION: Fill the sensor exposure array
 **/
int ov13855_f13v08b_fill_exposure_array(unsigned int gain,
