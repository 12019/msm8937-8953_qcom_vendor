 ...
 */
 #define LED_CALIB_MEASUREMENT_COUNT 3
...
typedef struct {
  ...
  float rg_ratios[LED_CALIB_MEASUREMENT_COUNT]; /**< measured rg ratios */
  float bg_ratios[LED_CALIB_MEASUREMENT_COUNT]; /**< measured bg ratios */
}awb_led_calib_measurement;
...
typedef struct{
  ...
  q3a_led_calib_mode_t mode;

  /* Indicates */
  awb_led_calibration_point single_led_data;

  /* Indicates the current LED state during calibration */
  awb_dual_led_calibration_state dual_led_state;
  /* Calibration data points before linearization. These are either loaded from file or
   * captured during calibration */
  awb_led_calibration_point dual_led_data[AWB_DUAL_LED_CALIB_ON_STATE_COUNT];
} awb_led_calibaration_data_t;
...
#endif
  awb_led_calibaration_data_t led_calib_data;         /**< LED Calibration data and settings. */
} awb_led_data_type;
...
typedef struct
{
  ...
  awb_led_data_type       led_data;      /**< Dual led data */
...
typedef struct
{
  ...
  const awb_led_calibaration_data_t*            led_calib_data;             /**< Dual LED calibration data. */
...
typedef enum
{
  ...
  AWB_SET_PARAM_LED_CALIBRATION_MODE,         /**< LED Calibration mode. */
...
typedef struct
{
  ...
  union
  {
    ...
    awb_led_data_type             led_data;      /**< AWB dual led setting. */
