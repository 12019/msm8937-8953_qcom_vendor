...
/** awb_port_forward_update_event_if_linked:
  ...
  */
static boolean awb_port_forward_update_event_if_linked(
  mct_port_t* mct_port,
  stats_update_t* awb_update)
{
  awb_port_private_t* awb_port = (awb_port_private_t *)(mct_port->port_private);
  boolean result = true;

  /* Only forward the AWB update if we are the master camera, and the peer ID is set,
     indicating dual camera is in use */
  if (awb_port != NULL &&
      awb_port->dual_cam_sensor_info == CAM_TYPE_MAIN &&
      awb_port->intra_peer_id != 0)
  {

    /* Forward the AWB update info to the slave session */
    result = stats_util_post_intramode_event(mct_port,
                                             awb_port->intra_peer_id,
                                             MCT_EVENT_MODULE_STATS_PEER_AWB_UPDATE,
                                             (void*)awb_update);
    if (!result)
    {
      AWB_MSG_ERROR("Error! failed to forward the AWB Update event to the slave (id=%d)",
                    awb_port->intra_peer_id);
    }
  }
  return result;
...
#ifdef AWB_AUX_UPDATE_ENABLE
    awb_port_forward_update_event_if_linked(port,&output->stats_update);
#endif
...
#ifdef AWB_AUX_UPDATE_ENABLE
    awb_port_forward_update_event_if_linked(port,&output_p->stats_update);
#endif
    p_awb_update_for_meta = awb_update;
  } else { /* Auto mode */
 ...
 *    @port: private AWB port data
 *    @event: module event received
 *
 * Links to peer AWB Port
 *
 * Return boolean
 **/
      ...
      if (awb_msg != NULL ) {
        ...
        if (q3a_param->type == Q3A_SET_AWB_PARAM) {
          ...
          case Q3A_ALL_SET_LED_CALIB_MODE: {
            awb_msg->type = MSG_AWB_SET;
            awb_msg->u.awb_set_parm.type = AWB_SET_PARAM_LED_CALIB_MODE;
            switch (q3a_param->u.q3a_all_param.u.led_calib_mode) {
            case CAM_LED_CALIBRATION_MODE_OFF:
              awb_msg->u.awb_set_parm.u.led_calib_mode = Q3A_LED_CALIB_MODE_OFF;
              break;
            case CAM_LED_CALIBRATION_MODE_DUAL:
              awb_msg->u.awb_set_parm.u.led_calib_mode = Q3A_LED_CALIB_MODE_DUAL;
              break;
            case CAM_LED_CALIBRATION_MODE_SINGLE:
              awb_msg->u.awb_set_parm.u.led_calib_mode = Q3A_LED_CALIB_MODE_SINGLE;
              break;
            default:
              AWB_ERR("Invalid led calibration mode");
              rc = FALSE;
              break;
            }
...
/** awb_port_handle_peer_awb_update:
  *    @mct_port:   MCT port object
  *    @aec_update: AWB update info
  *
  * Handles a forwarded peer event for AWB update, where the master session has
  * forwarded the AWB update info to the slave session
  *
  * Return: void
  */
static void awb_port_handle_peer_awb_update(
  mct_port_t* mct_port,
  stats_update_t* stats_update)
{
  ...
  awb_port_send_event(mct_port, MCT_EVENT_MODULE_EVENT,
    MCT_EVENT_MODULE_STATS_AWB_UPDATE,
    (void *)(stats_update),awb_port->cur_sof_id);
#else
 (void)stats_update;
 #endif
}
...
static boolean awb_port_intra_event(mct_port_t *port, mct_event_t *event)
{
  ...
  if (!awb_port_check_identity(&private->reserved_id, &event->identity)) {
    return FALSE;
  }

  switch(event->u.module_event.type)
  {
    case MCT_EVENT_MODULE_STATS_PEER_AWB_UPDATE:
      AWB_LOW("Received MCT_EVENT_MODULE_STATS_PEER_AWB_UPDATE");
      awb_port_handle_peer_awb_update(port,
          (stats_update_t*)event->u.module_event.module_event_data);
      break;

    default:
      AWB_ERR("Error! Received unknown intra-module event type: %d",
