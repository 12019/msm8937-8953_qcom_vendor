  ...
  is_output_type *is_output, int stream_type)
{
  ...
  if (is_info->is_type[stream_type] == IS_TYPE_EIS_2_0) {
    ...
    memset(&eis3_input, 0, sizeof(eis3_input_t));
    eis3_input.frame_times = frame_times;
    if (gyro_data->sample_len > 0) {
      eis3_input.gyro_times.first_gyro_ts = gyro_data->sample[0].timestamp;
      eis3_input.gyro_times.last_gyro_ts =
        gyro_data->sample[gyro_data->sample_len - 1].timestamp;
    }
    eis3_process(is_info->eis3_handle, &eis3_input, is_output);
  }
      ...
      if (is_info->is_inited[i]) {
         ...
         */
         if (is_info->is_type[IS_PREVIEW] == is_info->is_type[IS_VIDEO] &&
           ...
           memcpy(&is_output[IS_VIDEO], &is_output[IS_PREVIEW], sizeof(is_output_type));
         }
      }
    }
  }
    ...
    switch (param->u.stream_event_data.stream_event) {
      case IS_PREVIEW_STREAM_ON:
        if (!is_info->use_stats && is_info->run_is[IS_PREVIEW] && is_info->is_enabled[IS_PREVIEW]) {
          is_process_is_initialize(param->u.stream_event_data.is_info, IS_PREVIEW);
        }
        break;
      ...
      case IS_VIDEO_STREAM_ON:
        if (!is_info->use_stats && is_info->run_is[IS_VIDEO] && is_info->is_enabled[IS_VIDEO] &&
          ...
          is_process_is_initialize(param->u.stream_event_data.is_info, IS_VIDEO);
        }
        break;

      case IS_PREVIEW_STREAM_OFF:
        if (param->u.stream_event_data.is_info->is_inited[IS_PREVIEW]) {
          is_process_is_deinitialize(param->u.stream_event_data.is_info, IS_PREVIEW);
        }
        break;

      case IS_VIDEO_STREAM_OFF:
        if (param->u.stream_event_data.is_info->is_inited[IS_VIDEO]) {
          is_process_is_deinitialize(param->u.stream_event_data.is_info, IS_VIDEO);
        }
        break;
