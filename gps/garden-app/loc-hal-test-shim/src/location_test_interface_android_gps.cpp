...
AGpsInterface *pAgpsInterface = NULL;
GpsNiInterface *pGpsNiInterface = NULL;
AGpsRilInterface *pAgpsRilInterface = NULL;
GpsXtraInterface *pXtraInterface = NULL;
...
GpsXtraCallbacks myXtraCallbacks = {
    test_android_gps_xtra_download_req_cb,
    test_android_gps_create_thread_cb
};

AGpsCallbacks myAgpsCallbacks = {
    test_android_agps_status_cb,
    test_android_gps_create_thread_cb
};

GpsNiCallbacks myGpsNiCallbacks = {
...
int location_test_interface_android_gps::location_init(garden_cb_t *pcb) {
    ...
    pAgpsInterface = (AGpsInterface *) pGpsInterface->get_extension (AGPS_INTERFACE);
    if (NULL == pAgpsInterface) {
        return -1;
    }
    pAgpsInterface->init (&myAgpsCallbacks);

    pGpsNiInterface = (GpsNiInterface *) pGpsInterface->get_extension (GPS_NI_INTERFACE);
    ...
    pXtraInterface =  (GpsXtraInterface *) pGpsInterface->get_extension (GPS_XTRA_INTERFACE);
