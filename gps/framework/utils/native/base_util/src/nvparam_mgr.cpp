...
private:
  ...
  // internal functions
  int wrapper_mprintf (char* &output_buf, int &output_size,
                       const char *sentence,... );
...
int NvParamMgrImpl::wrapper_mprintf(char* &output_buf, int &output_size,
                                      const char *sentence,... )
{
  int result = -1;
  int nchars = 0;
  ...
  do
  {
    ...
    if ((output_buf == NULL) || ((nchars + 1) > output_size))
    {
      ...
      if ((nchars + 1) > output_size)
      {
        output_size = nchars + 1;
      }
