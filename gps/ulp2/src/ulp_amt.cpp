...
{
   int                   ret_val = -1;
   GpsPositionRecurrence dr_recurrence_type    = GPS_POSITION_RECURRENCE_SINGLE;
   uint32_t              dr_fix_interval       = 0; // -1: not set yet
   GpsPositionMode       dr_mode               = GPS_POSITION_MODE_STANDALONE;
   ...
   do
   {
      ...
      {
         dr_recurrence_type = GPS_POSITION_RECURRENCE_PERIODIC;
      }
      else
      {
         dr_recurrence_type = GPS_POSITION_RECURRENCE_SINGLE;
      }

      dr_fix_interval = ulp_data.gnss_fix_interval;
      // If there is a high accuracy fix and QUIPC is enabled to run,
      // use TBF of 1 second
      if (dr_recurrence_type == GPS_POSITION_RECURRENCE_SINGLE)
      {
         dr_recurrence_type = GPS_POSITION_RECURRENCE_PERIODIC;
