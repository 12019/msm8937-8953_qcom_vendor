...
/* package */class StandalonePanoramaEngine implements PanoramaEngine {
    ...
    private boolean debugSaveKeyFrames;
    private int debugPreviewFrameIndex;
    private int debugKeyFrameIndex;
    ...
    private class DoInitRunnable implements Runnable {
        ...
        public void run() {
            ...
            debugPerformanceLogging = CameraPreferenceManager.getEnableLogging(context);
            debugSavePreviewFrames = CameraPreferenceManager.getSavePreviewFrames(context);
            debugSaveKeyFrames = CameraPreferenceManager.getSaveKeyFrames(context);
    ...
    private class DoStartCaptureRunnable extends EventListenerRunnable {
        ...
        public void run() {
            ...
            debugPreviewFrameIndex = 0;
            debugKeyFrameIndex = 0;
        ...
        public void run() {
            ...
            if (state == State.FIRST_PREVIEW_FRAME_PENDING) {
                ...
                if (nInstanceInit(frame.width, frame.height, frame.stride,
                        SCVE_COLORFORMAT_YUV_NV21, cameraActor.getFocalLength())) {
            ...
            if (AppError.shouldTerminateCapture(result)) {
                ...
                if (isKeyFrame[0]) {
                    DoKeyFrame doKeyFrame = new DoKeyFrame(position[0], position[1]);
            ...
            if (debugSavePreviewFrames) {
                ...
                AsyncTask.execute(new DebugSavePreviewFrameRunnable(debugKeyFrameIndex,
                        debugPreviewFrameIndex, frame));
            } else {
                frame.sem.release();
            }
            debugPreviewFrameIndex++;
            previewFrame = null;
        }
    }
    ...
    private class DoKeyFrame implements Runnable, CameraActor.OnKeyFrameCapturedListener {
        ...
        private final int debugLocalKeyFrameIndex = debugKeyFrameIndex;
        private final int x;
        private final int y;
        private volatile byte[] keyFrameJpegData;

        public DoKeyFrame(int x, int y) {
            if (debugPerformanceLogging) {
        ...
        public void run() {
            ...
            int result = nProcessKeyFrame(keyFrameJpegData, x, y);
            if (AppError.shouldTerminateCapture(result)) {
                final AppError error;
                switch (result) {
                default:
            ...
            if (debugSaveKeyFrames) {
                AsyncTask.execute(new DebugSaveKeyFrameRunnable(debugKeyFrameIndex,
                        keyFrameJpegData));
            }
            debugKeyFrameIndex++;
        }
    ...
    private native int nProcessKeyFrame(byte[] keyFrameJpegData, int x, int y);
