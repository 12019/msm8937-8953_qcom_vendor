...
public class CameraActor {
    ...
    public interface StillImageCaptureCallback {
         ...
         * neither {@link #onCaptured(ByteBuffer)} nor {@link #onCaptureFailed()} will be invoked.
         */
        void onCaptureAborted();

        /**
         * This method will be invoked when a still image capture starts. It will be (eventually) be
         * followed by a call to {@link #onCaptured(ByteBuffer)}, {@link #onCaptureFailed()}, or
         * {@link #onCaptureAborted()}.
         */
         ...
         */
        void onCaptured(ByteBuffer jpeg);
         ...
         * {@link #onCaptured(ByteBuffer)} nor {@link #onCaptureAborted()} will be invoked.
         */
    ...
    private static final String TAG = CameraActor.class.getSimpleName();
    ...
    private class PictureCaptureCallback extends CameraCaptureSession.CaptureCallback {
        ...
        public void onCapturedImageAvailable(Image im) {
            ...
            if (!isFacingFront) {
                // No horizontal reflection required, just duplicate JPEG-encoded image
                buf = ByteBuffer.allocateDirect(src.limit());
                buf.put(src);
                buf.position(0);
            } else {
                // Front-facing camera, must horizontally reflect the image
                // TODO: BitmapFactory.decodeStream does not take EXIF orientation into account.
                //       If a device uses EXIF tags to express rotation and/or mirroring, it needs
                //       to be included in the transformation Matrix below. Use JNI and libexif to
                //       parse the Orientation tag within the direct ByteBuffer.
                Bitmap b = BitmapFactory.decodeStream(new ByteBufferInputStream(src));
                Matrix m = new Matrix();
                m.setScale(-1f, 1f, b.getWidth() / 2f, b.getHeight() / 2f);
                Bitmap b2 = Bitmap.createBitmap(b, 0, 0, b.getWidth(), b.getHeight(), m, false);
                ByteArrayOutputStream os = new ByteArrayOutputStream();
                b2.compress(Bitmap.CompressFormat.JPEG, 80, os);
                buf = ByteBuffer.wrap(os.toByteArray());
            }

            callback.onCaptured(buf);
        }
    }
